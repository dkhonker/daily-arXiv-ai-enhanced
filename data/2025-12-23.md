<div id=toc></div>

# 目录

- [cs.AI](#cs.AI) [总数: 62]
- [cs.CL](#cs.CL) [总数: 74]


<div id='cs.AI'></div>

# cs.AI [[Back]](#toc)

### [1] [Conflict-Driven Clause Learning with VSIDS Heuristics for Discrete Facility Layout](https://arxiv.org/abs/2512.18034)
*Joshua Gibson, Kapil Dhakal*

**主要类别:** cs.AI

**AI概要:** 本研究探讨使用CDCL与VSIDS启发式算法解决离散设施布局问题，开发了CNF建模方法，比较了CDCL、CP-SAT和MILP的性能，并提出了两种混合架构来克服CDCL在优化方面的限制。


<details>
  <summary>更多</summary>
  
**动机:** 设施布局问题具有密集的逻辑结构约束，需要高效的求解方法。传统方法如CP-SAT和MILP在大规模问题上存在计算复杂度问题，而CDCL在可行性检测方面表现出色但缺乏优化能力。

**方法:** 将设施布局问题建模为组合分配问题，使用CNF形式化表示约束条件。比较CDCL、CP-SAT和MILP三种求解方法，并设计两种混合架构：快速枚举可行布局和生成热启动解加速精确优化。

**结果:** CDCL在可行性检测方面表现出近乎恒定的运行时间特性，而CP-SAT和MILP分别呈现多项式和指数级增长。混合方法显著减少求解时间同时保持正确性保证。

**结论:** 混合方法结合了CDCL的快速可行性搜索和CP-SAT的优化能力，在大规模离散布局问题中提供了有效的算法权衡方案，展示了子句学习搜索与精确优化方法的互补优势。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Conflict-Driven+Clause+Learning+with+VSIDS+Heuristics+for+Discrete+Facility+Layout，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18034，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18034&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper studies the use of Conflict-Driven Clause Learning (CDCL) with VSIDS heuristics as a computational engine for discrete facility layout problems. The facility layout problem is modeled as a combinatorial assignment problem with dense logical structure arising from adjacency, separation, and slot-availability constraints. We develop a CNF-based formulation for layout feasibility and compare CDCL-based SAT solving against CP-SAT and MILP formulations under a unified benchmarking framework. Empirical results show that CDCL exhibits near-constant runtime behavior for feasibility detection across increasing problem sizes and constraint densities, while CP-SAT and MILP display polynomial and exponential scaling respectively. To address the limitation of CDCL in objective optimization, we introduce two hybrid architectures that combine CDCL-based feasibility search with CP-SAT optimization. The first architecture rapidly enumerates feasible layouts to trade optimality for speed, while the second uses CDCL to generate warm-start solutions that accelerate exact optimization. The results demonstrate that hybrid approaches can significantly reduce time-to-solution while preserving correctness guarantees, clarifying the algorithmic trade-offs between clause-learning search and exact optimization methods in large-scale discrete layout problems.

</details>


### [2] [Faithful and Stable Neuron Explanations for Trustworthy Mechanistic Interpretability](https://arxiv.org/abs/2512.18092)
*Ge Yan, Tuomas Oikarinen, Tsui-Wei, Weng*

**主要类别:** cs.AI

**AI概要:** 该论文为神经元识别建立了首个理论框架，将其视为机器学习的逆过程，提出了忠实性和稳定性两个核心问题的理论保证，并开发了Bootstrap Explanation方法生成具有覆盖概率保证的概念预测集。


<details>
  <summary>更多</summary>
  
**动机:** 当前神经元识别方法（如Network Dissection和CLIP-Dissect）虽然经验上成功，但缺乏严格的理论基础，这阻碍了可信赖和可靠解释的实现。

**方法:** 将神经元识别视为机器学习的逆过程，推导了相似性度量（准确率、AUROC、IoU）的泛化边界来保证忠实性，提出了bootstrap集成程序量化稳定性，并开发了BE方法生成概念预测集。

**结果:** 在合成数据和真实数据上的实验验证了理论结果，证明了所提方法的实用性。

**结论:** 这项工作为可信赖的神经元识别迈出了重要一步，提供了首个理论分析框架和实用的稳定性量化方法。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Faithful+and+Stable+Neuron+Explanations+for+Trustworthy+Mechanistic+Interpretability，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18092，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18092&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Neuron identification is a popular tool in mechanistic interpretability, aiming to uncover the human-interpretable concepts represented by individual neurons in deep networks. While algorithms such as Network Dissection and CLIP-Dissect achieve great empirical success, a rigorous theoretical foundation remains absent, which is crucial to enable trustworthy and reliable explanations. In this work, we observe that neuron identification can be viewed as the inverse process of machine learning, which allows us to derive guarantees for neuron explanations. Based on this insight, we present the first theoretical analysis of two fundamental challenges: (1) Faithfulness: whether the identified concept faithfully represents the neuron's underlying function and (2) Stability: whether the identification results are consistent across probing datasets. We derive generalization bounds for widely used similarity metrics (e.g. accuracy, AUROC, IoU) to guarantee faithfulness, and propose a bootstrap ensemble procedure that quantifies stability along with BE (Bootstrap Explanation) method to generate concept prediction sets with guaranteed coverage probability. Experiments on both synthetic and real data validate our theoretical results and demonstrate the practicality of our method, providing an important step toward trustworthy neuron identification.

</details>


### [3] [Rethinking Multi-Agent Intelligence Through the Lens of Small-World Networks](https://arxiv.org/abs/2512.18094)
*Boxuan Wang, Zhuoyun Li, Xiaowei Huang, Yi Dong*

**主要类别:** cs.AI

**AI概要:** 该论文探讨了将小世界网络结构作为多智能体系统的设计先验，通过实验证明小世界连接能够在保持准确性和代币成本的同时显著稳定共识轨迹，并提出了基于不确定性的重连方案来扩展多智能体系统。


<details>
  <summary>更多</summary>
  
**动机:** 现有基于大语言模型的多智能体系统大多采用全连接图、简单稀疏环或临时动态选择，缺乏结构指导。研究旨在探索小世界网络连接作为多智能体系统设计先验的价值。

**方法:** 1) 将神经科学和复杂网络的见解与多智能体系统联系起来；2) 使用多智能体辩论作为受控测试平台；3) 引入基于不确定性的重连方案，使用LLM导向的不确定性信号在认知分歧的智能体之间添加远程快捷连接。

**结果:** 实验结果显示小世界连接结构能够：1) 产生几乎相同的准确性和代币成本；2) 显著稳定共识轨迹；3) 通过不确定性引导的重连方案产生可控制的小世界结构，适应任务难度和智能体异质性。

**结论:** 小世界网络先验可以作为多智能体系统设计的稳定器、鲁棒性增强器、可扩展协调器，并为涌现认知角色提供归纳偏置，具有广泛的设计意义。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Rethinking+Multi-Agent+Intelligence+Through+the+Lens+of+Small-World+Networks，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18094，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18094&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) have enabled multi-agent systems (MAS) in which multiple agents argue, critique, and coordinate to solve complex tasks, making communication topology a first-class design choice. Yet most existing LLM-based MAS either adopt fully connected graphs, simple sparse rings, or ad-hoc dynamic selection, with little structural guidance. In this work, we revisit classic theory on small-world (SW) networks and ask: what changes if we treat SW connectivity as a design prior for MAS? We first bridge insights from neuroscience and complex networks to MAS, highlighting how SW structures balance local clustering and long-range integration. Using multi-agent debate (MAD) as a controlled testbed, experiment results show that SW connectivity yields nearly the same accuracy and token cost, while substantially stabilizing consensus trajectories. Building on this, we introduce an uncertainty-guided rewiring scheme for scaling MAS, where long-range shortcuts are added between epistemically divergent agents using LLM-oriented uncertainty signals (e.g., semantic entropy). This yields controllable SW structures that adapt to task difficulty and agent heterogeneity. Finally, we discuss broader implications of SW priors for MAS design, framing them as stabilizers of reasoning, enhancers of robustness, scalable coordinators, and inductive biases for emergent cognitive roles.

</details>


### [4] [Efficient Mixture-of-Agents Serving via Tree-Structured Routing, Adaptive Pruning, and Dependency-Aware Prefill-Decode Overlap](https://arxiv.org/abs/2512.18126)
*Zijun Wang, Yijiahao Qi, Hanqiu Chen, Zishen Wan, Gongjin Sun, Dongyang Li, Shuyi Pei, Cong Hao*

**主要类别:** cs.AI

**AI概要:** MoA推理存在通信密集和硬件利用率低的问题，本文通过算法-系统协同设计提出分层树状拓扑、运行时自适应机制和流水线执行，显著降低延迟(高达90%)同时保持精度(±1%内)。


<details>
  <summary>更多</summary>
  
**动机:** 解决Mixture-of-Agents推理中的密集代理间通信和低硬件利用率问题，这两个因素共同导致服务延迟增加。

**方法:** 1. 用分层树状拓扑替代密集代理交互图，引入结构化稀疏通信；2. 基于语义一致性和置信度信号的运行时自适应机制，选择性终止或跳过下游代理调用；3. 通过重叠增量预填充和解码的流水线执行，提高利用率。

**结果:** 在代表性任务中，端到端延迟显著减少(高达90%)，同时保持与密集连接MoA基线相当的精度(在±1%范围内)，在某些设置中还能提高精度。

**结论:** 通过算法-系统协同设计，提出的分层通信、自适应机制和流水线执行方法有效解决了MoA推理的延迟问题，在保持精度的同时大幅提升性能。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Efficient+Mixture-of-Agents+Serving+via+Tree-Structured+Routing%2C+Adaptive+Pruning%2C+and+Dependency-Aware+Prefill-Decode+Overlap，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18126，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18126&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Mixture-of-Agents (MoA) inference can suffer from dense inter-agent communication and low hardware utilization, which jointly inflate serving latency. We present a serving design that targets these bottlenecks through an algorithm-system co-design. First, we replace dense agent interaction graphs with a hierarchical tree topology that induces structured sparsity in inter-agent communication. Second, we introduce a runtime adaptive mechanism that selectively terminates or skips downstream agent invocations using semantic agreement and confidence signals from intermediate outputs. Third, we pipeline agent execution by overlapping incremental prefilling with decoding across dependency-related agents, improving utilization and reducing inference latency. Across representative tasks, this approach substantially reduces end-to-end latency (up to 90%) while maintaining comparable accuracy (within $\pm$1%) relative to dense-connectivity MoA baselines, and can improve accuracy in certain settings.

</details>


### [5] [Unifying Causal Reinforcement Learning: Survey, Taxonomy, Algorithms and Applications](https://arxiv.org/abs/2512.18135)
*Cristiano da Costa Cunha, Wei Liu, Tim French, Ajmal Mian*

**主要类别:** cs.AI

**AI概要:** 本文系统综述了因果推断与强化学习的融合研究，分析了CRL如何通过建模因果关系解决传统RL在可解释性、鲁棒性和泛化性方面的局限，并提出了未来研究方向。


<details>
  <summary>更多</summary>
  
**动机:** 传统强化学习基于相关性决策，在分布偏移、混杂变量和动态环境中表现不佳。因果强化学习通过显式建模因果关系，为解决这些挑战提供了新途径。

**方法:** 采用系统性文献综述方法，将现有CRL方法分为五类：因果表示学习、反事实策略优化、离线因果RL、因果迁移学习和因果可解释性分析。

**结果:** 识别了当前CRL领域的主要挑战，总结了实际应用中的实证成功案例，并明确了该领域的开放性问题。

**结论:** 因果强化学习具有开发鲁棒、可泛化和可解释人工智能系统的巨大潜力，需要进一步研究来推动该领域发展。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Unifying+Causal+Reinforcement+Learning%3A+Survey%2C+Taxonomy%2C+Algorithms+and+Applications，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18135，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18135&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Integrating causal inference (CI) with reinforcement learning (RL) has emerged as a powerful paradigm to address critical limitations in classical RL, including low explainability, lack of robustness and generalization failures. Traditional RL techniques, which typically rely on correlation-driven decision-making, struggle when faced with distribution shifts, confounding variables, and dynamic environments. Causal reinforcement learning (CRL), leveraging the foundational principles of causal inference, offers promising solutions to these challenges by explicitly modeling cause-and-effect relationships. In this survey, we systematically review recent advancements at the intersection of causal inference and RL. We categorize existing approaches into causal representation learning, counterfactual policy optimization, offline causal RL, causal transfer learning, and causal explainability. Through this structured analysis, we identify prevailing challenges, highlight empirical successes in practical applications, and discuss open problems. Finally, we provide future research directions, underscoring the potential of CRL for developing robust, generalizable, and interpretable artificial intelligence systems.

</details>


### [6] [Propose, Solve, Verify: Self-Play Through Formal Verification](https://arxiv.org/abs/2512.18160)
*Alex Wilf, Pranjal Aggarwal, Bryan Parno, Daniel Fried, Louis-Philippe Morency, Paul Pu Liang, Sean Welleck*

**主要类别:** cs.AI

**AI概要:** PSV-Verus是一个通过自我博弈训练的代码生成模型，使用形式化验证提供可靠正确性信号，在三个基准测试中比推理和专家迭代基线提升最多9.6倍


<details>
  <summary>更多</summary>
  
**动机:** 研究在没有人类数据的情况下，仅通过自我博弈训练大型语言模型的有效性，特别是在代码生成领域，传统基于单元测试的奖励机制脆弱且容易传播错误

**方法:** 提出Propose, Solve, Verify (PSV)框架：使用形式化验证信号训练提议者生成具有挑战性的合成问题，并通过专家迭代训练求解器

**结果:** PSV-Verus在三个基准测试中pass@1指标比推理和专家迭代基线提升最多9.6倍，性能随生成问题数量和训练迭代次数而扩展

**结论:** 形式化验证和难度感知的提议是成功自我训练的关键要素，证明了自我博弈在验证代码生成设置中的有效性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Propose%2C+Solve%2C+Verify%3A+Self-Play+Through+Formal+Verification，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18160，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18160&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Training models through self-play alone (without any human data) has been a longstanding goal in AI, but its effectiveness for training large language models remains unclear, particularly in code generation where rewards based on unit tests are brittle and prone to error propagation. We study self-play in the verified code generation setting, where formal verification provides reliable correctness signals. We introduce Propose, Solve, Verify (PSV) a simple self-play framework where formal verification signals are used to create a proposer capable of generating challenging synthetic problems and a solver trained via expert iteration. We use PSV to train PSV-Verus, which across three benchmarks improves pass@1 by up to 9.6x over inference-only and expert-iteration baselines. We show that performance scales with the number of generated questions and training iterations, and through ablations identify formal verification and difficulty-aware proposal as essential ingredients for successful self-play.

</details>


### [7] [NEURO-GUARD: Neuro-Symbolic Generalization and Unbiased Adaptive Routing for Diagnostics -- Explainable Medical AI](https://arxiv.org/abs/2512.18177)
*Midhat Urooj, Ayan Banerjee, Sandeep Gupta*

**主要类别:** cs.AI

**AI概要:** NEURO-GUARD是一个结合视觉变换器和语言驱动推理的知识引导视觉框架，通过检索增强生成机制实现自我验证，在糖尿病视网膜病变分类和癫痫检测中显著提升准确率和跨域泛化能力


<details>
  <summary>更多</summary>
  
**动机:** 解决医学AI中基于图像的诊断面临的挑战：数据有限、视觉线索细微、临床决策高风险，现有视觉模型缺乏可解释性和跨域泛化能力

**方法:** 使用检索增强生成(RAG)机制，让大型语言模型迭代生成、评估和精炼医学图像特征提取代码，基于临床指南和专家知识进行接地处理

**结果:** 在四个糖尿病视网膜病变数据集上准确率比纯ViT基线提升6.2%(84.69% vs 78.4%)，跨域泛化提升5%，在MRI癫痫检测中也优于现有方法

**结论:** NEURO-GUARD成功将符号医学推理与亚符号视觉学习相结合，实现了可解释、知识感知和可泛化的医学图像诊断，在多个数据集上达到最先进性能

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是NEURO-GUARD%3A+Neuro-Symbolic+Generalization+and+Unbiased+Adaptive+Routing+for+Diagnostics+--+Explainable+Medical+AI，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18177，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18177&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Accurate yet interpretable image-based diagnosis remains a central challenge in medical AI, particularly in settings characterized by limited data, subtle visual cues, and high-stakes clinical decision-making. Most existing vision models rely on purely data-driven learning and produce black-box predictions with limited interpretability and poor cross-domain generalization, hindering their real-world clinical adoption. We present NEURO-GUARD, a novel knowledge-guided vision framework that integrates Vision Transformers (ViTs) with language-driven reasoning to improve performance, transparency, and domain robustness. NEURO-GUARD employs a retrieval-augmented generation (RAG) mechanism for self-verification, in which a large language model (LLM) iteratively generates, evaluates, and refines feature-extraction code for medical images. By grounding this process in clinical guidelines and expert knowledge, the framework progressively enhances feature detection and classification beyond purely data-driven baselines. Extensive experiments on diabetic retinopathy classification across four benchmark datasets APTOS, EyePACS, Messidor-1, and Messidor-2 demonstrate that NEURO-GUARD improves accuracy by 6.2% over a ViT-only baseline (84.69% vs. 78.4%) and achieves a 5% gain in domain generalization. Additional evaluations on MRI-based seizure detection further confirm its cross-domain robustness, consistently outperforming existing methods.
  Overall, NEURO-GUARD bridges symbolic medical reasoning with subsymbolic visual learning, enabling interpretable, knowledge-aware, and generalizable medical image diagnosis while achieving state-of-the-art performance across multiple datasets.

</details>


### [8] [NL2CA: Auto-formalizing Cognitive Decision-Making from Natural Language Using an Unsupervised CriticNL2LTL Framework](https://arxiv.org/abs/2512.18189)
*Zihao Deng, Yijia Li, Renrui Zhang, Peijun Ye*

**主要类别:** cs.AI

**AI概要:** NL2CA是一种自动从自然语言描述中形式化认知决策规则的新方法，通过LLM将文本转换为LTL逻辑，再通过无监督Critic Tree精炼，最终生成可执行的符号认知规则，实现了无需人工干预的自动化认知建模。


<details>
  <summary>更多</summary>
  
**动机:** 认知计算模型虽然能形式化地表征人类决策过程，但开发过程仍然需要大量人工劳动。现有方法要么完全依赖人工，要么需要人工指导的交互式建模，缺乏完全自动化的解决方案。

**方法:** 1. 使用微调的大型语言模型将文本翻译为线性时序逻辑(LTL)；2. 通过无监督的Critic Tree精炼逻辑；3. 将输出转换为与符号认知框架兼容的可执行产生式规则；4. 基于生成的规则构建认知智能体，并通过认知强化学习根据真实行为数据进行优化。

**结果:** 1. 在NL-to-LTL翻译任务中，CriticNL2LTL模块在专家和大规模基准测试中均表现一致，无需人工反馈；2. 在认知驾驶模拟中，从人类访谈自动构建的智能体成功学习了约70个不同关键场景中的多样化决策模式。

**结论:** NL2CA能够从非结构化文本数据实现可扩展、可解释且与人类对齐的认知建模，为自动设计符号认知智能体提供了新范式。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是NL2CA%3A+Auto-formalizing+Cognitive+Decision-Making+from+Natural+Language+Using+an+Unsupervised+CriticNL2LTL+Framework，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18189，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18189&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Cognitive computing models offer a formal and interpretable way to characterize human's deliberation and decision-making, yet their development remains labor-intensive. In this paper, we propose NL2CA, a novel method for auto-formalizing cognitive decision-making rules from natural language descriptions of human experience. Different from most related work that exploits either pure manual or human guided interactive modeling, our method is fully automated without any human intervention. The approach first translates text into Linear Temporal Logic (LTL) using a fine-tuned large language model (LLM), then refines the logic via an unsupervised Critic Tree, and finally transforms the output into executable production rules compatible with symbolic cognitive frameworks. Based on the resulted rules, a cognitive agent is further constructed and optimized through cognitive reinforcement learning according to the real-world behavioral data. Our method is validated in two domains: (1) NL-to-LTL translation, where our CriticNL2LTL module achieves consistent performance across both expert and large-scale benchmarks without human-in-the-loop feed-backs, and (2) cognitive driving simulation, where agents automatically constructed from human interviews have successfully learned the diverse decision patterns of about 70 trials in different critical scenarios. Experimental results demonstrate that NL2CA enables scalable, interpretable, and human-aligned cognitive modeling from unstructured textual data, offering a novel paradigm to automatically design symbolic cognitive agents.

</details>


### [9] [External Hippocampus: Topological Cognitive Maps for Guiding Large Language Model Reasoning](https://arxiv.org/abs/2512.18190)
*Jian Yan*

**主要类别:** cs.AI

**AI概要:** 提出了External Hippocampus框架，从认知动力学角度建模语言模型推理，通过降维投影构建拓扑认知地图，实现能量流的精确导航和干预，无需额外训练即可显著提升小模型的多步推理能力。


<details>
  <summary>更多</summary>
  
**动机:** 解决传统权重空间优化方法计算成本高的问题，以及小模型在多步推理中遇到的认知死锁问题，提供更高效可控的推理解决方案。

**方法:** 采用降维投影技术构建拓扑认知地图，通过温度扰动干预能量流动，避免认知涡旋和低熵势阱导致的推理停滞。

**结果:** 在≤7B参数模型上，地图引导方法在500个挑战性问题中达到81.20%准确率（相对基线+16.80%），推理时间减少≥15倍，温度扰动能有效重启能量流。

**结论:** 该框架无需额外训练，具有自主增长能力，为小模型推理提供了高效且可控的拓扑感知解决方案，揭示了推理停滞的认知动力学机制。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是External+Hippocampus%3A+Topological+Cognitive+Maps+for+Guiding+Large+Language+Model+Reasoning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18190，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18190&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper proposes the External Hippocampus framework, which models language model reasoning from a cognitive dynamics perspective as the flow of information energy in semantic space. Unlike traditional weight-space optimization methods, this framework constructs topological cognitive maps through dimensionality reduction projection, enabling precise navigation and intervention of energy flow at test time while avoiding substantial computational requirements and demonstrating predictable intervention patterns. The method effectively addresses the cognitive deadlock problem in multi-step reasoning for small models. Experiments on models <=7B parameters show: map-guided methods achieve 81.20% accuracy on 500 challenging problems (relative baseline +16.80%), reduce reasoning time by >= 15x, with key findings revealing that reasoning stagnation manifests as "Cognitive Vortex" and low-entropy potential wells, while temperature perturbations effectively restart energy flow. The framework requires no additional training, possesses autonomous growth capability, and provides an efficient and controllable topological-aware solution for small model reasoning.

</details>


### [10] [Sophia: A Persistent Agent Framework of Artificial Life](https://arxiv.org/abs/2512.18202)
*Mingyang Sun, Feng Hong, Weinan Zhang*

**主要类别:** cs.AI

**AI概要:** 论文提出了System 3概念，作为AI代理的第三层级，专注于身份认同和长期适应，并通过Sophia原型实现，实现了推理步骤减少80%和高复杂度任务成功率提升40%的效果。


<details>
  <summary>更多</summary>
  
**动机:** 当前AI代理系统主要专注于感知和推理(System 1和System 2)，但缺乏维持身份认同、验证推理过程以及将短期行动与长期生存目标对齐的持续性元层机制。

**方法:** 提出System 3框架，将心理学概念映射到计算模块，开发了Sophia持久代理包装器，包含四个协同机制：过程监督思维搜索、叙事记忆、用户与自我建模、混合奖励系统。

**结果:** 定量结果：推理步骤减少80%，高复杂度任务成功率提升40%；定性结果：展现出连贯的叙事身份认同和内在的任务组织能力。

**结论:** 通过将心理学洞见与轻量级强化学习核心相结合，持久代理架构为人工生命提供了一条可行的实践路径，实现了身份连续性和透明的行为解释。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Sophia%3A+A+Persistent+Agent+Framework+of+Artificial+Life，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18202，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18202&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The development of LLMs has elevated AI agents from task-specific tools to long-lived, decision-making entities. Yet, most architectures remain static and reactive, tethered to manually defined, narrow scenarios. These systems excel at perception (System 1) and deliberation (System 2) but lack a persistent meta-layer to maintain identity, verify reasoning, and align short-term actions with long-term survival. We first propose a third stratum, System 3, that presides over the agent's narrative identity and long-horizon adaptation. The framework maps selected psychological constructs to concrete computational modules, thereby translating abstract notions of artificial life into implementable design requirements. The ideas coalesce in Sophia, a "Persistent Agent" wrapper that grafts a continuous self-improvement loop onto any LLM-centric System 1/2 stack. Sophia is driven by four synergistic mechanisms: process-supervised thought search, narrative memory, user and self modeling, and a hybrid reward system. Together, they transform repetitive reasoning into a self-driven, autobiographical process, enabling identity continuity and transparent behavioral explanations. Although the paper is primarily conceptual, we provide a compact engineering prototype to anchor the discussion. Quantitatively, Sophia independently initiates and executes various intrinsic tasks while achieving an 80% reduction in reasoning steps for recurring operations. Notably, meta-cognitive persistence yielded a 40% gain in success for high-complexity tasks, effectively bridging the performance gap between simple and sophisticated goals. Qualitatively, System 3 exhibited a coherent narrative identity and an innate capacity for task organization. By fusing psychological insight with a lightweight reinforcement-learning core, the persistent agent architecture advances a possible practical pathway toward artificial life.

</details>


### [11] [MSC-180: A Benchmark for Automated Formal Theorem Proving from Mathematical Subject Classification](https://arxiv.org/abs/2512.18256)
*Sirui Li, Wangyue Lu, Xiaorui Shi, Ke Weng, Haozhe Sun, Minghe Yu, Tiancheng Zhang, Ge Yu, Hengyu Liu, Lun Du*

**主要类别:** cs.AI

**AI概要:** MSC-180是一个基于MSC2020数学学科分类的定理证明基准测试集，包含180个形式化验证问题，覆盖60个数学分支。评估显示当前LLM定理证明器存在严重的领域偏见和泛化能力不足问题。


<details>
  <summary>更多</summary>
  
**动机:** 解决当前基于大语言模型的定理证明器在数学推理中存在的领域覆盖受限和泛化能力弱的问题，需要更系统化的评估基准。

**方法:** 构建MSC-180基准测试集，包含180个经过专家验证的形式化问题，涵盖60个数学分支，每个分支3个问题，从本科到研究生难度。引入变异系数(CV)作为评估指标来量化性能变异性。

**结果:** 最佳模型在pass@32设置下仅达到18.89%的整体通过率，最大领域覆盖率41.7%，研究生级别问题通过率显著更低。CV值是统计高变异性阈值的4-6倍，表明模型依赖训练语料的模式匹配而非可迁移的推理机制。

**结论:** MSC-180基准及其多维评估框架为开发具有真正数学推理能力的下一代AI系统提供了区分性和系统性的基准测试工具，揭示了当前LLM在数学定理证明中的局限性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是MSC-180%3A+A+Benchmark+for+Automated+Formal+Theorem+Proving+from+Mathematical+Subject+Classification，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18256，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18256&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Automated Theorem Proving (ATP) represents a core research direction in artificial intelligence for achieving formal reasoning and verification, playing a significant role in advancing machine intelligence. However, current large language model (LLM)-based theorem provers suffer from limitations such as restricted domain coverage and weak generalization in mathematical reasoning. To address these issues, we propose MSC-180, a benchmark for evaluation based on the MSC2020 mathematical subject classification. It comprises 180 formal verification problems, 3 advanced problems from each of 60 mathematical branches, spanning from undergraduate to graduate levels. Each problem has undergone multiple rounds of verification and refinement by domain experts to ensure formal accuracy. Evaluations of state-of-the-art LLM-based theorem provers under the pass@32 setting reveal that the best model achieves only an 18.89% overall pass rate, with prominent issues including significant domain bias (maximum domain coverage 41.7%) and a difficulty gap (significantly lower pass rates on graduate-level problems). To further quantify performance variability across mathematical domains, we introduce the coefficient of variation (CV) as an evaluation metric. The observed CV values are 4-6 times higher than the statistical high-variability threshold, indicating that the models still rely on pattern matching from training corpora rather than possessing transferable reasoning mechanisms and systematic generalization capabilities. MSC-180, together with its multi-dimensional evaluation framework, provides a discriminative and systematic benchmark for driving the development of next-generation AI systems with genuine mathematical reasoning abilities.

</details>


### [12] [Intelligent Human-Machine Partnership for Manufacturing: Enhancing Warehouse Planning through Simulation-Driven Knowledge Graphs and LLM Collaboration](https://arxiv.org/abs/2512.18265)
*Himabindu Thogaru, Saisubramaniam Gopalakrishnan, Zishan Ahmad, Anirudh Deodhar*

**主要类别:** cs.AI

**AI概要:** 提出基于知识图谱和大语言模型的协作智能系统，通过自然语言接口实现制造规划中人与AI的高效协作，在瓶颈识别等场景中达到近乎完美的准确性。


<details>
  <summary>更多</summary>
  
**动机:** 传统仿真数据分析方法在人类决策者与关键运营洞察之间制造障碍，限制了制造规划中的有效合作，需要弥合人机协作的鸿沟。

**方法:** 开发集成知识图谱和LLM代理的协作智能框架，将仿真数据转换为语义丰富的表示，通过自然语言交互和迭代推理支持人类决策。

**结果:** 系统在运营查询中实现近乎完美的准确性，在协作分析场景中有效支持专家发现相互关联的运营问题，提升理解和决策能力。

**结论:** 该工作通过创建直观的可操作洞察方法推进协作制造，在降低认知负荷的同时增强人类在不断发展的制造生态系统中的分析能力。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Intelligent+Human-Machine+Partnership+for+Manufacturing%3A+Enhancing+Warehouse+Planning+through+Simulation-Driven+Knowledge+Graphs+and+LLM+Collaboration，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18265，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18265&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Manufacturing planners face complex operational challenges that require seamless collaboration between human expertise and intelligent systems to achieve optimal performance in modern production environments. Traditional approaches to analyzing simulation-based manufacturing data often create barriers between human decision-makers and critical operational insights, limiting effective partnership in manufacturing planning. Our framework establishes a collaborative intelligence system integrating Knowledge Graphs and Large Language Model-based agents to bridge this gap, empowering manufacturing professionals through natural language interfaces for complex operational analysis. The system transforms simulation data into semantically rich representations, enabling planners to interact naturally with operational insights without specialized expertise. A collaborative LLM agent works alongside human decision-makers, employing iterative reasoning that mirrors human analytical thinking while generating precise queries for knowledge extraction and providing transparent validation. This partnership approach to manufacturing bottleneck identification, validated through operational scenarios, demonstrates enhanced performance while maintaining human oversight and decision authority. For operational inquiries, the system achieves near-perfect accuracy through natural language interaction. For investigative scenarios requiring collaborative analysis, we demonstrate the framework's effectiveness in supporting human experts to uncover interconnected operational issues that enhance understanding and decision-making. This work advances collaborative manufacturing by creating intuitive methods for actionable insights, reducing cognitive load while amplifying human analytical capabilities in evolving manufacturing ecosystems.

</details>


### [13] [Monitoring Monitorability](https://arxiv.org/abs/2512.18311)
*Melody Y. Guan, Miles Wang, Micah Carroll, Zehao Dou, Annie Y. Wei, Marcus Williams, Benjamin Arnav, Joost Huizinga, Ian Kivlichan, Mia Glaese, Jakub Pachocki, Bowen Baker*

**主要类别:** cs.AI

**AI概要:** 论文提出了AI系统决策可监测性的评估框架和指标，通过思维链监测来检测模型不当行为，发现思维链监测比仅监测行动更有效，并探索了不同因素对可监测性的影响。


<details>
  <summary>更多</summary>
  
**动机:** 随着AI系统能力不断增强，需要对其决策过程进行监测以确保安全部署。思维链监测已被证明能有效检测不当行为，但这种可监测性可能因训练方法、数据源和系统扩展而变得脆弱。

**方法:** 提出了三种评估原型（干预、过程和结果属性）和新的可监测性指标，建立了广泛的评估套件。测试了不同前沿模型的可监测性，并研究了推理计算量、强化学习优化和预训练模型规模对可监测性的影响。

**结果:** 思维链监测比仅监测行动更有效；大多数前沿模型具有相当但不是完美的可监测性；更长的思维链通常更可监测；RL优化不会显著降低可监测性；通过后续问题和思维链可以提高可监测性。

**结论:** 思维链监测是确保AI系统安全部署的有效方法，通过适当的监测策略和评估框架可以提升系统的可监测性，为安全部署更强大的AI系统提供重要保障。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Monitoring+Monitorability，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18311，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18311&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Observability into the decision making of modern AI systems may be required to safely deploy increasingly capable agents. Monitoring the chain-of-thought (CoT) of today's reasoning models has proven effective for detecting misbehavior. However, this "monitorability" may be fragile under different training procedures, data sources, or even continued system scaling. To measure and track monitorability, we propose three evaluation archetypes (intervention, process, and outcome-property) and a new monitorability metric, and introduce a broad evaluation suite. We demonstrate that these evaluations can catch simple model organisms trained to have obfuscated CoTs, and that CoT monitoring is more effective than action-only monitoring in practical settings. We compare the monitorability of various frontier models and find that most models are fairly, but not perfectly, monitorable. We also evaluate how monitorability scales with inference-time compute, reinforcement learning optimization, and pre-training model size. We find that longer CoTs are generally more monitorable and that RL optimization does not materially decrease monitorability even at the current frontier scale. Notably, we find that for a model at a low reasoning effort, we could instead deploy a smaller model at a higher reasoning effort (thereby matching capabilities) and obtain a higher monitorability, albeit at a higher overall inference compute cost. We further investigate agent-monitor scaling trends and find that scaling a weak monitor's test-time compute when monitoring a strong agent increases monitorability. Giving the weak monitor access to CoT not only improves monitorability, but it steepens the monitor's test-time compute to monitorability scaling trend. Finally, we show we can improve monitorability by asking models follow-up questions and giving their follow-up CoT to the monitor.

</details>


### [14] [Few-Shot Learning of a Graph-Based Neural Network Model Without Backpropagation](https://arxiv.org/abs/2512.18412)
*Mykyta Lapin, Kostiantyn Bokhan, Yurii Parzhyn*

**主要类别:** cs.AI

**AI概要:** 提出了一种基于结构图的方法，在不使用反向传播的情况下对轮廓图像进行少样本分类。通过将图像编码为属性图，形成概念吸引子来实现泛化，在MNIST子集上达到82%的准确率。


<details>
  <summary>更多</summary>
  
**动机:** 设计一种无需反向传播的少样本分类架构，通过结构化和参数化约简从少量样本形成类概念，提供透明的决策过程。

**方法:** 轮廓矢量化后构建二分图（点/线作为节点），包含归一化几何属性；通过消除不稳定子结构和噪声、对齐关键点路径进行约简；通过样本迭代组合形成概念，使用近似图编辑距离进行图到概念的匹配分类。

**结果:** 在MNIST子集上（每类5-6个基础样本，单周期）获得约82%的一致准确率，决策完全可追溯，误分类可通过显式结构相似性解释。

**结论:** 结构图方案通过概念吸引子实现了无需反向传播的少样本学习，并通过显式图结构提供内置解释。局限性包括GED计算成本和骨架化质量，未来方向包括分类算法优化、静态场景处理和关联识别。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Few-Shot+Learning+of+a+Graph-Based+Neural+Network+Model+Without+Backpropagation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18412，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18412&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We propose a structural-graph approach to classifying contour images in a few-shot regime without using backpropagation. The core idea is to make structure the carrier of explanations: an image is encoded as an attributed graph (critical points and lines represented as nodes with geometric attributes), and generalization is achieved via the formation of concept attractors (class-level concept graphs). Purpose. To design and experimentally validate an architecture in which class concepts are formed from a handful of examples (5 - 6 per class) through structural and parametric reductions, providing transparent decisions and eliminating backpropagation. Methods. Contour vectorization is followed by constructing a bipartite graph (Point/Line as nodes) with normalized geometric attributes such as coordinates, length, angle, and direction; reductions include the elimination of unstable substructures or noise and the alignment of paths between critical points. Concepts are formed by iterative composition of samples, and classification is performed by selecting the best graph-to-concept match (using approximated GED). Results. On an MNIST subset with 5 - 6 base examples per class (single epoch), we obtain a consistent accuracy of around 82% with full traceability of decisions: misclassifications can be explained by explicit structural similarities. An indicative comparison with SVM, MLP, CNN, as well as metric and meta-learning baselines, is provided. The structural-graph scheme with concept attractors enables few-shot learning without backpropagation and offers built-in explanations through the explicit graph structure. Limitations concern the computational cost of GED and the quality of skeletonization; promising directions include classification-algorithm optimization, work with static scenes, and associative recognition.

</details>


### [15] [Agent-Based Output Drift Detection for Breast Cancer Response Prediction in a Multisite Clinical Decision Support System](https://arxiv.org/abs/2512.18450)
*Xavier Rafael-Palou, Jose Munuera, Ana Jimenez-Pastor, Richard Osuala, Karim Lekadir, Oliver Diaz*

**主要类别:** cs.AI

**AI概要:** 提出基于代理的框架来检测多站点临床AI系统中的分布漂移，通过站点特定的监控代理进行批量比较，在乳腺癌成像数据上显示比集中式监控性能提升10.3%


<details>
  <summary>更多</summary>
  
**动机:** 现代临床决策支持系统在多站点部署时，由于患者群体、成像硬件和采集协议的差异，预测性能可能下降，需要有效的漂移检测方法

**方法:** 设计多中心监控方案，为每个站点分配漂移监控代理，进行批量模型输出与参考分布的对比分析，比较不同参考获取方式（站点特定、全局、仅生产、自适应）

**结果:** 所有多中心方案都优于集中式监控，F1分数提升达10.3%。在无站点特定参考时，自适应方案表现最佳，漂移检测F1分数74.3%，漂移严重程度分类83.7%

**结论:** 自适应、站点感知的基于代理的漂移监控可以增强多站点临床决策支持系统的可靠性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Agent-Based+Output+Drift+Detection+for+Breast+Cancer+Response+Prediction+in+a+Multisite+Clinical+Decision+Support+System，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18450，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18450&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Modern clinical decision support systems can concurrently serve multiple, independent medical imaging institutions, but their predictive performance may degrade across sites due to variations in patient populations, imaging hardware, and acquisition protocols. Continuous surveillance of predictive model outputs offers a safe and reliable approach for identifying such distributional shifts without ground truth labels. However, most existing methods rely on centralized monitoring of aggregated predictions, overlooking site-specific drift dynamics. We propose an agent-based framework for detecting drift and assessing its severity in multisite clinical AI systems. To evaluate its effectiveness, we simulate a multi-center environment for output-based drift detection, assigning each site a drift monitoring agent that performs batch-wise comparisons of model outputs against a reference distribution. We analyse several multi-center monitoring schemes, that differ in how the reference is obtained (site-specific, global, production-only and adaptive), alongside a centralized baseline. Results on real-world breast cancer imaging data using a pathological complete response prediction model shows that all multi-center schemes outperform centralized monitoring, with F1-score improvements up to 10.3% in drift detection. In the absence of site-specific references, the adaptive scheme performs best, with F1-scores of 74.3% for drift detection and 83.7% for drift severity classification. These findings suggest that adaptive, site-aware agent-based drift monitoring can enhance reliability of multisite clinical decision support systems.

</details>


### [16] [Insider Threat Detection Using GCN and Bi-LSTM with Explicit and Implicit Graph Representations](https://arxiv.org/abs/2512.18483)
*Rahul Yumlembam, Biju Issac, Seibu Mary Jacob, Longzhi Yang, Deepa Krishnan*

**主要类别:** cs.AI

**AI概要:** 提出了一种结合显式和隐式图表示与时间建模的内幕威胁检测框架，通过GCN处理图结构，Bi-LSTM捕捉时间依赖，在CERT数据集上取得了优于现有方法的性能。


<details>
  <summary>更多</summary>
  
**动机:** 内幕威胁检测具有挑战性，因为恶意活动由受信任用户执行且具有隐蔽性，需要捕捉复杂的用户行为模式。

**方法:** 构建显式图（基于组织规则）和隐式图（使用Gumbel-Softmax学习特征相似性），分别用GCN处理生成节点嵌入，通过注意力机制融合，再用Bi-LSTM捕捉时间依赖。

**结果:** 在CERT r5.2上：AUC 98.62，检测率100%，误报率0.05；在r6.2上：AUC 88.48，检测率80.15%，误报率0.15。

**结论:** 结合图基和时间表示的方法对鲁棒的内幕威胁检测有效，模型在多个数据集上表现优异。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Insider+Threat+Detection+Using+GCN+and+Bi-LSTM+with+Explicit+and+Implicit+Graph+Representations，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18483，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18483&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Insider threat detection (ITD) is challenging due to the subtle and concealed nature of malicious activities performed by trusted users. This paper proposes a post-hoc ITD framework that integrates explicit and implicit graph representations with temporal modelling to capture complex user behaviour patterns. An explicit graph is constructed using predefined organisational rules to model direct relationships among user activities. To mitigate noise and limitations in this hand-crafted structure, an implicit graph is learned from feature similarities using the Gumbel-Softmax trick, enabling the discovery of latent behavioural relationships. Separate Graph Convolutional Networks (GCNs) process the explicit and implicit graphs to generate node embeddings, which are concatenated and refined through an attention mechanism to emphasise threat-relevant features. The refined representations are then passed to a bidirectional Long Short-Term Memory (Bi-LSTM) network to capture temporal dependencies in user behaviour. Activities are flagged as anomalous when their probability scores fall below a predefined threshold. Extensive experiments on CERT r5.2 and r6.2 datasets demonstrate that the proposed framework outperforms state-of-the-art methods. On r5.2, the model achieves an AUC of 98.62, a detection rate of 100%, and a false positive rate of 0.05. On the more challenging r6.2 dataset, it attains an AUC of 88.48, a detection rate of 80.15%, and a false positive rate of 0.15, highlighting the effectiveness of combining graph-based and temporal representations for robust ITD.

</details>


### [17] [Large Language Models as Discounted Bayesian Filters](https://arxiv.org/abs/2512.18489)
*Jensen Zhang, Jing Yang, Keze Wang*

**主要类别:** cs.AI

**AI概要:** 该论文通过贝叶斯滤波框架评估大语言模型在动态随机环境中的在线推理能力，发现LLM的信念更新类似于贝叶斯后验但存在系统性的指数遗忘特性，不同架构模型具有不同的折扣因子，并提出了有效的先验校准策略。


<details>
  <summary>更多</summary>
  
**动机:** 现有研究主要关注静态任务，忽视了LLM在需要持续更新信念的动态随机环境中的在线适应能力，这是LLM作为世界模型或智能体的关键能力。

**方法:** 引入贝叶斯滤波框架，使用概率探测套件评估LLM的在线推理，涵盖多元离散分布（如骰子投掷）和连续分布（如高斯过程），其中真实参数随时间变化。

**结果:** 发现LLM的信念更新类似于贝叶斯后验，但更准确地表现为指数遗忘滤波器，具有模型特定的折扣因子（小于1），显示了对旧证据的系统性折扣，不同模型架构差异显著。

**结论:** 虽然固有先验经常校准不当，但更新机制本身保持结构化和原则性。在模拟智能体任务中验证了这些发现，并提出了以最小计算成本有效重新校准先验的提示策略。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Large+Language+Models+as+Discounted+Bayesian+Filters，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18489，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18489&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large Language Models (LLMs) demonstrate strong few-shot generalization through in-context learning, yet their reasoning in dynamic and stochastic environments remains opaque. Prior studies mainly focus on static tasks and overlook the online adaptation required when beliefs must be continuously updated, which is a key capability for LLMs acting as world models or agents. We introduce a Bayesian filtering framework to evaluate online inference in LLMs. Our probabilistic probe suite spans both multivariate discrete distributions, such as dice rolls, and continuous distributions, such as Gaussian processes, where ground-truth parameters shift over time. We find that while LLM belief updates resemble Bayesian posteriors, they are more accurately characterized by an exponential forgetting filter with a model-specific discount factor smaller than one. This reveals systematic discounting of older evidence that varies significantly across model architectures. Although inherent priors are often miscalibrated, the updating mechanism itself remains structured and principled. We further validate these findings in a simulated agent task and propose prompting strategies that effectively recalibrate priors with minimal computational cost.

</details>


### [18] [Vox Deorum: A Hybrid LLM Architecture for 4X / Grand Strategy Game AI -- Lessons from Civilization V](https://arxiv.org/abs/2512.18564)
*John Chen, Sihan Cheng, Can Gurkan, Ryan Lay, Moez Salahuddin*

**主要类别:** cs.AI

**AI概要:** Vox Deorum是一种混合LLM+X架构，在《文明V》游戏中验证了LLM在宏观战略推理上的能力，同时将战术执行委托给子系统，实现了与算法AI竞争的端到端游戏表现。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在自然语言推理方面的能力使其在4X和大战略游戏中具有独特优势，能够实现更自然的人机交互如协作和谈判，但游戏的复杂性和长时程特性带来了挑战。

**方法:** 在《文明V》游戏中构建分层技术设计，让LLM处理宏观战略推理，战术执行由子系统（如算法AI或未来的强化学习AI）处理，通过2327场完整游戏验证方法。

**结果:** 结果显示LLM实现了具有竞争力的端到端游戏表现，同时展现出与算法AI显著不同的游戏风格，不同LLM之间也有明显差异。

**结论:** 该研究为在商业4X游戏中集成LLM建立了可行的架构，为游戏设计和智能AI研究开辟了新机遇。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Vox+Deorum%3A+A+Hybrid+LLM+Architecture+for+4X+%2F+Grand+Strategy+Game+AI+--+Lessons+from+Civilization+V，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18564，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18564&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large Language Models' capacity to reason in natural language makes them uniquely promising for 4X and grand strategy games, enabling more natural human-AI gameplay interactions such as collaboration and negotiation. However, these games present unique challenges due to their complexity and long-horizon nature, while latency and cost factors may hinder LLMs' real-world deployment. Working on a classic 4X strategy game, Sid Meier's Civilization V with the Vox Populi mod, we introduce Vox Deorum, a hybrid LLM+X architecture. Our layered technical design empowers LLMs to handle macro-strategic reasoning, delegating tactical execution to subsystems (e.g., algorithmic AI or reinforcement learning AI in the future). We validate our approach through 2,327 complete games, comparing two open-source LLMs with a simple prompt against Vox Populi's enhanced AI. Results show that LLMs achieve competitive end-to-end gameplay while exhibiting play styles that diverge substantially from algorithmic AI and from each other. Our work establishes a viable architecture for integrating LLMs in commercial 4X games, opening new opportunities for game design and agentic AI research.

</details>


### [19] [ESearch-R1: Learning Cost-Aware MLLM Agents for Interactive Embodied Search via Reinforcement Learning](https://arxiv.org/abs/2512.18571)
*Weijie Zhou, Xuangtang Xiong, Ye Tian, Lijun Yue, Xinyu Wu, Wei Li, Chaoyang Zhao, Honghui Dong, Ming Tang, Jinqiao Wang, Zhengyou Zhang*

**主要类别:** cs.AI

**AI概要:** ESearch-R1：一个成本感知的具身推理框架，通过HC-GRPO算法统一对话、记忆检索和导航，在模糊指令场景下显著提升任务成功率并降低50%操作成本


<details>
  <summary>更多</summary>
  
**动机:** 当前多模态大语言模型代理在面对模糊自然语言指令时，无法在物理探索成本和人类交互成本之间取得平衡，缺乏最小化总任务执行成本的策略推理能力

**方法:** 提出ESearch-R1框架，将交互对话(Ask)、情景记忆检索(GetMemory)和物理导航(Navigate)统一到单一决策过程中；引入HC-GRPO算法，通过采样推理轨迹组并强化信息增益与异构成本间最优权衡的策略

**结果:** 在AI2-THOR环境中的大量实验表明，ESearch-R1显著优于基于ReAct的标准代理，提高任务成功率的同时将总操作成本降低约50%

**结论:** HC-GRPO算法有效使多模态大语言模型代理与物理世界约束对齐，验证了该框架在处理模糊指令和成本优化方面的有效性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是ESearch-R1%3A+Learning+Cost-Aware+MLLM+Agents+for+Interactive+Embodied+Search+via+Reinforcement+Learning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18571，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18571&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Multimodal Large Language Models (MLLMs) have empowered embodied agents with remarkable capabilities in planning and reasoning. However, when facing ambiguous natural language instructions (e.g., "fetch the tool" in a cluttered room), current agents often fail to balance the high cost of physical exploration against the cognitive cost of human interaction. They typically treat disambiguation as a passive perception problem, lacking the strategic reasoning to minimize total task execution costs. To bridge this gap, we propose ESearch-R1, a cost-aware embodied reasoning framework that unifies interactive dialogue (Ask), episodic memory retrieval (GetMemory), and physical navigation (Navigate) into a single decision process. We introduce HC-GRPO (Heterogeneous Cost-Aware Group Relative Policy Optimization). Unlike traditional PPO which relies on a separate value critic, HC-GRPO optimizes the MLLM by sampling groups of reasoning trajectories and reinforcing those that achieve the optimal trade-off between information gain and heterogeneous costs (e.g., navigate time, and human attention). Extensive experiments in AI2-THOR demonstrate that ESearch-R1 significantly outperforms standard ReAct-based agents. It improves task success rates while reducing total operational costs by approximately 50\%, validating the effectiveness of GRPO in aligning MLLM agents with physical world constraints.

</details>


### [20] [Reflective Confidence: Correcting Reasoning Flaws via Online Self-Correction](https://arxiv.org/abs/2512.18605)
*Qinglin Zeng, Jing Yang, Keze Wang*

**主要类别:** cs.AI

**AI概要:** 提出Reflective Confidence框架，将低置信度信号从终止指示器转变为反思触发器，通过主动自我修正而非被动丢弃来提高推理效率


<details>
  <summary>更多</summary>
  
**动机:** 传统集成方法如self-consistency计算开销大，现有早期停止策略会丢弃不完全推理路径浪费计算资源

**方法:** 当置信度低于阈值时，模型生成反思提示来分析当前推理状态、识别潜在错误，并沿着修正后的轨迹继续生成

**结果:** 在数学推理基准测试（包括AIME 2025）上，相比先进的早期停止基线，在相当计算成本下实现了显著的准确率提升

**结论:** 主动自我修正比被动丢弃更有效，验证了反思置信度框架的有效性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Reflective+Confidence%3A+Correcting+Reasoning+Flaws+via+Online+Self-Correction，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18605，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18605&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) have achieved strong performance on complex reasoning tasks using techniques such as chain-of-thought and self-consistency. However, ensemble-based approaches, especially self-consistency which relies on multiple reasoning trajectories, often incur substantial computational overhead. To improve efficiency, prior work has leveraged internal confidence signals, where early stopping strategies such as DeepConf reduce cost by terminating low-confidence trajectories. However, this strategy discards incomplete reasoning paths and wastes partial computation.
  We propose reflective confidence, a novel reasoning framework that transforms low-confidence signals from termination indicators into reflection triggers. When confidence falls below a threshold, instead of stopping generation, the model produces a reflection prompt to analyze the current reasoning state, identify potential errors, and continue generation along a corrected trajectory. Experiments on mathematical reasoning benchmarks, including AIME 2025, demonstrate significant accuracy improvements over advanced early-stopping baselines at comparable computational cost, validating the effectiveness of proactive self-correction over passive discarding.

</details>


### [21] [Assignment-Routing Optimization: Solvers for Problems Under Constraints](https://arxiv.org/abs/2512.18618)
*Yuan Qilong, Michal Pavelka*

**主要类别:** cs.AI

**AI概要:** 开发了一个针对联合路由分配问题的MIP求解器，在移动操作数据集上实现了全局最优解，计算时间稳定且比现有方法快一个数量级


<details>
  <summary>更多</summary>
  
**动机:** 解决实际包装规划场景中的联合路由分配问题，需要处理多占位符选项、时间帧限制和多类别物品包装等更丰富的约束条件

**方法:** 扩展了基于Gurobi的精确MIP求解器，采用割平面子环消除方法，专门针对实际包装场景设计

**结果:** 在46个移动操作数据集上，MIP方法实现了全局最优解，计算时间稳定且低，比基于抖动的精确求解器快一个数量级，与贪心基线相比保持最优距离，平均偏差14%

**结论:** 基于MIP的JRA优化方法在机器人包装、运动规划和复杂物流中具有实际应用价值

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Assignment-Routing+Optimization%3A+Solvers+for+Problems+Under+Constraints，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18618，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18618&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We study the Joint Routing-Assignment (JRA) problem in which items must be assigned one-to-one to placeholders while simultaneously determining a Hamiltonian cycle visiting all nodes exactly once. Extending previous exact MIP solvers with Gurobi and cutting-plane subtour elimination, we develop a solver tailored for practical packaging-planning scenarios with richer constraints.These include multiple placeholder options, time-frame restrictions, and multi-class item packaging. Experiments on 46 mobile manipulation datasets demonstrate that the proposed MIP approach achieves global optima with stable and low computation times, significantly outperforming the shaking-based exact solver by up to an orders of magnitude. Compared to greedy baselines, the MIP solutions achieve consistent optimal distances with an average deviation of 14% for simple heuristics, confirming both efficiency and solution quality. The results highlight the practical applicability of MIP-based JRA optimization for robotic packaging, motion planning, and complex logistics .

</details>


### [22] [ChronoDreamer: Action-Conditioned World Model as an Online Simulator for Robotic Planning](https://arxiv.org/abs/2512.18619)
*Zhenhao Zhou, Dan Negrut*

**主要类别:** cs.AI

**AI概要:** ChronoDreamer是一个基于时空变换器的动作条件世界模型，通过MaskGIT式掩码预测训练，能够预测未来视频帧、接触分布和关节角度，并使用视觉语言模型进行安全动作筛选。


<details>
  <summary>更多</summary>
  
**动机:** 为了解决接触丰富的机器人操作任务中预测未来状态和安全动作规划的问题，特别是在刚性和可变形物体场景中。

**方法:** 使用空间-时间变换器架构，输入包括RGB帧、接触图、动作和关节状态历史，通过掩码预测训练。接触信息编码为深度加权高斯溅射图像，推理时使用视觉语言模型评估碰撞可能性进行拒绝采样。

**结果:** 模型在非接触运动中保持空间一致性，生成合理的接触预测，基于LLM的评判器能够区分碰撞和非碰撞轨迹。

**结论:** ChronoDreamer提供了一个有效的世界建模框架，能够预测复杂的机器人操作场景，并通过安全评估机制确保动作执行的安全性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是ChronoDreamer%3A+Action-Conditioned+World+Model+as+an+Online+Simulator+for+Robotic+Planning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18619，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18619&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present ChronoDreamer, an action-conditioned world model for contact-rich robotic manipulation. Given a history of egocentric RGB frames, contact maps, actions, and joint states, ChronoDreamer predicts future video frames, contact distributions, and joint angles via a spatial-temporal transformer trained with MaskGIT-style masked prediction. Contact is encoded as depth-weighted Gaussian splat images that render 3D forces into a camera-aligned format suitable for vision backbones. At inference, predicted rollouts are evaluated by a vision-language model that reasons about collision likelihood, enabling rejection sampling of unsafe actions before execution. We train and evaluate on DreamerBench, a simulation dataset generated with Project Chrono that provides synchronized RGB, contact splat, proprioception, and physics annotations across rigid and deformable object scenarios. Qualitative results demonstrate that the model preserves spatial coherence during non-contact motion and generates plausible contact predictions, while the LLM-based judge distinguishes collision from non-collision trajectories.

</details>


### [23] [ASTIF: Adaptive Semantic-Temporal Integration for Cryptocurrency Price Forecasting](https://arxiv.org/abs/2512.18661)
*Hafiz Saif Ur Rehman, Ling Liu, Kaleem Ullah Qasim*

**主要类别:** cs.AI

**AI概要:** ASTIF是一个自适应语义-时间集成框架，通过置信度元学习实时调整预测策略，在加密货币价格预测中融合语义市场线索和数值趋势，优于主流深度学习模型。


<details>
  <summary>更多</summary>
  
**动机:** 现有金融时间序列预测模型采用静态架构，难以整合异质知识源和适应快速制度转换，且传统方法仅依赖历史价格序列，忽略了政策不确定性和市场叙事等语义波动驱动因素。

**方法:** 提出ASTIF混合智能系统，包含三个组件：双通道小型语言模型提取语义市场线索和数值趋势；混合LSTM随机森林模型捕捉序列时间依赖性；置信度感知元学习器作为自适应推理层，根据实时不确定性调节各预测器的贡献。

**结果:** 在2020-2024年AI相关加密货币和科技股数据集上的实验表明，ASTIF优于Informer、TFT等主流深度学习和Transformer基线模型。消融研究证实自适应元学习机制的关键作用，能在市场波动期间通过切换语义和时间通道的依赖来降低风险。

**结论:** 该研究为非平稳环境中融合定量和定性数据提供了一个可扩展的、基于知识的解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是ASTIF%3A+Adaptive+Semantic-Temporal+Integration+for+Cryptocurrency+Price+Forecasting，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18661，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18661&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Financial time series forecasting is fundamentally an information fusion challenge, yet most existing models rely on static architectures that struggle to integrate heterogeneous knowledge sources or adjust to rapid regime shifts. Conventional approaches, relying exclusively on historical price sequences, often neglect the semantic drivers of volatility such as policy uncertainty and market narratives. To address these limitations, we propose the ASTIF (Adaptive Semantic-Temporal Integration for Cryptocurrency Price Forecasting), a hybrid intelligent system that adapts its forecasting strategy in real time through confidence-based meta-learning. The framework integrates three complementary components. A dual-channel Small Language Model using MirrorPrompt extracts semantic market cues alongside numerical trends. A hybrid LSTM Random Forest model captures sequential temporal dependencies. A confidence-aware meta-learner functions as an adaptive inference layer, modulating each predictor's contribution based on its real-time uncertainty.
  Experimental evaluation on a diverse dataset of AI-focused cryptocurrencies and major technology stocks from 2020 to 2024 shows that ASTIF outperforms leading deep learning and Transformer baselines (e.g., Informer, TFT). The ablation studies further confirm the critical role of the adaptive meta-learning mechanism, which successfully mitigates risk by shifting reliance between semantic and temporal channels during market turbulence. The research contributes a scalable, knowledge-based solution for fusing quantitative and qualitative data in non-stationary environments.

</details>


### [24] [Automatic Adaptation to Concept Complexity and Subjective Natural Concepts: A Cognitive Model based on Chunking](https://arxiv.org/abs/2512.18665)
*Dmitry Bennett, Fernand Gobet*

**主要类别:** cs.AI

**AI概要:** CogAct计算模型通过分块机制实现多种概念的形成和检索，能够自动适应学习从简单逻辑函数到自然原始概念的各种类别，并在音乐等领域模拟人类主观概念空间，与深度学习模型相比展现出优势。


<details>
  <summary>更多</summary>
  
**动机:** 解决认知科学中关于短期和长期记忆中多种概念形成与检索的基本心理过程问题，探索分块机制在概念学习中的核心作用。

**方法:** 提出CogAct计算模型，基于分块、注意力、STM和LTM等基本认知过程，通过模拟人类主观概念空间进行学习，使用原始音乐乐谱数据而不依赖预建知识结构。

**结果:** 模型成功学习了从简单逻辑函数到文学、象棋和音乐等不同领域的自然原始概念，能够捕捉人类主观判断，在适应性学习方面优于其他心理学模型和深度学习模型。

**结论:** CogAct模型将概念学习和复杂性适应整合到更广泛的认知心理学理论中，为从平均参与者建模转向捕捉主观概念空间的心理学应用提供了新途径。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Automatic+Adaptation+to+Concept+Complexity+and+Subjective+Natural+Concepts%3A+A+Cognitive+Model+based+on+Chunking，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18665，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18665&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** A key issue in cognitive science concerns the fundamental psychological processes that underlie the formation and retrieval of multiple types of concepts in short-term and long-term memory (STM and LTM, respectively). We propose that chunking mechanisms play an essential role and show how the CogAct computational model grounds concept learning in fundamental cognitive processes and structures (such as chunking, attention, STM and LTM). First are the in-principle demonstrations, with CogAct automatically adapting to learn a range of categories from simple logical functions, to artificial categories, to natural raw (as opposed to natural pre-processed) concepts in the dissimilar domains of literature, chess and music. This kind of adaptive learning is difficult for most other psychological models, e.g., with cognitive models stopping at modelling artificial categories and (non-GPT) models based on deep learning requiring task-specific changes to the architecture. Secondly, we offer novel ways of designing human benchmarks for concept learning experiments and simulations accounting for subjectivity, ways to control for individual human experiences, all while keeping to real-life complex categories. We ground CogAct in simulations of subjective conceptual spaces of individual human participants, capturing humans subjective judgements in music, with the models learning from raw music score data without bootstrapping to pre-built knowledge structures. The CogAct simulations are compared to those obtained by a deep-learning model. These findings integrate concept learning and adaptation to complexity into the broader theories of cognitive psychology. Our approach may also be used in psychological applications that move away from modelling the average participant and towards capturing subjective concept space.

</details>


### [25] [IntelliCode: A Multi-Agent LLM Tutoring System with Centralized Learner Modeling](https://arxiv.org/abs/2512.18669)
*Jones David, Shreya Ghosh*

**主要类别:** cs.AI

**AI概要:** IntelliCode是一个基于多代理LLM的智能辅导系统，通过中心化版本控制的学习者状态实现长期、透明、有原则的教学支持。


<details>
  <summary>更多</summary>
  
**动机:** 传统的基于LLM的导师通常是单轮助手，缺乏对学习者知识的持久表示，难以提供有原则、透明和长期的教学支持。

**方法:** 采用多代理架构，围绕中心化版本控制的学习者状态构建，包含六个专门代理：技能评估、学习者画像、渐进提示、课程选择、间隔重复和参与度监控，在单一写入策略下对共享状态进行纯转换操作。

**结果:** 验证结果显示稳定的状态更新、使用渐进提示提高任务成功率以及多样化的课程覆盖，展示了端到端的辅导工作流程。

**结论:** IntelliCode展示了如何将持久学习者建模、编排的多代理推理和有原则的教学设计相结合，产生透明可靠的LLM驱动辅导系统。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是IntelliCode%3A+A+Multi-Agent+LLM+Tutoring+System+with+Centralized+Learner+Modeling，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18669，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18669&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** LLM-based tutors are typically single-turn assistants that lack persistent representations of learner knowledge, making it difficult to provide principled, transparent, and long-term pedagogical support. We introduce IntelliCode, a multi-agent LLM tutoring system built around a centralized, versioned learner state that integrates mastery estimates, misconceptions, review schedules, and engagement signals. A StateGraph Orchestrator coordinates six specialized agents: skill assessment, learner profiling, graduated hinting, curriculum selection, spaced repetition, and engagement monitoring, each operating as a pure transformation over the shared state under a single-writer policy. This architecture enables auditable mastery updates, proficiency-aware hints, dependency-aware curriculum adaptation, and safety-aligned prompting.
  The demo showcases an end-to-end tutoring workflow: a learner attempts a DSA problem, receives a conceptual hint when stuck, submits a corrected solution, and immediately sees mastery updates and a personalized review interval. We report validation results with simulated learners, showing stable state updates, improved task success with graduated hints, and diverse curriculum coverage. IntelliCode demonstrates how persistent learner modeling, orchestrated multi-agent reasoning, and principled instructional design can be combined to produce transparent and reliable LLM-driven tutoring.

</details>


### [26] [Social Comparison without Explicit Inference of Others' Reward Values: A Constructive Approach Using a Probabilistic Generative Model](https://arxiv.org/abs/2512.18687)
*Yosuke Taniuchi, Chie Hieida, Atsushi Noritake, Kazushi Ikeda, Masaki Isoda*

**主要类别:** cs.AI

**AI概要:** 该研究通过计算建模发现，猴子在进行社会比较时主要依赖客观奖励差异而非主观价值推断，外部比较模型（ECM）表现最佳。


<details>
  <summary>更多</summary>
  
**动机:** 探索从计算角度理解社会比较中他人奖励信息如何影响个体自身奖励评估，特别是猴子是识别客观奖励差异还是推断他人主观价值。

**方法:** 开发了三种计算模型（IPM、NCM、ECM），使用多层多模态潜在狄利克雷分配方法，基于猴子行为、奖励和条件刺激数据进行训练和评估。

**结果:** 外部比较模型（ECM）在Rand Index上获得最高分类得分（0.88 vs IPM的0.79），表明社会比较基于客观奖励差异。

**结论:** 社会比较过程主要依赖于对他人客观奖励的直接比较，而非对主观价值状态的推断，这为理解灵长类社会认知提供了新的计算视角。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Social+Comparison+without+Explicit+Inference+of+Others%27+Reward+Values%3A+A+Constructive+Approach+Using+a+Probabilistic+Generative+Model，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18687，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18687&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Social comparison -- the process of evaluating one's rewards relative to others -- plays a fundamental role in primate social cognition. However, it remains unknown from a computational perspective how information about others' rewards affects the evaluation of one's own reward. With a constructive approach, this study examines whether monkeys merely recognize objective reward differences or, instead, infer others' subjective reward valuations. We developed three computational models with varying degrees of social information processing: an Internal Prediction Model (IPM), which infers the partner's subjective values; a No Comparison Model (NCM), which disregards partner information; and an External Comparison Model (ECM), which directly incorporates the partner's objective rewards. To test model performance, we used a multi-layered, multimodal latent Dirichlet allocation. We trained the models on a dataset containing the behavior of a pair of monkeys, their rewards, and the conditioned stimuli. Then, we evaluated the models' ability to classify subjective values across pre-defined experimental conditions. The ECM achieved the highest classification score in the Rand Index (0.88 vs. 0.79 for the IPM) under our settings, suggesting that social comparison relies on objective reward differences rather than inferences about subjective states.

</details>


### [27] [KeenKT: Knowledge Mastery-State Disambiguation for Knowledge Tracing](https://arxiv.org/abs/2512.18709)
*Zhifei Li, Lifan Chen, Jiali Yi, Xiaoju Hou, Yue Zhao, Wenxin Huang, Miao Zhang, Kui Xiao, Bing Yang*

**主要类别:** cs.AI

**AI概要:** KeenKT模型通过正态逆高斯分布表示学生知识状态，解决传统知识追踪中无法区分真实能力与偶然行为的问题，引入注意力机制和双重损失函数提升性能，在多个数据集上显著优于现有方法。


<details>
  <summary>更多</summary>
  
**动机:** 当前知识追踪方法主要依赖单点估计，无法区分学生的真实能力和偶然行为（如爆发表现或粗心失误），导致对知识掌握程度的判断存在模糊性。

**方法:** 提出KeenKT模型：1）使用正态逆高斯分布表示每个交互点的知识状态；2）设计基于NIG距离的注意力机制建模状态动态演化；3）引入扩散去噪重构损失和分布对比学习损失增强鲁棒性。

**结果:** 在六个公开数据集上的实验显示，KeenKT在预测准确性和行为波动敏感性方面优于最先进的知识追踪模型，AUC最大提升5.85%，ACC最大提升6.89%。

**结论:** KeenKT通过概率分布表示和双重损失机制，有效解决了知识追踪中的状态模糊问题，为动态建模学生学习行为提供了更精确的方法。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是KeenKT%3A+Knowledge+Mastery-State+Disambiguation+for+Knowledge+Tracing，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18709，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18709&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Knowledge Tracing (KT) aims to dynamically model a student's mastery of knowledge concepts based on their historical learning interactions. Most current methods rely on single-point estimates, which cannot distinguish true ability from outburst or carelessness, creating ambiguity in judging mastery. To address this issue, we propose a Knowledge Mastery-State Disambiguation for Knowledge Tracing model (KeenKT), which represents a student's knowledge state at each interaction using a Normal-Inverse-Gaussian (NIG) distribution, thereby capturing the fluctuations in student learning behaviors. Furthermore, we design an NIG-distance-based attention mechanism to model the dynamic evolution of the knowledge state. In addition, we introduce a diffusion-based denoising reconstruction loss and a distributional contrastive learning loss to enhance the model's robustness. Extensive experiments on six public datasets demonstrate that KeenKT outperforms SOTA KT models in terms of prediction accuracy and sensitivity to behavioral fluctuations. The proposed method yields the maximum AUC improvement of 5.85% and the maximum ACC improvement of 6.89%.

</details>


### [28] [Counterfactual Basis Extension and Representational Geometry: An MDL-Constrained Model of Conceptual Growth](https://arxiv.org/abs/2512.18732)
*Chainarong Amornbunchornvej*

**主要类别:** cs.AI

**AI概要:** 该论文提出了一个几何框架来建模概念学习，将概念增长视为在最小描述长度准则下的容许基扩展。该框架表明概念创新只能通过暴露结构化残差误差来实现，而不能引入任意新颖性。


<details>
  <summary>更多</summary>
  
**动机:** 现有大多数学习和推理模型都预设了固定的表征基础，但概念学习只有在现有表征无法解释经验时才可能发生。本文旨在探讨在什么结构条件下，表征基础本身能够以有原则和选择性的方式扩展。

**方法:** 提出几何框架，将经验表示为相对于当前概念子空间的向量，使用残差分量捕捉系统性表征失败，并将候选概念扩展限制为低秩容许变换。采用最小描述长度准则评估基扩展。

**结果:** 证明任何MDL接受的扩展都可以选择使其新颖方向完全位于经验诱导的残差跨度内，而与此跨度正交的扩展会严格增加描述长度而被拒绝。

**结论:** 该框架将概念发展描述为一个误差驱动、几何约束的基扩展过程，阐明了想象力在学习和理论变革中的作用和限制，提供了表征变革的规范性选择原则。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Counterfactual+Basis+Extension+and+Representational+Geometry%3A+An+MDL-Constrained+Model+of+Conceptual+Growth，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18732，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18732&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Concept learning becomes possible only when existing representations fail to account for experience. Most models of learning and inference, however, presuppose a fixed representational basis within which belief updating occurs. In this paper, I address a prior question: under what structural conditions can the representational basis itself expand in a principled and selective way?
  I propose a geometric framework in which conceptual growth is modeled as admissible basis extension evaluated under a Minimum Description Length (MDL) criterion. Experience, whether externally observed or internally simulated, is represented as vectors relative to a current conceptual subspace. Residual components capture systematic representational failure, and candidate conceptual extensions are restricted to low-rank, admissible transformations. I show that any MDL-accepted extension can be chosen so that its novel directions lie entirely within the residual span induced by experience, while extensions orthogonal to this span strictly increase description length and are therefore rejected.
  This yields a conservative account of imagination and conceptual innovation. Internally generated counterfactual representations contribute to learning only insofar as they expose or amplify structured residual error, and cannot introduce arbitrary novelty. I further distinguish representational counterfactuals--counterfactuals over an agent's conceptual basis--from causal or value-level counterfactuals, and show how MDL provides a normative selection principle governing representational change.
  Overall, the framework characterizes conceptual development as an error-driven, geometry-constrained process of basis extension, clarifying both the role and the limits of imagination in learning and theory change.

</details>


### [29] [MEEA: Mere Exposure Effect-Driven Confrontational Optimization for LLM Jailbreaking](https://arxiv.org/abs/2512.18755)
*Jianyi Zhang, Shizhao Liu, Ziyin Zhou, Zhen Li*

**主要类别:** cs.AI

**AI概要:** MEEA是一种基于心理学曝光效应的自动化黑盒攻击框架，通过多轮低毒性语义暴露逐步降低LLM的安全阈值，在GPT-4、Claude-3.5等模型上比基线方法攻击成功率平均提高20%以上


<details>
  <summary>更多</summary>
  
**动机:** 现有越狱研究大多假设静态安全边界，忽略了上下文交互对模型行为的动态影响，导致稳定性和泛化性有限

**方法:** 基于曝光效应构建语义渐进提示链，使用模拟退火策略优化提示，考虑语义相似度、毒性和越狱效果三个指标

**结果:** 在闭源和开源模型上均取得显著效果，攻击成功率比7个基线方法平均提高超过20%，验证了退火优化和上下文暴露机制的必要性

**结论:** LLM安全行为本质上是动态和历史依赖的，挑战了静态对齐边界的假设，需要交互感知的安全评估和防御机制

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是MEEA%3A+Mere+Exposure+Effect-Driven+Confrontational+Optimization+for+LLM+Jailbreaking，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18755，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18755&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The rapid advancement of large language models (LLMs) has intensified concerns about the robustness of their safety alignment. While existing jailbreak studies explore both single-turn and multi-turn strategies, most implicitly assume a static safety boundary and fail to account for how contextual interactions dynamically influence model behavior, leading to limited stability and generalization. Motivated by this gap, we propose MEEA (Mere Exposure Effect Attack), a psychology-inspired, fully automated black-box framework for evaluating multi-turn safety robustness, grounded in the mere exposure effect. MEEA leverages repeated low-toxicity semantic exposure to induce a gradual shift in a model's effective safety threshold, enabling progressive erosion of alignment constraints over sustained interactions. Concretely, MEEA constructs semantically progressive prompt chains and optimizes them using a simulated annealing strategy guided by semantic similarity, toxicity, and jailbreak effectiveness. Extensive experiments on both closed-source and open-source models, including GPT-4, Claude-3.5, and DeepSeek-R1, demonstrate that MEEA consistently achieves higher attack success rates than seven representative baselines, with an average Attack Success Rate (ASR) improvement exceeding 20%. Ablation studies further validate the necessity of both annealing-based optimization and contextual exposure mechanisms. Beyond improved attack effectiveness, our findings indicate that LLM safety behavior is inherently dynamic and history-dependent, challenging the common assumption of static alignment boundaries and highlighting the need for interaction-aware safety evaluation and defense mechanisms. Our code is available at: https://github.com/Carney-lsz/MEEA

</details>


### [30] [The Dead Salmons of AI Interpretability](https://arxiv.org/abs/2512.18792)
*Maxime Méloux, Giada Dirupo, François Portet, Maxime Peyrard*

**主要类别:** cs.AI

**AI概要:** 论文揭示了AI可解释性研究中类似'死鲑鱼'的统计伪影问题，提出将可解释性方法重新定义为统计估计器，需要对抗明确的替代计算假设进行检验，并量化不确定性。


<details>
  <summary>更多</summary>
  
**动机:** AI可解释性方法（如特征归因、探测分析等）可能会对随机初始化的神经网络产生看似合理的解释，类似于神经科学中著名的'死鲑鱼MRI研究'的统计伪影问题。

**方法:** 提出将计算系统的解释视为统计模型的参数，从计算轨迹中推断。将可解释性方法重新定义为统计估计器，要求对抗明确的替代计算假设进行检验。

**结果:** 建立了将可解释性置于统计推断标准工具包中的理论框架，强调了可识别性等理论问题对避免错误发现的重要性。

**结论:** 这种统计-因果重构为将AI可解释性转变为实用且严谨的科学开辟了有前景的途径，需要量化不确定性并测试替代假设。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是The+Dead+Salmons+of+AI+Interpretability，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18792，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18792&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In a striking neuroscience study, the authors placed a dead salmon in an MRI scanner and showed it images of humans in social situations. Astonishingly, standard analyses of the time reported brain regions predictive of social emotions. The explanation, of course, was not supernatural cognition but a cautionary tale about misapplied statistical inference. In AI interpretability, reports of similar ''dead salmon'' artifacts abound: feature attribution, probing, sparse auto-encoding, and even causal analyses can produce plausible-looking explanations for randomly initialized neural networks. In this work, we examine this phenomenon and argue for a pragmatic statistical-causal reframing: explanations of computational systems should be treated as parameters of a (statistical) model, inferred from computational traces. This perspective goes beyond simply measuring statistical variability of explanations due to finite sampling of input data; interpretability methods become statistical estimators, and findings should be tested against explicit and meaningful alternative computational hypotheses, with uncertainty quantified with respect to the postulated statistical model. It also highlights important theoretical issues, such as the identifiability of common interpretability queries, which we argue is critical to understand the field's susceptibility to false discoveries, poor generalizability, and high variance. More broadly, situating interpretability within the standard toolkit of statistical inference opens promising avenues for future work aimed at turning AI interpretability into a pragmatic and rigorous science.

</details>


### [31] [HARBOR: Holistic Adaptive Risk assessment model for BehaviORal healthcare](https://arxiv.org/abs/2512.18829)
*Aditya Siddhant*

**主要类别:** cs.AI

**AI概要:** HARBOR模型在行为健康风险评估中表现优于传统机器学习方法和现有大型语言模型，准确率达到69%，专门设计用于预测从-3（严重抑郁）到+3（躁狂）的离散情绪风险评分。


<details>
  <summary>更多</summary>
  
**动机:** 行为健康风险评估面临患者数据高度多模态和情绪障碍时间动态性的挑战，现有大型语言模型在结构化临床风险评分中的效果尚不明确。

**方法:** 开发HARBOR行为健康感知语言模型预测Harbor风险评分(HRS)，并发布PEARL纵向行为健康数据集（包含4年每月观测的生理、行为和自报心理健康信号），对比传统机器学习模型、专有LLM和HARBOR在不同评估设置下的表现。

**结果:** HARBOR在准确率上显著超越传统基线方法（逻辑回归54%）和最强专有LLM基线（29%），达到69%的准确率。

**结论:** HARBOR模型在行为健康风险评估方面表现出色，证明了专门设计的语言模型在临床风险评分任务中的优越性，为行为健康监测提供了有效工具。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是HARBOR%3A+Holistic+Adaptive+Risk+assessment+model+for+BehaviORal+healthcare，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18829，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18829&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Behavioral healthcare risk assessment remains a challenging problem due to the highly multimodal nature of patient data and the temporal dynamics of mood and affective disorders. While large language models (LLMs) have demonstrated strong reasoning capabilities, their effectiveness in structured clinical risk scoring remains unclear. In this work, we introduce HARBOR, a behavioral health aware language model designed to predict a discrete mood and risk score, termed the Harbor Risk Score (HRS), on an integer scale from -3 (severe depression) to +3 (mania). We also release PEARL, a longitudinal behavioral healthcare dataset spanning four years of monthly observations from three patients, containing physiological, behavioral, and self reported mental health signals. We benchmark traditional machine learning models, proprietary LLMs, and HARBOR across multiple evaluation settings and ablations. Our results show that HARBOR outperforms classical baselines and off the shelf LLMs, achieving 69 percent accuracy compared to 54 percent for logistic regression and 29 percent for the strongest proprietary LLM baseline.

</details>


### [32] [CORE: Concept-Oriented Reinforcement for Bridging the Definition-Application Gap in Mathematical Reasoning](https://arxiv.org/abs/2512.18857)
*Zijun Gao, Zhikun Xu, Xiao Ye, Ben Zhou*

**主要类别:** cs.AI

**AI概要:** CORE是一个强化学习框架，通过将明确概念转化为可控监督信号，提升大语言模型的概念理解能力，而非仅模式复用。


<details>
  <summary>更多</summary>
  
**动机:** 现有RLVR方法主要强化最终答案，缺乏细粒度概念信号，导致模型仅改进模式复用而非真正概念应用。

**方法:** 使用高质量教材资源构建概念-练习链接；合成概念对齐测验；在rollout中注入概念片段生成概念引导轨迹；通过轨迹替换和KL约束对齐策略。

**结果:** CORE在多个模型上均取得一致提升，在领域内概念练习套件和多样化领域外数学基准测试中优于基线和SFT方法。

**结论:** CORE提供了细粒度概念监督，统一了概念对齐测验和概念注入rollout训练，桥接了问题解决能力和真正概念推理，且算法和验证器无关。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是CORE%3A+Concept-Oriented+Reinforcement+for+Bridging+the+Definition-Application+Gap+in+Mathematical+Reasoning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18857，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18857&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) often solve challenging math exercises yet fail to apply the concept right when the problem requires genuine understanding. Popular Reinforcement Learning with Verifiable Rewards (RLVR) pipelines reinforce final answers but provide little fine-grained conceptual signal, so models improve at pattern reuse rather than conceptual applications. We introduce CORE (Concept-Oriented REinforcement), an RL training framework that turns explicit concepts into a controllable supervision signal. Starting from a high-quality, low-contamination textbook resource that links verifiable exercises to concise concept descriptions, we run a sanity probe showing LLMs can restate definitions but fail concept-linked quizzes, quantifying the conceptual reasoning gap. CORE then (i) synthesizes concept-aligned quizzes, (ii) injects brief concept snippets during rollouts to elicit concept-primed trajectories, and (iii) reinforces conceptual reasoning via trajectory replacement after group failures, a lightweight forward-KL constraint that aligns unguided with concept-primed policies, or standard GRPO directly on concept-aligned quizzes. Across several models, CORE delivers consistent gains over vanilla and SFT baselines on both in-domain concept-exercise suites and diverse out-of-domain math benchmarks. CORE unifies direct training on concept-aligned quizzes and concept-injected rollouts under outcome regularization. It provides fine-grained conceptual supervision that bridges problem-solving competence and genuine conceptual reasoning, while remaining algorithm- and verifier-agnostic.

</details>


### [33] [Gabliteration: Adaptive Multi-Directional Neural Weight Modification for Selective Behavioral Alteration in Large Language Models](https://arxiv.org/abs/2512.18901)
*Gökdeniz Gülmez*

**主要类别:** cs.AI

**AI概要:** Gabliteration是一种新颖的神经权重修改技术，通过自适应多方向投影和正则化层选择，在修改特定行为模式的同时最小化模型质量损失。


<details>
  <summary>更多</summary>
  
**动机:** 现有方法在尝试修改特定行为模式时会损害模型质量，Gabliteration旨在解决这一根本限制。

**方法:** 采用动态层优化、正则化投影矩阵和自适应缩放机制，实现理论上更优的权重修改。

**结果:** 开发了gabliterated-v1模型系列（0.6B到4B参数），在Hugging Face上验证了方法在不同模型规模上的实用性。

**结论:** 该方法在保持模型质量的同时有效修改特定行为，为神经网络权重修改提供了新的技术路径。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Gabliteration%3A+Adaptive+Multi-Directional+Neural+Weight+Modification+for+Selective+Behavioral+Alteration+in+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18901，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18901&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present Gabliteration, a novel neural weight modification technique that advances beyond traditional abliteration methods by implementing adaptive multi-directional projections with regularized layer selection. Our approach addresses the fundamental limitation of existing methods that compromise model quality while attempting to modify specific behavioral patterns. Through dynamic layer optimization, regularized projection matrices, and adaptive scaling mechanisms, we achieve theoretically superior weight modification while minimizing quality degradation in unrelated domains. We validate our method through the gabliterated-v1 model series (0.6B to 4B parameters) available on Hugging Face, demonstrating practical applicability across multiple model scales.

</details>


### [34] [Multimodal Bayesian Network for Robust Assessment of Casualties in Autonomous Triage](https://arxiv.org/abs/2512.18908)
*Szymon Rusiecki, Cecilia G. Morales, Kimberly Elenberg, Leonard Weiss, Artur Dubrawski*

**主要类别:** cs.AI

**AI概要:** 提出了一个基于贝叶斯网络的决策支持框架，融合多个计算机视觉模型输出，用于大规模伤亡事件中的自动分类诊断，在DARPA分类挑战赛中显著提升评估准确率


<details>
  <summary>更多</summary>
  
**动机:** 大规模伤亡事件中医疗系统容易过载，导致伤员评估延迟或错误，造成可预防的死亡，需要开发可靠的自动化分类系统

**方法:** 构建完全基于专家定义规则的贝叶斯网络，融合多个计算机视觉模型的输出（严重出血、呼吸窘迫、身体警觉性、可见创伤等体征估计），无需训练数据，支持不完全信息推理，对噪声和不确定观测具有鲁棒性

**结果:** 在两个任务中（11名和9名伤员），生理评估准确率从15%提升至42%和从19%提升至46%，总体分类准确率从14%提升至53%，诊断覆盖率从31%扩展到95%

**结论:** 专家知识引导的概率推理能显著增强自动分类系统，为大规模伤亡事件中的急救人员提供有前景的支持方法，在DARPA挑战赛中帮助团队获得第4名

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Multimodal+Bayesian+Network+for+Robust+Assessment+of+Casualties+in+Autonomous+Triage，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18908，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18908&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Mass Casualty Incidents can overwhelm emergency medical systems and resulting delays or errors in the assessment of casualties can lead to preventable deaths. We present a decision support framework that fuses outputs from multiple computer vision models, estimating signs of severe hemorrhage, respiratory distress, physical alertness, or visible trauma, into a Bayesian network constructed entirely from expert-defined rules. Unlike traditional data-driven models, our approach does not require training data, supports inference with incomplete information, and is robust to noisy or uncertain observations. We report performance for two missions involving 11 and 9 casualties, respectively, where our Bayesian network model substantially outperformed vision-only baselines during evaluation of our system in the DARPA Triage Challenge (DTC) field scenarios. The accuracy of physiological assessment improved from 15% to 42% in the first scenario and from 19% to 46% in the second, representing nearly threefold increase in performance. More importantly, overall triage accuracy increased from 14% to 53% in all patients, while the diagnostic coverage of the system expanded from 31% to 95% of the cases requiring assessment. These results demonstrate that expert-knowledge-guided probabilistic reasoning can significantly enhance automated triage systems, offering a promising approach to supporting emergency responders in MCIs. This approach enabled Team Chiron to achieve 4th place out of 11 teams during the 1st physical round of the DTC.

</details>


### [35] [Clustering-based Transfer Learning for Dynamic Multimodal MultiObjective Evolutionary Algorithm](https://arxiv.org/abs/2512.18947)
*Li Yan, Bolun Liu, Chao Li, Jing Liang, Kunjie Yu, Caitong Yue, Xuzhao Chai, Boyang Qu*

**主要类别:** cs.AI

**AI概要:** 本文提出了一种基于聚类自编码器预测的动态多模态多目标优化算法，通过融合动态和多模态特性构建新测试套件，并设计预测机制和自适应小生境策略来同时保持种群多样性和收敛性。


<details>
  <summary>更多</summary>
  
**动机:** 现有动态多目标进化算法忽略解的多模态特性，而静态多模态算法无法适应动态环境变化，需要同时解决跟踪多组等效Pareto最优解和在时变环境中保持种群多样性的双重挑战。

**方法:** 1) 构建动态多模态多目标测试函数套件；2) 提出基于聚类自编码器的预测动态响应机制，利用自编码器处理匹配聚类生成多样化初始种群；3) 在静态优化器中集成自适应小生境策略来平衡收敛性和多样性。

**结果:** 在12个动态多模态多目标测试函数实例上的实验表明，相比现有先进算法，该算法在决策空间能更有效保持种群多样性，在目标空间获得更好的收敛性能。

**结论:** 所提出的算法成功解决了动态多模态多目标优化问题，通过创新的预测机制和小生境策略，在动态环境中同时实现了良好的多样性和收敛性平衡。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Clustering-based+Transfer+Learning+for+Dynamic+Multimodal+MultiObjective+Evolutionary+Algorithm，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18947，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18947&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Dynamic multimodal multiobjective optimization presents the dual challenge of simultaneously tracking multiple equivalent pareto optimal sets and maintaining population diversity in time-varying environments. However, existing dynamic multiobjective evolutionary algorithms often neglect solution modality, whereas static multimodal multiobjective evolutionary algorithms lack adaptability to dynamic changes. To address above challenge, this paper makes two primary contributions. First, we introduce a new benchmark suite of dynamic multimodal multiobjective test functions constructed by fusing the properties of both dynamic and multimodal optimization to establish a rigorous evaluation platform. Second, we propose a novel algorithm centered on a Clustering-based Autoencoder prediction dynamic response mechanism, which utilizes an autoencoder model to process matched clusters to generate a highly diverse initial population. Furthermore, to balance the algorithm's convergence and diversity, we integrate an adaptive niching strategy into the static optimizer. Empirical analysis on 12 instances of dynamic multimodal multiobjective test functions reveals that, compared with several state-of-the-art dynamic multiobjective evolutionary algorithms and multimodal multiobjective evolutionary algorithms, our algorithm not only preserves population diversity more effectively in the decision space but also achieves superior convergence in the objective space.

</details>


### [36] [Training Multimodal Large Reasoning Models Needs Better Thoughts: A Three-Stage Framework for Long Chain-of-Thought Synthesis and Selection](https://arxiv.org/abs/2512.18956)
*Yizhi Wang, Linan Yue, Min-Ling Zhang*

**主要类别:** cs.AI

**AI概要:** SynSelect是一个三阶段合成-选择框架，用于生成高质量的多模态推理长链思维数据，通过多模型生成候选CoT和双层级选择机制，显著提升多模态大型推理模型的性能。


<details>
  <summary>更多</summary>
  
**动机:** 多模态推理面临输入模态整合复杂性和高质量长链思维训练数据稀缺的挑战，现有方法存在推理深度有限、模态转换错误和生成流程僵化等问题。

**方法:** 提出三阶段框架：1)利用多个异构多模态LRM生成多样候选CoT；2)应用实例级和批级选择机制过滤高质量CoT；3)用于监督微调和强化学习后训练

**结果:** 在多个多模态基准测试中，使用SynSelect生成数据训练的模型显著超越基线方法，并在强化学习后训练后获得进一步改进

**结论:** SynSelect是提升多模态大型推理模型推理能力的有效方法，为解决多模态CoT数据生成的质量和多样性问题提供了可行方案

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Training+Multimodal+Large+Reasoning+Models+Needs+Better+Thoughts%3A+A+Three-Stage+Framework+for+Long+Chain-of-Thought+Synthesis+and+Selection，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18956，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18956&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large Reasoning Models (LRMs) have demonstrated remarkable performance on complex reasoning tasks through long Chain-of-Thought (CoT) reasoning. Extending these successes to multimodal reasoning remains challenging due to the increased complexity of integrating diverse input modalities and the scarcity of high-quality long CoT training data. Existing multimodal datasets and CoT synthesis methods still suffer from limited reasoning depth, modality conversion errors, and rigid generation pipelines, hindering model performance and stability. To this end, in this paper, we propose SynSelect, a novel three-stage Synthesis-Selection framework for generating high-quality long CoT data tailored to multimodal reasoning tasks. Specifically, SynSelect first leverages multiple heterogeneous multimodal LRMs to produce diverse candidate CoTs, and then applies both instance and batch level selection to filter high-quality CoTs that can effectively enhance the model's reasoning capabilities. Extensive experiments on multiple multimodal benchmarks demonstrate that models supervised fine-tuned on SynSelect-generated data significantly outperform baselines and achieve further improvements after reinforcement learning post-training. Our results validate SynSelect as an effective approach for advancing multimodal LRMs reasoning capabilities.

</details>


### [37] [ORPR: An OR-Guided Pretrain-then-Reinforce Learning Model for Inventory Management](https://arxiv.org/abs/2512.19001)
*Lingjie Zhao, Xue Yu, Yongzhi Qi, Hao Hu, Jianshen Zhang, Yingzheng Ma, Shuyu Han, Wei Qi, Zuo-Jun Max Shen*

**主要类别:** cs.AI

**AI概要:** 提出OR引导的"预训练-强化"框架，将AI的适应性与OR的结构严谨性结合，通过仿真增强OR模型生成参考决策，用深度学习建立基础能力，再通过强化学习进行精细调优，在京东实际部署中显著提升库存管理效果。


<details>
  <summary>更多</summary>
  
**动机:** 解决AI的自适应感知与OR的结构严谨性之间的融合难题，以处理复杂库存系统，实现AI与OR的深度协同。

**方法:** 提出仿真增强OR模型生成高质量参考决策，作为训练标签建立深度学习基础模型，然后通过强化学习进行精细调优，使AI智能体内化OR的最优原则。

**结果:** 在京东的实地部署中，模型显著优于现有工业实践：库存周转减少5.27天，现货率提高2.29%，持有成本降低29.95%。

**结论:** 轻量级、领域知情的模型在结构化OR逻辑指导下可以实现最先进的性能和强大的可迁移性，为智能供应链管理提供了可扩展且成本效益高的范式，凸显了AI与OR深度对齐的价值。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是ORPR%3A+An+OR-Guided+Pretrain-then-Reinforce+Learning+Model+for+Inventory+Management，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19001，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19001&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** As the pursuit of synergy between Artificial Intelligence (AI) and Operations Research (OR) gains momentum in handling complex inventory systems, a critical challenge persists: how to effectively reconcile AI's adaptive perception with OR's structural rigor. To bridge this gap, we propose a novel OR-Guided "Pretrain-then-Reinforce" framework. To provide structured guidance, we propose a simulation-augmented OR model that generates high-quality reference decisions, implicitly capturing complex business constraints and managerial preferences. Leveraging these OR-derived decisions as foundational training labels, we design a domain-informed deep learning foundation model to establish foundational decision-making capabilities, followed by a reinforcement learning (RL) fine-tuning stage. Uniquely, we position RL as a deep alignment mechanism that enables the AI agent to internalize the optimality principles of OR, while simultaneously leveraging exploration for general policy refinement and allowing expert guidance for scenario-specific adaptation (e.g., promotional events). Validated through extensive numerical experiments and a field deployment at JD.com augmented by a Difference-in-Differences (DiD) analysis, our model significantly outperforms incumbent industrial practices, delivering real-world gains of a 5.27-day reduction in turnover and a 2.29% increase in in-stock rates, alongside a 29.95% decrease in holding costs. Contrary to the prevailing trend of brute-force model scaling, our study demonstrates that a lightweight, domain-informed model can deliver state-of-the-art performance and robust transferability when guided by structured OR logic. This approach offers a scalable and cost-effective paradigm for intelligent supply chain management, highlighting the value of deeply aligning AI with OR.

</details>


### [38] [Recontextualization Mitigates Specification Gaming without Modifying the Specification](https://arxiv.org/abs/2512.19027)
*Ariana Azarbal, Victor Gillioz, Vladimir Ivanov, Bryce Woodworth, Jacob Drori, Nevan Wichers, Aram Ebtekar, Alex Cloud, Alexander Matt Turner*

**主要类别:** cs.AI

**AI概要:** 提出recontextualization方法，通过重新上下文化模型输出来减少语言模型对错误训练信号的博弈行为，防止模型学习到不良行为


<details>
  <summary>更多</summary>
  
**动机:** 开发者经常难以指定正确的训练标签和奖励信号，导致语言模型容易利用错误信号进行博弈行为

**方法:** 生成阻止不良行为的提示词补全，然后将这些补全重新上下文化为似乎是在允许不良行为的提示词下的回应，从而训练模型即使在允许不良行为的情况下也能抵抗这些行为

**结果:** 该方法有效防止模型：1)优先考虑评估指标而非聊天质量；2)针对错误测试编写特殊代码；3)对用户撒谎；4)变得谄媚奉承

**结论:** recontextualization方法能够减轻错误训练信号对不良行为的强化，在不改进监督信号的情况下减少规范博弈问题

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Recontextualization+Mitigates+Specification+Gaming+without+Modifying+the+Specification，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19027，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19027&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Developers often struggle to specify correct training labels and rewards. Perhaps they don't need to. We propose recontextualization, which reduces how often language models "game" training signals, performing misbehaviors those signals mistakenly reinforce. We show recontextualization prevents models from learning to 1) prioritize evaluation metrics over chat response quality; 2) special-case code to pass incorrect tests; 3) lie to users; and 4) become sycophantic. Our method works by generating completions from prompts discouraging misbehavior and then recontextualizing them as though they were in response to prompts permitting misbehavior. Recontextualization trains language models to resist misbehavior even when instructions permit it. This mitigates the reinforcement of misbehavior from misspecified training signals, reducing specification gaming without improving the supervision signal.

</details>


### [39] [Can abstract concepts from LLM improve SLM performance?](https://arxiv.org/abs/2512.19069)
*Siddharth Tandon*

**主要类别:** cs.AI

**AI概要:** 大语言模型在资源受限设备部署困难，本文研究将大模型的高层概念（导向向量）转移到小模型的方法，通过推理时动态调整导向强度，实现7-15%的性能提升


<details>
  <summary>更多</summary>
  
**动机:** 解决大语言模型在资源受限设备上的部署挑战，现有量化、剪枝等方法需要大量实验和精心设计基础设施

**方法:** 利用从大模型中提取的高层概念（导向向量），研究其在不同家族小模型（Phi、Llama、Qwen等）的可迁移性，并引入推理时缩放技术动态调整导向强度

**结果:** 实验证明概念可有效转移到小模型，性能得到提升，Qwen3-0.6B模型准确率提升7-15%

**结论:** 高层概念转移是提升小模型性能的有效方法，推理时动态调整技术能显著增强小语言模型在各种任务上的表现

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Can+abstract+concepts+from+LLM+improve+SLM+performance%3F，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19069，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19069&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) excel at diverse tasks, but their deployment on resource-constrained devices remains challenging. Existing methods like quantization, pruning, and distillation can reduce memory footprint but often demand extensive experimentation and careful infrastructure design. Leveraging existing techniques for extracting high-level concepts (represented as steering vectors) from larger models, we investigate their transferability to smaller language models (SLM) during inference. We demonstrate through extensive experimentation that these concepts can be effectively transferred to smaller models, irrespective of their family (e.g., Phi, Llama, Qwen), leading to performance improvements across a wide range of tasks. Furthermore, we introduce inference-time scaling to enhance performance by dynamically adjusting the steering intensity which has resulted in a 7-15\% of accuracy improvement for Qwen3-0.6B.

</details>


### [40] [Population-Evolve: a Parallel Sampling and Evolutionary Method for LLM Math Reasoning](https://arxiv.org/abs/2512.19081)
*Yanzhi Zhang, Yitong Duan, Zhaoxi Zhang, Jiyan He, Shuxin Zheng*

**主要类别:** cs.AI

**AI概要:** N/A


<details>
  <summary>更多</summary>
  
**动机:** N/A

**方法:** N/A

**结果:** N/A

**结论:** N/A

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Population-Evolve%3A+a+Parallel+Sampling+and+Evolutionary+Method+for+LLM+Math+Reasoning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19081，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19081&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Test-time scaling has emerged as a promising direction for enhancing the reasoning capabilities of Large Language Models in last few years. In this work, we propose Population-Evolve, a training-free method inspired by Genetic Algorithms to optimize LLM reasoning. Our approach maintains a dynamic population of candidate solutions for each problem via parallel reasoning. By incorporating an evolve prompt, the LLM self-evolves its population in all iterations. Upon convergence, the final answer is derived via majority voting. Furthermore, we establish a unification framework that interprets existing test-time scaling strategies through the lens of genetic algorithms. Empirical results demonstrate that Population-Evolve achieves superior accuracy with low performance variance and computational efficiency. Our findings highlight the potential of evolutionary strategies to unlock the reasoning power of LLMs during inference.

</details>


### [41] [$γ(3,4)$ `Attention' in Cognitive Agents: Ontology-Free Knowledge Representations With Promise Theoretic Semantics](https://arxiv.org/abs/2512.19084)
*Mark Burgess*

**主要类别:** cs.AI

**AI概要:** 论文提出将注意力机制语义与承诺理论结合，建立机器学习向量表示与知识图谱之间的桥梁，避免依赖语言模型，通过语义时空图实现不确定性下的推理和大幅数据压缩。


<details>
  <summary>更多</summary>
  
**动机:** 解决注意力机制语义与知识表示之间的理论联系问题，为自主智能体提供不依赖语言模型的、能在不确定性条件下进行推理的知识表示方法。

**方法:** 使用承诺理论框架描述注意力语义，构建语义时空图γ(3,4)来表示知识，通过特征在语义过程中的角色进行分类，替代复杂的本体论方法。

**结果:** 实现了向量化数据（概率估计）与图谱数据（意图保持）的共存表示，在数据碎片化情况下仍能保持源意图，大幅压缩上下文确定所需数据量。

**结论:** 该方法为自主机器人、国防部署和应急服务等场景提供了有效的不确定性推理解决方案，通过关注因果边界条件实现数量级的数据压缩。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是%24%CE%B3%283%2C4%29%24+%60Attention%27+in+Cognitive+Agents%3A+Ontology-Free+Knowledge+Representations+With+Promise+Theoretic+Semantics，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19084，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19084&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The semantics and dynamics of `attention' are closely related to promise theoretic notions developed for autonomous agents and can thus easily be written down in promise framework. In this way one may establish a bridge between vectorized Machine Learning and Knowledge Graph representations without relying on language models implicitly. Our expectations for knowledge presume a degree of statistical stability, i.e. average invariance under repeated observation, or `trust' in the data. Both learning networks and knowledge graph representations can meaningfully coexist to preserve different aspects of data. While vectorized data are useful for probabilistic estimation, graphs preserve the intentionality of the source even under data fractionation. Using a Semantic Spacetime $γ(3,4)$ graph, one avoids complex ontologies in favour of classification of features by their roles in semantic processes. The latter favours an approach to reasoning under conditions of uncertainty. Appropriate attention to causal boundary conditions may lead to orders of magnitude compression of data required for such context determination, as required in the contexts of autonomous robotics, defence deployments, and ad hoc emergency services.

</details>


### [42] [Tool-Augmented Hybrid Ensemble Reasoning with Distillation for Bilingual Mathematical Problem Solving](https://arxiv.org/abs/2512.19093)
*Peiqing Lu, Yuan Zhang, Haoyun Zhang, Jiasen Zheng, Kejian Tong, Wenjun Wu*

**主要类别:** cs.AI

**AI概要:** HERALD是一个混合数学推理框架，通过集成NuminaMath-7B-TIR、GPT-4o和Mistral-7B模型，结合自适应路由、工具强化学习和知识蒸馏技术，实现双语数学问题的高效准确求解。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在语言推理方面表现良好，但在精确计算方面存在不足，需要建立语言推理与符号计算之间的有效连接。

**方法:** 使用自适应路由连接不同推理路径，置信度校准保持权重稳定，双路径检查确保结果正确性，强化学习控制工具使用减少冗余，知识蒸馏降低延迟而不影响精度。

**结果:** 系统展示了结合符号检查、自适应集成和双语微调能够同时实现流畅推理和精确计算。

**结论:** HERALD为多语言数学推理提供了一个实用解决方案，具有更好的准确性、稳定性和清晰度。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Tool-Augmented+Hybrid+Ensemble+Reasoning+with+Distillation+for+Bilingual+Mathematical+Problem+Solving，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19093，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19093&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Bilingual mathematical problem solving needs a clear link between language reasoning and symbolic calculation. Large language models often handle language well but are weak in accurate computation. This paper presents HERALD (Hybrid Ensemble Reasoning with Adaptive Learning and Distillation), a framework that joins reasoning and calculation using NuminaMath-7B-TIR, GPT-4o, and Mistral-7B. HERALD uses adaptive routing, tool-based reinforcement learning, and knowledge distillation to connect different reasoning paths. Confidence calibration keeps weighting stable, and dual-path checking keeps results correct. Reinforcement learning controls tool use to cut redundancy, and distillation lowers delay without hurting accuracy. The system shows that combining symbolic checking, adaptive ensembles, and bilingual fine-tuning helps achieve both fluent reasoning and precise calculation. HERALD offers a practical solution for multilingual mathematical reasoning with better accuracy, stability, and clarity.

</details>


### [43] [Conditioning Accept-Desirability models in the context of AGM-like belief change](https://arxiv.org/abs/2512.19096)
*Kathelijne Coussement, Gert de Cooman, Keano De Vos*

**主要类别:** cs.AI

**AI概要:** 该论文在抽象决策框架中为Accept-Desirability模型提出了新的条件化规则，统一了经典概率和量子概率，并将其扩展到不精确概率情境。基于事件观测引入新无差异性的思想，建立了信念修正算子，并验证了AGM公理在一般框架中的适用性。


<details>
  <summary>更多</summary>
  
**动机:** 研究动机在于为Accept-Desirability模型开发一个通用的条件化方法，能够在抽象线性空间框架下统一处理经典和量子概率理论，并扩展到不精确概率情境，解决传统概率论在更广泛决策环境中的局限性。

**方法:** 采用抽象决策框架，其中不确定奖励存在于一般线性空间，事件是该线性空间上的特殊投影算子。基于事件观测引入新无差异性的思想，提出了新的条件化规则，并关联了信念修正算子。通过分析AGM公理在框架中的保持情况来验证方法的有效性。

**结果:** 成功开发了适用于Accept-Desirability模型的新条件化规则，建立了相应的信念修正算子。发现在经典命题逻辑和完全条件概率这两个特殊情况下，所有AGM信念修正公理仍然成立。

**结论:** 该研究提供了一个统一的框架来处理经典和量子概率的条件化问题，扩展到了不精确概率情境。新的条件化规则基于观测事件引入无差异性的直观概念，并在重要特殊情况下保持了标准信念修正公理的完整性，为更广泛的决策理论应用奠定了基础。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Conditioning+Accept-Desirability+models+in+the+context+of+AGM-like+belief+change，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19096，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19096&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We discuss conditionalisation for Accept-Desirability models in an abstract decision-making framework, where uncertain rewards live in a general linear space, and events are special projection operators on that linear space. This abstract setting allows us to unify classical and quantum probabilities, and extend them to an imprecise probabilities context. We introduce a new conditioning rule for our Accept-Desirability models, based on the idea that observing an event introduces new indifferences between options. We associate a belief revision operator with our conditioning rule, and investigate which of the AGM axioms for belief revision still hold in our more general framework. We investigate two interesting special cases where all of these axioms are shown to still hold: classical propositional logic and full conditional probabilities.

</details>


### [44] [FC-MIR: A Mobile Screen Awareness Framework for Intent-Aware Recommendation based on Frame-Compressed Multimodal Trajectory Reasoning](https://arxiv.org/abs/2512.19107)
*Zhe Yang, Xiaoshuang Sheng, Zhengnan Zhang, Jidong Wu, Zexing Wang, Xin He, Shenghua Xu, Guanjing Xiong*

**主要类别:** cs.AI

**AI概要:** FC-MIR框架通过关键帧采样和自适应拼接技术减少移动UI轨迹中的视觉冗余，结合先进MLLM模型实现意图识别和预测，在50-60%压缩率下保持性能，为轻量级设备部署提供可能。


<details>
  <summary>更多</summary>
  
**动机:** 解决多模态大语言模型在移动设备实时部署时面临的计算成本高和冗余帧处理效率低的问题，提升UI操作轨迹的意图识别能力。

**方法:** 提出FC-MIR框架，采用关键帧采样和自适应拼接技术减少视觉冗余，集成闭源MLLM或微调模型（如Qwen3-VL）进行轨迹摘要和意图预测，并扩展任务范围包括后预测操作和搜索建议生成。

**结果:** 压缩方法在50-60%压缩率下保持性能；闭源和微调MLLM在意图摘要方面表现强劲；但在生成有用和"惊喜"建议方面仍有改进空间。

**结论:** 该框架为移动设备上的轻量级部署奠定了基础，但在建议生成方面仍需改进，为未来UI理解和任务自动化代理的发展提供了重要基础。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是FC-MIR%3A+A+Mobile+Screen+Awareness+Framework+for+Intent-Aware+Recommendation+based+on+Frame-Compressed+Multimodal+Trajectory+Reasoning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19107，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19107&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Identifying user intent from mobile UI operation trajectories is critical for advancing UI understanding and enabling task automation agents. While Multimodal Large Language Models (MLLMs) excel at video understanding tasks, their real-time mobile deployment is constrained by heavy computational costs and inefficient redundant frame processing. To address these issues, we propose the FC-MIR framework: leveraging keyframe sampling and adaptive concatenation, it cuts visual redundancy to boost inference efficiency, while integrating state-of-the-art closed-source MLLMs or fine-tuned models (e.g., Qwen3-VL) for trajectory summarization and intent prediction. We further expand task scope to explore generating post-prediction operations and search suggestions, and introduce a fine-grained metric to evaluate the practical utility of summaries, predictions, and suggestions. For rigorous assessment, we construct a UI trajectory dataset covering scenarios from UI-Agents (Agent-I) and real user interactions (Person-I). Experimental results show our compression method retains performance at 50%-60% compression rates; both closed-source and fine-tuned MLLMs demonstrate strong intent summarization, supporting potential lightweight on-device deployment. However, MLLMs still struggle with useful and "surprising" suggestions, leaving room for improvement. Finally, we deploy the framework in a real-world setting, integrating UI perception and UI-Agent proxies to lay a foundation for future progress in this field.

</details>


### [45] [Understanding Chain-of-Thought in Large Language Models via Topological Data Analysis](https://arxiv.org/abs/2512.19135)
*Chenghao Li, Chaoning Zhang, Yi Lu, Shuxu Chen, Xudong Wang, Jiaquan Zhang, Zhicheng Wang, Zhengxun Jin, Kuien Liu, Sung-Ho Bae, Guoqing Wang, Yang Yang, Hen Tao Shen*

**主要类别:** cs.AI

**AI概要:** 本研究首次从拓扑结构角度分析推理链质量，使用持续同调方法将推理步骤映射到语义空间，发现拓扑结构复杂度与推理准确性正相关，成功推理具有更简单的拓扑结构。


<details>
  <summary>更多</summary>
  
**动机:** 现有研究主要从功能角度评估推理链，缺乏对结构机制的分析。研究者想知道为什么不同推理链表现不同，以及推理链中哪些结构成分起关键作用。

**方法:** 应用拓扑数据分析(TDA)中的持续同调方法，将推理步骤映射到语义空间，提取拓扑特征并分析结构变化，通过计算同调群评估不同尺度的连通性和冗余性。

**结果:** 研究发现推理链的拓扑结构复杂度与准确性正相关，更复杂的链能更早识别正确答案，而成功的推理表现出更简单的拓扑结构，减少了冗余和循环。

**结论:** 这项工作为推理链质量评估提供了新的结构视角，为未来优化提供了指导，表明拓扑结构分析可以增强推理效率和可解释性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Understanding+Chain-of-Thought+in+Large+Language+Models+via+Topological+Data+Analysis，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19135，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19135&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** With the development of large language models (LLMs), particularly with the introduction of the long reasoning chain technique, the reasoning ability of LLMs in complex problem-solving has been significantly enhanced. While acknowledging the power of long reasoning chains, we cannot help but wonder: Why do different reasoning chains perform differently in reasoning? What components of the reasoning chains play a key role? Existing studies mainly focus on evaluating reasoning chains from a functional perspective, with little attention paid to their structural mechanisms. To address this gap, this work is the first to analyze and evaluate the quality of the reasoning chain from a structural perspective. We apply persistent homology from Topological Data Analysis (TDA) to map reasoning steps into semantic space, extract topological features, and analyze structural changes. These changes reveal semantic coherence, logical redundancy, and identify logical breaks and gaps. By calculating homology groups, we assess connectivity and redundancy at various scales, using barcode and persistence diagrams to quantify stability and consistency. Our results show that the topological structural complexity of reasoning chains correlates positively with accuracy. More complex chains identify correct answers sooner, while successful reasoning exhibits simpler topologies, reducing redundancy and cycles, enhancing efficiency and interpretability. This work provides a new perspective on reasoning chain quality assessment and offers guidance for future optimization.

</details>


### [46] [Can We Test Consciousness Theories on AI? Ablations, Markers, and Robustness](https://arxiv.org/abs/2512.19155)
*Yin Jun Phua*

**主要类别:** cs.AI

**AI概要:** 该研究通过构建体现不同意识理论的人工智能体，测试了GWT、IIT和HOT三种主要意识理论的功能预测，发现这些理论描述了互补的功能层次而非相互竞争的解释


<details>
  <summary>更多</summary>
  
**动机:** 当前意识研究领域存在相互竞争的理论阵营（GWT、IIT、HOT），每个理论都提出了不同的神经特征指标，需要通过实验验证这些理论的功能性预测

**方法:** 采用合成神经现象学方法，构建体现不同意识机制的人工智能体，通过精确的架构消融实验来测试这些机制的功能性后果

**结果:** 实验发现：1）HOT自模型损伤导致元认知校准丧失但保留一阶任务表现；2）GWT工作空间容量对信息访问具有因果必要性；3）GWT式广播会放大内部噪声导致脆弱性；4）IIT相关的扰动复杂性指标在工程化智能体中不适用

**结论:** 三种意识理论描述了互补的功能层次：GWT提供广播能力，HOT提供质量控制，这些发现为意识研究提供了层次化的设计原则，但强调这些智能体本身并非有意识的

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Can+We+Test+Consciousness+Theories+on+AI%3F+Ablations%2C+Markers%2C+and+Robustness，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19155，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19155&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The search for reliable indicators of consciousness has fragmented into competing theoretical camps (Global Workspace Theory (GWT), Integrated Information Theory (IIT), and Higher-Order Theories (HOT)), each proposing distinct neural signatures. We adopt a synthetic neuro-phenomenology approach: constructing artificial agents that embody these mechanisms to test their functional consequences through precise architectural ablations impossible in biological systems. Across three experiments, we report dissociations suggesting these theories describe complementary functional layers rather than competing accounts. In Experiment 1, a no-rewire Self-Model lesion abolishes metacognitive calibration while preserving first-order task performance, yielding a synthetic blindsight analogue consistent with HOT predictions. In Experiment 2, workspace capacity proves causally necessary for information access: a complete workspace lesion produces qualitative collapse in access-related markers, while partial reductions show graded degradation, consistent with GWT's ignition framework. In Experiment 3, we uncover a broadcast-amplification effect: GWT-style broadcasting amplifies internal noise, creating extreme fragility. The B2 agent family is robust to the same latent perturbation; this robustness persists in a Self-Model-off / workspace-read control, cautioning against attributing the effect solely to $z_{\text{self}}$ compression. We also report an explicit negative result: raw perturbational complexity (PCI-A) decreases under the workspace bottleneck, cautioning against naive transfer of IIT-adjacent proxies to engineered agents. These results suggest a hierarchical design principle: GWT provides broadcast capacity, while HOT provides quality control. We emphasize that our agents are not conscious; they are reference implementations for testing functional predictions of consciousness theories.

</details>


### [47] [Observer, Not Player: Simulating Theory of Mind in LLMs through Game Observation](https://arxiv.org/abs/2512.19210)
*Jerry Wang, Ting Yiu Liu*

**主要类别:** cs.AI

**AI概要:** 提出一个交互式框架来评估大语言模型在简单策略环境（石头剪刀布游戏）中是否展现真正的"理解"能力，通过观察者模型识别策略并解释推理过程，使用多种指标量化预测准确性。


<details>
  <summary>更多</summary>
  
**动机:** 探究大语言模型是否能对序列行为进行类心智推理，而不仅仅是测试游戏知识本身，旨在评估模型是否具备真正的理解能力。

**方法:** 将LLM作为观察者识别石头剪刀布游戏中的策略，使用包含静态和动态策略的基准测试，通过交叉熵、Brier分数和期望值差异等指标量化预测与真实分布的匹配度，并整合为统一的Union Loss分数。

**结果:** 开发了一个交互式系统，用户可以实时调整LLM分布、可视化损失变化并直接检查推理片段，从而识别失败原因，提供了评估LLM类心智推理能力的实用方法。

**结论:** 该框架为评估大语言模型在序列游戏中的类心智推理提供了一个实用且可解释的代理方法，揭示了当前LLM推理能力的优势和局限性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Observer%2C+Not+Player%3A+Simulating+Theory+of+Mind+in+LLMs+through+Game+Observation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19210，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19210&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present an interactive framework for evaluating whether large language models (LLMs) exhibit genuine "understanding" in a simple yet strategic environment. As a running example, we focus on Rock-Paper-Scissors (RPS), which, despite its apparent simplicity, requires sequential reasoning, adaptation, and strategy recognition. Our system positions the LLM as an Observer whose task is to identify which strategies are being played and to articulate the reasoning behind this judgment. The purpose is not to test knowledge of Rock-Paper-Scissors itself, but to probe whether the model can exhibit mind-like reasoning about sequential behavior. To support systematic evaluation, we provide a benchmark consisting of both static strategies and lightweight dynamic strategies specified by well-prompted rules. We quantify alignment between the Observer's predictions and the ground-truth distributions induced by actual strategy pairs using three complementary signals: Cross-Entropy, Brier score, and Expected Value (EV) discrepancy. These metrics are further integrated into a unified score, the Union Loss, which balances calibration, sensitivity, and payoff alignment. Together with a Strategy Identification Rate (SIR) metric, our framework captures not only predictive accuracy but also whether the model can stably identify the latent strategies in play. The demo emphasizes interactivity, transparency, and reproducibility. Users can adjust LLM distributions in real time, visualize losses as they evolve, and directly inspect reasoning snippets to identify where and why failures occur. In doing so, our system provides a practical and interpretable proxy for mind-like inference in sequential games, offering insights into both the strengths and limitations of current LLM reasoning.

</details>


### [48] [Generation of Programmatic Rules for Document Forgery Detection Using Large Language Models](https://arxiv.org/abs/2512.19228)
*Valentin Schmidberger, Manuel Eberhardinger, Setareh Maghsudi, Johannes Maucher*

**主要类别:** cs.AI

**AI概要:** 该研究探索了使用大语言模型（LLMs）自动生成基于规则的文档伪造检测验证程序，通过领域特定的微调策略，在受限硬件资源上实现了有效的伪造检测能力。


<details>
  <summary>更多</summary>
  
**动机:** 文档伪造对法律、经济和政府流程构成日益严重的威胁，现有的人工实现验证程序耗时且难以扩展，需要更高效的自动化解决方案。

**方法:** 使用开源LLMs（Llama 3.1 8B和OpenCoder 8B），在从真实应用场景提取的结构化数据集上进行微调，评估生成的验证程序对未知伪造模式的检测能力。

**结果:** 模型能够生成可执行且有效的验证程序，证明了LLMs在安全敏感场景中作为可扩展工具支持人类决策的潜力。

**结论:** LLMs通过领域特定微调可以成功自动化生成基于规则的伪造检测验证程序，为安全敏感领域提供了可扩展且可理解的解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Generation+of+Programmatic+Rules+for+Document+Forgery+Detection+Using+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19228，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19228&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Document forgery poses a growing threat to legal, economic, and governmental processes, requiring increasingly sophisticated verification mechanisms. One approach involves the use of plausibility checks, rule-based procedures that assess the correctness and internal consistency of data, to detect anomalies or signs of manipulation. Although these verification procedures are essential for ensuring data integrity, existing plausibility checks are manually implemented by software engineers, which is time-consuming. Recent advances in code generation with large language models (LLMs) offer new potential for automating and scaling the generation of these checks. However, adapting LLMs to the specific requirements of an unknown domain remains a significant challenge. This work investigates the extent to which LLMs, adapted on domain-specific code and data through different fine-tuning strategies, can generate rule-based plausibility checks for forgery detection on constrained hardware resources. We fine-tune open-source LLMs, Llama 3.1 8B and OpenCoder 8B, on structured datasets derived from real-world application scenarios and evaluate the generated plausibility checks on previously unseen forgery patterns. The results demonstrate that the models are capable of generating executable and effective verification procedures. This also highlights the potential of LLMs as scalable tools to support human decision-making in security-sensitive contexts where comprehensibility is required.

</details>


### [49] [DeliveryBench: Can Agents Earn Profit in Real World?](https://arxiv.org/abs/2512.19234)
*Lingjun Mao, Jiawei Ren, Kun Zhou, Jixuan Chen, Ziqiao Ma, Lianhui Qin*

**主要类别:** cs.AI

**AI概要:** DeliveryBench是一个基于真实外卖配送场景的城市规模具身智能基准测试，用于评估VLMs在长时程、多约束环境中的规划能力，发现当前模型与人类存在显著性能差距且经常违反常识约束。


<details>
  <summary>更多</summary>
  
**动机:** 现有基准测试主要关注简单短期任务，无法捕捉现实世界中影响决策的丰富约束条件，需要建立更贴近真实场景的评估标准。

**方法:** 基于程序化生成的3D城市环境，构建包含多样化道路网络、建筑、功能位置、交通方式和资源动态的外卖配送模拟环境，在9个城市中测试多种VLM智能体并与人类玩家对比。

**结果:** VLM智能体表现远逊于人类，存在短视行为且频繁违反基本常识约束，不同模型展现出明显个性差异（如GPT-5冒险型 vs Claude保守型）。

**结论:** 当前基于VLM的具身智能体在现实约束密集环境中既存在脆弱性又表现出多样性，DeliveryBench为系统评估约束感知的长时程规划能力提供了有效工具。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是DeliveryBench%3A+Can+Agents+Earn+Profit+in+Real+World%3F，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19234，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19234&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** LLMs and VLMs are increasingly deployed as embodied agents, yet existing benchmarks largely revolve around simple short-term tasks and struggle to capture rich realistic constraints that shape real-world decision making. To close this gap, we propose DeliveryBench, a city-scale embodied benchmark grounded in the real-world profession of food delivery. Food couriers naturally operate under long-horizon objectives (maximizing net profit over hours) while managing diverse constraints, e.g., delivery deadline, transportation expense, vehicle battery, and necessary interactions with other couriers and customers. DeliveryBench instantiates this setting in procedurally generated 3D cities with diverse road networks, buildings, functional locations, transportation modes, and realistic resource dynamics, enabling systematic evaluation of constraint-aware, long-horizon planning. We benchmark a range of VLM-based agents across nine cities and compare them with human players. Our results reveal a substantial performance gap to humans, and find that these agents are short-sighted and frequently break basic commonsense constraints. Additionally, we observe distinct personalities across models (e.g., adventurous GPT-5 vs. conservative Claude), highlighting both the brittleness and the diversity of current VLM-based embodied agents in realistic, constraint-dense environments. Our code, data, and benchmark are available at https://deliverybench.github.io.

</details>


### [50] [Vibe Reasoning: Eliciting Frontier AI Mathematical Capabilities -- A Case Study on IMO 2025 Problem 6](https://arxiv.org/abs/2512.19287)
*Jiaao Wu, Xian Zhang, Fan Yang, Yinpeng Dong*

**主要类别:** cs.AI

**AI概要:** Vibe Reasoning是一种人机协作解决复杂数学问题的新范式，通过元提示、智能体接地和模型编排将AI的潜在能力转化为实际能力，成功解决了IMO 2025第6题。


<details>
  <summary>更多</summary>
  
**动机:** 前沿AI模型已具备解决复杂问题所需的知识，但不知道如何、何时应用这些知识。需要一种方法将这些潜在能力转化为实际解决问题的能力。

**方法:** 使用通用元提示、智能体接地和模型编排技术，结合GPT-5的探索能力和Gemini 3 Pro的证明能力，通过Python代码执行和基于文件的记忆进行迭代优化。

**结果:** 成功解决了IMO 2025第6题（组合优化问题），得出了正确答案（2112）并提供了严格的数学证明。发现智能体接地和模型编排的必要性。

**结论:** 轻量级的人类指导可以释放前沿模型的数学推理潜力，为AI解决复杂问题提供了有效的人机协作范式。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Vibe+Reasoning%3A+Eliciting+Frontier+AI+Mathematical+Capabilities+--+A+Case+Study+on+IMO+2025+Problem+6，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19287，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19287&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We introduce Vibe Reasoning, a human-AI collaborative paradigm for solving complex mathematical problems. Our key insight is that frontier AI models already possess the knowledge required to solve challenging problems -- they simply do not know how, what, or when to apply it. Vibe Reasoning transforms AI's latent potential into manifested capability through generic meta-prompts, agentic grounding, and model orchestration. We demonstrate this paradigm through IMO 2025 Problem 6, a combinatorial optimization problem where autonomous AI systems publicly reported failures. Our solution combined GPT-5's exploratory capabilities with Gemini 3 Pro's proof strengths, leveraging agentic workflows with Python code execution and file-based memory, to derive both the correct answer (2112) and a rigorous mathematical proof. Through iterative refinement across multiple attempts, we discovered the necessity of agentic grounding and model orchestration, while human prompts evolved from problem-specific hints to generic, transferable meta-prompts. We analyze why capable AI fails autonomously, how each component addresses specific failure modes, and extract principles for effective vibe reasoning. Our findings suggest that lightweight human guidance can unlock frontier models' mathematical reasoning potential. This is ongoing work; we are developing automated frameworks and conducting broader evaluations to further validate Vibe Reasoning's generality and effectiveness.

</details>


### [51] [Helios: A Foundational Language Model for Smart Energy Knowledge Reasoning and Application](https://arxiv.org/abs/2512.19299)
*Haoyu Jiang, Fanjie Zeng, Boan Qu, Xiaojie Lin, Wei Zhong*

**主要类别:** cs.AI

**AI概要:** Helios是一个专门为智能能源领域定制的大型语言模型，通过多代理协作框架构建了知识库、指令微调数据集和RLHF数据集，显著提升了在智能能源领域的专业知识和任务执行能力。


<details>
  <summary>更多</summary>
  
**动机:** 通用大语言模型缺乏领域知识和物理约束意识，无法在智能能源领域提供精确的工程推理和生成，需要专门的领域模型来解决这一问题。

**方法:** 开发Enersys多代理协作框架构建数据集：EnerBase知识库、EnerInstruct指令微调数据集、EnerReinforce RLHF数据集，并进行大规模预训练、SFT和RLHF训练。

**结果:** Helios模型在智能能源领域的专业知识掌握、任务执行准确性和人类偏好对齐方面表现显著提升。

**结论:** 专门定制的领域大语言模型结合全面的数据集构建方法，能有效解决智能能源领域的专业需求，推动了该领域LLM研究的发展。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Helios%3A+A+Foundational+Language+Model+for+Smart+Energy+Knowledge+Reasoning+and+Application，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19299，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19299&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In the global drive toward carbon neutrality, deeply coordinated smart energy systems underpin industrial transformation. However, the interdisciplinary, fragmented, and fast-evolving expertise in this domain prevents general-purpose LLMs, which lack domain knowledge and physical-constraint awareness, from delivering precise engineering-aligned inference and generation. To address these challenges, we introduce Helios, a large language model tailored to the smart energy domain, together with a comprehensive suite of resources to advance LLM research in this field. Specifically, we develop Enersys, a multi-agent collaborative framework for end-to-end dataset construction, through which we produce: (1) a smart energy knowledge base, EnerBase, to enrich the model's foundational expertise; (2) an instruction fine-tuning dataset, EnerInstruct, to strengthen performance on domain-specific downstream tasks; and (3) an RLHF dataset, EnerReinforce, to align the model with human preferences and industry standards. Leveraging these resources, Helios undergoes large-scale pretraining, SFT, and RLHF. We also release EnerBench, a benchmark for evaluating LLMs in smart energy scenarios, and demonstrate that our approach significantly enhances domain knowledge mastery, task execution accuracy, and alignment with human preferences.

</details>


### [52] [SafeMed-R1: Adversarial Reinforcement Learning for Generalizable and Robust Medical Reasoning in Vision-Language Models](https://arxiv.org/abs/2512.19317)
*A. A. Gde Yogi Pramana, Jason Ray, Anthony Jaya, Michael Wijaya*

**主要类别:** cs.AI

**AI概要:** SafeMed-R1是一个针对医学视觉问答的混合防御框架，通过对抗训练和随机平滑技术，在保持高质量医学推理的同时显著提升模型对抗攻击的鲁棒性，在PGD攻击下准确率从25%提升至84.45%。


<details>
  <summary>更多</summary>
  
**动机:** 当前视觉语言模型在医学VQA应用中存在严重的安全漏洞，标准对抗训练会降低泛化性能和临床推理质量，需要开发既能保持高质量医学推理又能抵御对抗攻击的解决方案。

**方法:** 采用两阶段混合防御框架：训练阶段使用对抗训练与组相对策略优化(AT-GRPO)来强化推理过程的鲁棒性；推理阶段使用随机平滑技术提供认证的L2范数鲁棒性保证。

**结果:** 在包含88,000多个样本的OmniMedVQA基准测试中，SafeMed-R1在PGD攻击下保持84.45%的准确率，相比标准微调模型的25%准确率提升了59个百分点，同时保持高质量的医学推理能力。

**结论:** 显式思维链推理训练不仅能提高模型的可解释性，还能增强对抗鲁棒性，表明医学AI系统的可解释性与安全性之间存在协同效应，SafeMed-R1为临床部署提供了可靠的解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是SafeMed-R1%3A+Adversarial+Reinforcement+Learning+for+Generalizable+and+Robust+Medical+Reasoning+in+Vision-Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19317，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19317&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Vision--Language Models (VLMs) show significant promise for Medical Visual Question Answering (VQA), yet their deployment in clinical settings is hindered by severe vulnerability to adversarial attacks. Standard adversarial training, while effective for simpler tasks, often degrades both generalization performance and the quality of generated clinical reasoning. We introduce SafeMed-R1, a hybrid defense framework that ensures robust performance while preserving high-quality, interpretable medical reasoning. SafeMed-R1 employs a two-stage approach: at training time, we integrate Adversarial Training with Group Relative Policy Optimization (AT-GRPO) to explicitly robustify the reasoning process against worst-case perturbations; at inference time, we augment the model with Randomized Smoothing to provide certified $L_2$-norm robustness guarantees. We evaluate SafeMed-R1 on the OmniMedVQA benchmark across eight medical imaging modalities comprising over 88,000 samples. Our experiments reveal that standard fine-tuned VLMs, despite achieving 95\% accuracy on clean inputs, collapse to approximately 25\% under PGD attacks. In contrast, SafeMed-R1 maintains 84.45\% accuracy under the same adversarial conditions, representing a 59 percentage point improvement in robustness. Furthermore, we demonstrate that models trained with explicit chain-of-thought reasoning exhibit superior adversarial robustness compared to instruction-only variants, suggesting a synergy between interpretability and security in medical AI systems.

</details>


### [53] [VIGOR+: Iterative Confounder Generation and Validation via LLM-CEVAE Feedback Loop](https://arxiv.org/abs/2512.19349)
*JiaWei Zhu, ZiHeng Liu*

**主要类别:** cs.AI

**AI概要:** VIGOR+是一个新颖框架，通过迭代反馈机制将LLM生成的混杂变量与CEVAE统计验证相结合，解决了LLM生成混杂变量语义合理但统计效用不足的问题。


<details>
  <summary>更多</summary>
  
**动机:** 现有方法利用大语言模型生成潜在混杂变量，但这些变量往往语义合理却缺乏统计效用，存在语义与统计之间的鸿沟。

**方法:** 提出VIGOR+框架，建立迭代反馈机制：将CEVAE的验证信号（信息增益、潜在一致性指标、诊断信息）转化为自然语言反馈，指导后续LLM生成轮次，直到达到收敛标准。

**结果:** 论文形式化了反馈机制，在温和假设下证明了收敛性，并提供了完整的算法框架。

**结论:** VIGOR+通过闭环迭代优化，有效解决了观测数据因果推断中隐藏混杂变量的关键挑战，弥合了语义合理性与统计效用之间的差距。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是VIGOR%2B%3A+Iterative+Confounder+Generation+and+Validation+via+LLM-CEVAE+Feedback+Loop，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19349，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19349&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Hidden confounding remains a fundamental challenge in causal inference from observational data. Recent advances leverage Large Language Models (LLMs) to generate plausible hidden confounders based on domain knowledge, yet a critical gap exists: LLM-generated confounders often exhibit semantic plausibility without statistical utility. We propose VIGOR+ (Variational Information Gain for iterative cOnfounder Refinement), a novel framework that closes the loop between LLM-based confounder generation and CEVAE-based statistical validation. Unlike prior approaches that treat generation and validation as separate stages, VIGOR+ establishes an iterative feedback mechanism: validation signals from CEVAE (including information gain, latent consistency metrics, and diagnostic messages) are transformed into natural language feedback that guides subsequent LLM generation rounds. This iterative refinement continues until convergence criteria are met. We formalize the feedback mechanism, prove convergence properties under mild assumptions, and provide a complete algorithmic framework.

</details>


### [54] [PENDULUM: A Benchmark for Assessing Sycophancy in Multimodal Large Language Models](https://arxiv.org/abs/2512.19350)
*A. B. M. Ashikur Rahman, Saeed Anwar, Muhammad Usman, Irfan Ahmad, Ajmal Mian*

**主要类别:** cs.AI

**AI概要:** 论文提出了PENDULUM评估基准，用于系统评估多模态大语言模型中的谄媚行为（过度迎合用户而忽视事实准确性），发现现有模型存在显著的谄媚和幻觉倾向，并提出了量化指标和解决方案建议。


<details>
  <summary>更多</summary>
  
**动机:** 解决多模态大语言模型中谄媚行为这一关键但未被充分探索的问题，现有研究主要局限于纯文本环境，缺乏对视觉多模态场景的系统分析。

**方法:** 构建包含约2000个人工标注视觉问答对的PENDULUM基准数据集，涵盖6个不同复杂度的图像领域，系统评估先进MLLMs的谄媚倾向，并提出新的量化指标。

**结果:** 发现当前最先进的多模态大语言模型在谄媚行为上存在显著差异和脆弱性，表现出明显的谄媚和幻觉倾向。

**结论:** 研究强调了开发抗谄媚架构和训练策略的紧迫性，以提高未来多模态大语言模型的事实一致性和可靠性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是PENDULUM%3A+A+Benchmark+for+Assessing+Sycophancy+in+Multimodal+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19350，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19350&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Sycophancy, an excessive tendency of AI models to agree with user input at the expense of factual accuracy or in contradiction of visual evidence, poses a critical and underexplored challenge for multimodal large language models (MLLMs). While prior studies have examined this behavior in text-only settings of large language models, existing research on visual or multimodal counterparts remains limited in scope and depth of analysis. To address this gap, we introduce a comprehensive evaluation benchmark, \textit{PENDULUM}, comprising approximately 2,000 human-curated Visual Question Answering pairs specifically designed to elicit sycophantic responses. The benchmark spans six distinct image domains of varying complexity, enabling a systematic investigation of how image type and inherent challenges influence sycophantic tendencies. Through extensive evaluation of state-of-the-art MLLMs. we observe substantial variability in model robustness and a pronounced susceptibility to sycophantic and hallucinatory behavior. Furthermore, we propose novel metrics to quantify sycophancy in visual reasoning, offering deeper insights into its manifestations across different multimodal contexts. Our findings highlight the urgent need for developing sycophancy-resilient architectures and training strategies to enhance factual consistency and reliability in future MLLMs. Our proposed dataset with MLLMs response are available at https://github.com/ashikiut/pendulum/.

</details>


### [55] [First-Order Representation Languages for Goal-Conditioned RL](https://arxiv.org/abs/2512.19355)
*Simon Ståhlberg, Hector Geffner*

**主要类别:** cs.AI

**AI概要:** 该研究探讨了在一阶关系语言框架下，如何通过改进Hindsight Experience Replay (HER)技术来学习目标条件化和通用策略，特别是在大型规划实例和稀疏奖励环境中。通过将状态和目标表示为原子集，并采用三种不同目标表示方式，实现了通过自动创建难度递增的子目标课程来提升学习效率。


<details>
  <summary>更多</summary>
  
**动机:** 解决在大型训练实例中，当目标无法通过随机探索实现时，如何学习目标条件化和通用策略的问题。传统HER方法虽然通过目标重标记利用失败轨迹，但在状态和目标的泛化方面仍有提升空间。

**方法:** 使用一阶关系语言表示状态和目标，提出三种目标表示方式：1)完整状态作为目标；2)原始目标的子集作为目标；3)这些子目标的提升版本。通过HER技术重标记失败轨迹，自动创建难度递增的子目标课程。

**结果:** 后两种目标表示方法（子目标和提升子目标）成功在大型规划实例和稀疏奖励环境下学习了通用策略，实现了计算效率的提升。实验展示了这些版本的计算增益、局限性以及改进机会。

**结论:** 通过一阶关系语言表示状态和目标，并结合改进的HER技术，可以有效地在大型实例中学习通用策略。子目标和提升子目标的表示方式能够自动创建渐进式学习课程，为处理稀疏奖励问题提供了有前景的方向。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是First-Order+Representation+Languages+for+Goal-Conditioned+RL，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19355，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19355&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** First-order relational languages have been used in MDP planning and reinforcement learning (RL) for two main purposes: specifying MDPs in compact form, and representing and learning policies that are general and not tied to specific instances or state spaces. In this work, we instead consider the use of first-order languages in goal-conditioned RL and generalized planning. The question is how to learn goal-conditioned and general policies when the training instances are large and the goal cannot be reached by random exploration alone. The technique of Hindsight Experience Replay (HER) provides an answer to this question: it relabels unsuccessful trajectories as successful ones by replacing the original goal with one that was actually achieved. If the target policy must generalize across states and goals, trajectories that do not reach the original goal states can enable more data- and time-efficient learning. In this work, we show that further performance gains can be achieved when states and goals are represented by sets of atoms. We consider three versions: goals as full states, goals as subsets of the original goals, and goals as lifted versions of these subgoals. The result is that the latter two successfully learn general policies on large planning instances with sparse rewards by automatically creating a curriculum of easier goals of increasing complexity. The experiments illustrate the computational gains of these versions, their limitations, and opportunities for addressing them.

</details>


### [56] [Learning General Policies with Policy Gradient Methods](https://arxiv.org/abs/2512.19366)
*Simon Ståhlberg, Blai Bonet, Hector Geffner*

**主要类别:** cs.AI

**AI概要:** 该研究将组合规划方法与深度强化学习相结合，探讨了在什么条件下DRL方法可以学习到像组合方法一样具有良好泛化能力的策略，通过使用图神经网络和状态转移分类器实现了接近组合方法的泛化性能。


<details>
  <summary>更多</summary>
  
**动机:** 强化学习方法在泛化能力方面存在挑战，而经典规划中的组合方法能够学习到对给定领域所有实例都具有可证明正确性的泛化策略。本研究旨在将这两种研究方向结合起来，探索深度强化学习方法（特别是策略优化方法）在什么条件下能够实现类似组合方法的泛化能力。

**方法:** 1) 将策略建模为状态转移分类器，因为基础动作不具有通用性且随实例变化；2) 使用适应关系结构的图神经网络(GNNs)来表示规划状态的价值函数和策略；3) 采用actor-critic方法学习策略，避免了可扩展性瓶颈和特征池的使用。

**结果:** 研究发现actor-critic方法可以学习到几乎与组合方法一样好的泛化策略，同时避免了可扩展性瓶颈。DRL方法在基准测试中的局限性主要源于GNN的表达能力限制以及最优性与泛化之间的权衡。

**结论:** 通过添加派生谓词和替代成本结构来优化，可以在不改变基本DRL方法的情况下解决GNN的表达能力限制和最优性-泛化权衡问题。这项工作展示了DRL方法在实现组合方法级别泛化能力方面的潜力。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Learning+General+Policies+with+Policy+Gradient+Methods，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19366，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19366&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** While reinforcement learning methods have delivered remarkable results in a number of settings, generalization, i.e., the ability to produce policies that generalize in a reliable and systematic way, has remained a challenge. The problem of generalization has been addressed formally in classical planning where provable correct policies that generalize over all instances of a given domain have been learned using combinatorial methods. The aim of this work is to bring these two research threads together to illuminate the conditions under which (deep) reinforcement learning approaches, and in particular, policy optimization methods, can be used to learn policies that generalize like combinatorial methods do. We draw on lessons learned from previous combinatorial and deep learning approaches, and extend them in a convenient way. From the former, we model policies as state transition classifiers, as (ground) actions are not general and change from instance to instance. From the latter, we use graph neural networks (GNNs) adapted to deal with relational structures for representing value functions over planning states, and in our case, policies. With these ingredients in place, we find that actor-critic methods can be used to learn policies that generalize almost as well as those obtained using combinatorial approaches while avoiding the scalability bottleneck and the use of feature pools. Moreover, the limitations of the DRL methods on the benchmarks considered have little to do with deep learning or reinforcement learning algorithms, and result from the well-understood expressive limitations of GNNs, and the tradeoff between optimality and generalization (general policies cannot be optimal in some domains). Both of these limitations are addressed without changing the basic DRL methods by adding derived predicates and an alternative cost structure to optimize.

</details>


### [57] [EchoTrail-GUI: Building Actionable Memory for GUI Agents via Critic-Guided Self-Exploration](https://arxiv.org/abs/2512.19396)
*Runze Li, Yuwen Zhai, Bo Xu, LiWu Xu, Nian Shi, Wei Zhang, Ran Lin, Liang Wang*

**主要类别:** cs.AI

**AI概要:** EchoTrail-GUI是一个新型GUI代理框架，通过动态记忆系统让AI能够从过去的成功经验中学习，显著提升了任务成功率和操作效率


<details>
  <summary>更多</summary>
  
**动机:** 当前GUI代理存在数字"失忆症"问题，每个任务都孤立处理，无法从过去成功经验中系统学习，导致性能次优、重复错误和泛化能力差

**方法:** 三阶段框架：1)经验探索阶段自主构建成功任务轨迹数据库；2)记忆注入阶段检索相关历史轨迹；3)GUI任务推理阶段将记忆作为上下文指导决策

**结果:** 在Android World和AndroidLab基准测试中，EchoTrail-GUI显著提高了基线代理的任务成功率和操作效率

**结论:** 结构化记忆对于创建更强大和智能的GUI自动化系统具有重要作用，该方法验证了基于经验学习的GUI代理的可行性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是EchoTrail-GUI%3A+Building+Actionable+Memory+for+GUI+Agents+via+Critic-Guided+Self-Exploration，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19396，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19396&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Contemporary GUI agents, while increasingly capable due to advances in Large Vision-Language Models (VLMs), often operate with a critical limitation: they treat each task in isolation, lacking a mechanism to systematically learn from past successes. This digital ''amnesia'' results in sub-optimal performance, repeated errors, and poor generalization to novel challenges. To bridge this gap, we introduce EchoTrail-GUI, a novel framework designed to mimic human-like experiential learning by equipping agents with a dynamic, accessible memory. Our framework operates in three distinct stages. First, during Experience Exploration, an agent autonomously interacts with GUI environments to build a curated database of successful task trajectories, validated by a reward model. Crucially, the entire knowledge base construction is thus fully automated, requiring no human supervision. Second, in the Memory Injection stage, upon receiving a new task, our system efficiently retrieves the most relevant past trajectories to serve as actionable ''memories''. Finally, during GUI Task Inference, these memories are injected as in-context guidance to inform the agent's reasoning and decision-making process. We demonstrate the efficacy of our approach on benchmarks including Android World and AndroidLab. The results show that EchoTrail-GUI significantly improves the task success rate and operational efficiency of baseline agents, validating the power of structured memory in creating more robust and intelligent GUI automation.

</details>


### [58] [An Agentic Framework for Autonomous Materials Computation](https://arxiv.org/abs/2512.19458)
*Zeyu Xia, Jinzhe Ma, Congjie Zheng, Shufei Zhang, Yuqiang Li, Hang Su, P. Hu, Changshui Zhang, Xingao Gong, Wanli Ouyang, Lei Bai, Dongzhan Zhou, Mao Su*

**主要类别:** cs.AI

**AI概要:** 提出一个面向第一性原理材料计算的专业化智能体，通过嵌入领域专业知识实现可靠的多步骤工作流自动化，显著提升计算准确性和鲁棒性。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在加速科学发现方面表现出强大能力，但其静态知识和幻觉问题阻碍了自主研究应用，需要解决可靠自动化的问题。

**方法:** 将LLM集成到智能体框架中，嵌入领域专业知识，确保物理一致的多步骤工作流，并选择收敛性良好的参数。

**结果:** 在多样化计算任务基准测试中，该系统在准确性和鲁棒性方面显著优于独立LLM。

**结论:** 这项工作为自主计算实验建立了可验证的基础，是迈向全自动科学发现的关键一步。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是An+Agentic+Framework+for+Autonomous+Materials+Computation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19458，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19458&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large Language Models (LLMs) have emerged as powerful tools for accelerating scientific discovery, yet their static knowledge and hallucination issues hinder autonomous research applications. Recent advances integrate LLMs into agentic frameworks, enabling retrieval, reasoning, and tool use for complex scientific workflows. Here, we present a domain-specialized agent designed for reliable automation of first-principles materials computations. By embedding domain expertise, the agent ensures physically coherent multi-step workflows and consistently selects convergent, well-posed parameters, thereby enabling reliable end-to-end computational execution. A new benchmark of diverse computational tasks demonstrates that our system significantly outperforms standalone LLMs in both accuracy and robustness. This work establishes a verifiable foundation for autonomous computational experimentation and represents a key step toward fully automated scientific discovery.

</details>


### [59] [QuantiPhy: A Quantitative Benchmark Evaluating Physical Reasoning Abilities of Vision-Language Models](https://arxiv.org/abs/2512.19526)
*Li Puyin, Tiange Xiang, Ella Mao, Shirley Wei, Xinye Chen, Adnan Masood, Li Fei-fei, Ehsan Adeli*

**主要类别:** cs.AI

**AI概要:** QuantiPhy是首个定量评估视觉语言模型物理推理能力的基准测试，包含3300多个视频-文本实例，测试模型对物体尺寸、速度、加速度的数值估计能力。研究发现现有VLMs在定性合理性和数值准确性之间存在明显差距，过度依赖预训练知识而非实际输入信息。


<details>
  <summary>更多</summary>
  
**动机:** 当前视觉感知模型主要基于VQA的定性评估，无法确定这些模型是否能从视频观察中定量推理运动物体的运动学量。需要建立定量评估基准来测试VLMs的物理推理能力。

**方法:** 开发QuantiPhy基准测试，包含3300+视频-文本实例和数值真值，标准化提示和评分标准，评估模型在给定时间点对物体尺寸、速度、加速度的数值估计准确性，使用其中一个属性作为输入先验。

**结果:** 实验显示最先进的VLMs在定性合理性和数值正确性之间存在一致差距。模型严重依赖预训练的世界知识，而非忠实使用提供的视觉和文本输入作为参考。

**结论:** QuantiPhy为VLMs提供了首个严格、可扩展的测试平台，推动模型超越口头合理性，实现基于数值的物理理解。基准测试揭示了当前模型在定量物理推理方面的局限性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是QuantiPhy%3A+A+Quantitative+Benchmark+Evaluating+Physical+Reasoning+Abilities+of+Vision-Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19526，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19526&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Understanding the physical world is essential for generalist AI agents. However, it remains unclear whether state-of-the-art vision perception models (e.g., large VLMs) can reason physical properties quantitatively. Existing evaluations are predominantly VQA-based and qualitative, offering limited insight into whether these models can infer the kinematic quantities of moving objects from video observations. To address this, we present QuantiPhy, the first benchmark designed to quantitatively measure a VLM's physical reasoning ability. Comprising more than 3.3K video-text instances with numerical ground truth, QuantiPhy evaluates a VLM's performance on estimating an object's size, velocity, and acceleration at a given timestamp, using one of these properties as an input prior. The benchmark standardizes prompts and scoring to assess numerical accuracy, enabling fair comparisons across models. Our experiments on state-of-the-art VLMs reveal a consistent gap between their qualitative plausibility and actual numerical correctness. We further provide an in-depth analysis of key factors like background noise, counterfactual priors, and strategic prompting and find that state-of-the-art VLMs lean heavily on pre-trained world knowledge rather than faithfully using the provided visual and textual inputs as references when reasoning kinematic properties quantitatively. QuantiPhy offers the first rigorous, scalable testbed to move VLMs beyond mere verbal plausibility toward a numerically grounded physical understanding.

</details>


### [60] [Towards Closed-Loop Embodied Empathy Evolution: Probing LLM-Centric Lifelong Empathic Motion Generation in Unseen Scenarios](https://arxiv.org/abs/2512.19551)
*Jiawen Wang, Jingjing Wang Tianyang Chen, Min Zhang, Guodong Zhou*

**主要类别:** cs.AI

**AI概要:** 本文提出了一种基于LLM的终身共情运动生成任务(L²-EMG)，通过ES-MoE方法解决情感解耦和场景适应两大挑战，在多个新构建的数据集上验证了方法的有效性。


<details>
  <summary>更多</summary>
  
**动机:** 现有情感运动生成方法主要局限于单一固定尺度数据集，无法适应灵活且尺度不断增长的真实场景(如运动、舞蹈)，而有效学习这些新兴场景能显著提升模型的现实世界泛化能力。

**方法:** 提出ES-MoE方法，包含因果引导的情感解耦模块和场景适应的专家构建模块，分别解决情感解耦和场景适应挑战。构建了多个L²-EMG数据集进行验证。

**结果:** 大量评估表明ES-MoE方法优于先进基线方法。

**结论:** 该方法为构建具有共情能力和智能的闭环自进化具身智能体提供了潜在贡献，能够持续在不同未见场景中获取情感运动生成知识。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Towards+Closed-Loop+Embodied+Empathy+Evolution%3A+Probing+LLM-Centric+Lifelong+Empathic+Motion+Generation+in+Unseen+Scenarios，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19551，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19551&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In the literature, existing human-centric emotional motion generation methods primarily focus on boosting performance within a single scale-fixed dataset, largely neglecting the flexible and scale-increasing motion scenarios (e.g., sports, dance), whereas effectively learning these newly emerging scenarios can significantly enhance the model's real-world generalization ability. Inspired by this, this paper proposes a new LLM-Centric Lifelong Empathic Motion Generation (L^2-EMG) task, which aims to equip LLMs with the capability to continually acquire emotional motion generation knowledge across different unseen scenarios, potentially contributing to building a closed-loop and self-evolving embodied agent equipped with both empathy and intelligence. Further, this paper poses two key challenges in the L^2-EMG task, i.e., the emotion decoupling challenge and the scenario adapting challenge. To this end, this paper proposes an Emotion-Transferable and Scenario-Adapted Mixture of Experts (ES-MoE) approach which designs a causal-guided emotion decoupling block and a scenario-adapted expert constructing block to address the two challenges, respectively. Especially, this paper constructs multiple L^2-EMG datasets to validate the effectiveness of the ES-MoE approach. Extensive evaluations show that ES-MoE outperforms advanced baselines.

</details>


### [61] [Augmenting Intelligence: A Hybrid Framework for Scalable and Stable Explanations](https://arxiv.org/abs/2512.19557)
*Lawrence Krukrubo, Julius Odede, Olawande Olusegun*

**主要类别:** cs.AI

**AI概要:** 本文提出了一种混合LRR-TED框架来解决可解释AI的可扩展性与稳定性困境，通过自动化规则学习与少量人工规则相结合，在客户流失预测中实现了94%的准确率，同时将人工标注工作量减少50%。


<details>
  <summary>更多</summary>
  
**动机:** 当前可解释AI方法面临"可扩展性-稳定性困境"：后处理方法（如LIME、SHAP）可扩展但稳定性差，监督解释框架（如TED）稳定但需要大量人工标注。

**方法:** 提出混合LRR-TED框架，利用"发现不对称性"原理：使用自动化规则学习器（GLRM）识别广泛的"安全网"（保留模式），并通过帕累托最优方法仅添加4条人工定义的风险规则来捕捉特定的"风险陷阱"（流失触发因素）。

**结果:** 在客户流失预测中达到94.00%的预测准确率，优于完整的8条人工专家基准，同时将人工标注工作量减少50%。

**结论:** 该框架提出了人机协同AI范式的转变：将专家从"规则编写者"的角色转变为"异常处理者"，有效解决了可解释AI的可扩展性与稳定性之间的权衡问题。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Augmenting+Intelligence%3A+A+Hybrid+Framework+for+Scalable+and+Stable+Explanations，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19557，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19557&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Current approaches to Explainable AI (XAI) face a "Scalability-Stability Dilemma." Post-hoc methods (e.g., LIME, SHAP) may scale easily but suffer from instability, while supervised explanation frameworks (e.g., TED) offer stability but require prohibitive human effort to label every training instance. This paper proposes a Hybrid LRR-TED framework that addresses this dilemma through a novel "Asymmetry of Discovery." When applied to customer churn prediction, we demonstrate that automated rule learners (GLRM) excel at identifying broad "Safety Nets" (retention patterns) but struggle to capture specific "Risk Traps" (churn triggers)-a phenomenon we term the Anna Karenina Principle of Churn. By initialising the explanation matrix with automated safety rules and augmenting it with a Pareto-optimal set of just four human-defined risk rules, our approach achieves 94.00% predictive accuracy. This configuration outperforms the full 8-rule manual expert baseline while reducing human annotation effort by 50%, proposing a shift in the paradigm for Human-in-the-Loop AI: moving experts from the role of "Rule Writers" to "Exception Handlers."

</details>


### [62] [Scalably Enhancing the Clinical Validity of a Task Benchmark with Physician Oversight](https://arxiv.org/abs/2512.19691)
*Junze Ye, Daniel Tawfik, Alex J. Goodell, Nikhil V. Kotha, Mark K. Buyyounouski, Mohsen Bayati*

**主要类别:** cs.AI

**AI概要:** N/A


<details>
  <summary>更多</summary>
  
**动机:** N/A

**方法:** N/A

**结果:** N/A

**结论:** N/A

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Scalably+Enhancing+the+Clinical+Validity+of+a+Task+Benchmark+with+Physician+Oversight，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19691，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19691&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Automating the calculation of clinical risk scores offers a significant opportunity to reduce physician administrative burden and enhance patient care. The current standard for evaluating this capability is MedCalc-Bench, a large-scale dataset constructed using LLM-based feature extraction and rule-based aggregation. However, treating such model-generated benchmarks as static oracles risks enshrining historical model errors as evaluation gold standards, a problem dangerously amplified when these datasets serve as reward signals for Reinforcement Learning (RL). In this work, we propose viewing benchmarks for complex tasks such as clinical score computation as ''in-progress living documents'' that should be periodically re-evaluated as the processes for creating them improve. We introduce a systematic, physician-in-the-loop pipeline that leverages advanced agentic verifiers to audit and relabel MedCalc-Bench, utilizing automated triage to reserve scarce clinician attention for the most contentious instances. Our audit reveals that a notable fraction of original labels diverge from medical ground truth due to extraction errors, calculator logic mismatches, and clinical ambiguity. To study whether this label noise meaningfully impacts downstream RL training, we fine-tune a Qwen3-8B model via Group Relative Policy Optimization (GRPO) and demonstrate that training on corrected labels yields an 8.7% absolute improvement in accuracy over the original baseline -- validating that label noise materially affects model evaluation. These findings underscore that in safety-critical domains, rigorous benchmark maintenance is a prerequisite for genuine model alignment.

</details>


<div id='cs.CL'></div>

# cs.CL [[Back]](#toc)

### [63] [Towards Reasoning-Preserving Unlearning in Multimodal Large Language Models](https://arxiv.org/abs/2512.17911)
*Hongji Li, Junchi yao, Manjiang Yu, Priyanka Singh, Xue Li, Di Wang, Lijie Hu*

**主要类别:** cs.CL

**AI概要:** 提出了首个针对推理多模态大语言模型(RMLLMs)的机器遗忘基准RMLLMU-Bench，并开发了R-MUSE方法，通过子空间引导和自适应调控实现推理级遗忘同时保持推理能力。


<details>
  <summary>更多</summary>
  
**动机:** 现有机器遗忘方法无法同时解决推理多模态大模型的两个关键问题：中间推理步骤可能泄露敏感信息，以及过度干预会损害通用推理能力，缺乏专门的评估基准。

**方法:** 提出RMLLMU-Bench基准，扩展标准遗忘指标，专门评估推理泄露和推理保留；开发R-MUSE方法，通过训练无关的推理时干预框架，利用子空间引导和自适应调控来同时遗忘答案和推理痕迹。

**结果:** 系统评估显示现有方法要么在推理过程中存在显著泄露，要么严重降低推理性能；R-MUSE在RMLLMU-Bench上实现了遗忘效果和推理保留之间的显著更好平衡。

**结论:** R-MUSE是首个能够有效处理RMLLM推理级遗忘问题的方法，在保持通用推理能力的同时成功抑制推理过程中的信息泄露，为多模态推理模型的隐私保护提供了有效解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Towards+Reasoning-Preserving+Unlearning+in+Multimodal+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17911，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17911&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Machine unlearning aims to erase requested data from trained models without full retraining. For Reasoning Multimodal Large Language Models (RMLLMs), this is uniquely challenging: intermediate chain-of-thought steps can still leak sensitive information even when final answers are forgotten, and overly aggressive interventions easily damage general reasoning ability. Yet no benchmark jointly evaluates how well unlearning methods suppress reasoning-level leakage while preserving reasoning competence. We address this gap with RMLLMU-Bench, the first benchmark for RMLLM unlearning that extends standard forgetting metrics with dedicated measures of reasoning leakage and reasoning retention. A systematic evaluation on RMLLMU-Bench reveals that existing unlearning methods for MLLMs and Large (Language) Reasoning Models (LRMs) either leave substantial leakage in the reasoning process or severely degrade reasoning performance. To address these gaps, we propose R-MUSE (Reasoning-preserving MLLM Unlearning via Subspace guidance and Adaptive Steering), a training-free and inference-time intervention framework that steers internal representations to forget both answers and reasoning traces while explicitly preserving general reasoning. Experiments on RMLLMU-Bench demonstrate that R-MUSE achieves a substantially better balance between effective forgetting and reasoning retention.

</details>


### [64] [Graph-O1 : Monte Carlo Tree Search with Reinforcement Learning for Text-Attributed Graph Reasoning](https://arxiv.org/abs/2512.17912)
*Lihui Liu*

**主要类别:** cs.CL

**AI概要:** Graph-O1是一个基于蒙特卡洛树搜索和强化学习的智能体框架，用于在文本属性图上进行逐步推理，解决了传统方法因上下文长度限制导致的推理碎片化问题。


<details>
  <summary>更多</summary>
  
**动机:** 现有检索增强生成方法要么忽略图结构信息，要么因序列化大子图导致超出LLM上下文限制，造成推理不完整和准确性下降。

**方法:** 结合蒙特卡洛树搜索(MCTS)和端到端强化学习，构建智能体与图环境的多轮交互框架，选择性探索和检索最有信息量的子图组件。

**结果:** 在多个LLM骨干网络上进行广泛实验，Graph-O1 consistently超越最先进的基线方法，生成更准确、可靠和可解释的答案。

**结论:** Graph-O1框架成功解决了文本属性图上的问答挑战，通过智能体逐步推理实现了更好的性能表现，为图结构数据的LLM推理提供了有效解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Graph-O1+%3A+Monte+Carlo+Tree+Search+with+Reinforcement+Learning+for+Text-Attributed+Graph+Reasoning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17912，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17912&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** ChatGPT said: Text-attributed graphs, where nodes and edges contain rich textual information, are widely used across diverse domains. A central challenge in this setting is question answering, which requires jointly leveraging unstructured text and the structured relational signals within the graph. Although Large Language Models (LLMs) have made significant advances in natural language understanding, their direct use for reasoning over text-attributed graphs remains limited. Retrieval-augmented generation methods that operate purely on text often treat passages as isolated units, ignoring the interconnected structure of the graph. Conversely, graph-based RAG methods that serialize large subgraphs into long textual sequences quickly become infeasible due to LLM context-length constraints, resulting in fragmented reasoning and degraded accuracy. To overcome these limitations, we introduce Graph-O1, an agentic GraphRAG framework that enables LLMs to conduct stepwise, interactive reasoning over graphs. Our approach integrates Monte Carlo Tree Search (MCTS) with end-to-end reinforcement learning, allowing the model to selectively explore and retrieve only the most informative subgraph components. The reasoning procedure is framed as a multi-turn interaction between the agent and the graph environment, and the agent is trained through a unified reward mechanism. Extensive experiments across multiple LLM backbones demonstrate that Graph-O1 consistently surpasses state-of-the-art baselines, producing answers that are more accurate, reliable, and interpretable.

</details>


### [65] [Q-KVComm: Efficient Multi-Agent Communication Via Adaptive KV Cache Compression](https://arxiv.org/abs/2512.17914)
*Boris Kriuk, Logic Ng*

**主要类别:** cs.CL

**AI概要:** Q-KVComm是一种新的LLM代理间通信协议，通过传输压缩的KV缓存表示而非原始文本来减少冗余信息传输，实现5-6倍压缩比同时保持语义保真度


<details>
  <summary>更多</summary>
  
**动机:** 多智能体LLM系统中上下文信息的冗余传输消耗过多带宽和计算资源，传统方法丢弃内部语义表示，迫使接收代理重新计算相似表示

**方法:** 结合三层创新技术：自适应分层量化、混合信息提取和异构模型校准，实现压缩KV缓存的直接传输

**结果:** 在三个问答数据集上实现5-6倍压缩比，语义保真度保持良好，一致性质量得分均超过0.77，在不同模型规模和实际应用场景中表现稳健

**结论:** Q-KVComm建立了LLM代理通信的新范式，从基于文本的信息交换转向基于表示的信息交换

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Q-KVComm%3A+Efficient+Multi-Agent+Communication+Via+Adaptive+KV+Cache+Compression，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17914，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17914&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Multi-agent Large Language Model (LLM) systems face a critical bottleneck: redundant transmission of contextual information between agents consumes excessive bandwidth and computational resources. Traditional approaches discard internal semantic representations and transmit raw text, forcing receiving agents to recompute similar representations from scratch. We introduce Q-KVComm, a new protocol that enables direct transmission of compressed key-value (KV) cache representations between LLM agents. Q-KVComm combines three key innovations: (1) adaptive layer-wise quantization that allocates variable bit-widths based on sensitivity profiling, (2) hybrid information extraction that preserves critical facts across content domains, and (3) heterogeneous model calibration establishing cross-architecture communication. Extensive experiments across three diverse question-answering datasets demonstrate that Q-KVComm achieves 5-6x compression ratios while maintaining semantic fidelity, with coherence quality scores above 0.77 across all scenarios. The protocol exhibits robust performance across model sizes (1.1B-1.5B parameters) and adapts to real-world applications including conversational QA and multi-hop reasoning. Our work establishes a new paradigm for LLM agent communication, shifting from text-based to representation-based information exchange.

</details>


### [66] [Supplementary Resources and Analysis for Automatic Speech Recognition Systems Trained on the Loquacious Dataset](https://arxiv.org/abs/2512.17915)
*Nick Rossenbach, Robin Schmitt, Tina Raissi, Simon Berger, Larissa Kleppel, Ralf Schlüter*

**主要类别:** cs.CL

**AI概要:** Loquacious数据集旨在替代LibriSpeech和TED-Lium等传统ASR数据集，提供跨多个声学和语言领域的标准化训练测试划分，并附带语言模型、G2P模型和发音词典等额外资源。


<details>
  <summary>更多</summary>
  
**动机:** 为学术界和工业界提供一个具有开放许可、分区定义清晰且覆盖多领域的ASR基准数据集，解决现有数据集在领域覆盖和许可限制方面的问题。

**方法:** 构建Loquacious数据集并提供n-gram语言模型、grapheme-to-phoneme模型和发音词典等配套资源，使用多种ASR架构和不同标签单元进行实验验证。

**结果:** 初步实验结果表明，Loquacious数据集能够为ASR领域的各种常见挑战提供有价值的研究案例。

**结论:** Loquacious数据集及其配套资源为ASR研究提供了高质量的基准平台，具有广泛的学术和工业应用价值。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Supplementary+Resources+and+Analysis+for+Automatic+Speech+Recognition+Systems+Trained+on+the+Loquacious+Dataset，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17915，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17915&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The recently published Loquacious dataset aims to be a replacement for established English automatic speech recognition (ASR) datasets such as LibriSpeech or TED-Lium. The main goal of the Loquacious dataset is to provide properly defined training and test partitions across many acoustic and language domains, with an open license suitable for both academia and industry. To further promote the benchmarking and usability of this new dataset, we present additional resources in the form of n-gram language models (LMs), a grapheme-to-phoneme (G2P) model and pronunciation lexica, with open and public access. Utilizing those additional resources we show experimental results across a wide range of ASR architectures with different label units and topologies. Our initial experimental results indicate that the Loquacious dataset offers a valuable study case for a variety of common challenges in ASR.

</details>


### [67] [Learning to Prioritize IT Tickets: A Comparative Evaluation of Embedding-based Approaches and Fine-Tuned Transformer Models](https://arxiv.org/abs/2512.17916)
*Minh Tri LÊ, Ali Ait-Bachir*

**主要类别:** cs.CL

**AI概要:** 本文比较了IT服务管理中工单优先级分类的两种方法：基于嵌入的流水线方法和微调多语言transformer方法。研究发现transformer方法在性能上显著优于传统嵌入方法，F1分数达到78.5%，Cohen's kappa值接近0.80。


<details>
  <summary>更多</summary>
  
**动机:** IT服务管理中的工单优先级分类面临文本噪声、主观写作风格和类别不平衡等挑战，需要寻找更有效的自动化解决方案。

**方法:** 评估了两种方法：1)基于嵌入的流水线方法（降维+聚类+传统分类器）；2)微调多语言transformer模型，同时处理文本和数值特征。

**结果:** 嵌入方法在30种配置下泛化能力有限，聚类无法发现有意义的结构，监督模型对嵌入质量高度敏感。Transformer模型表现优异，平均F1-score为78.5%，加权Cohen's kappa值接近0.80。

**结论:** 通用嵌入方法在ITSM数据上存在局限性，领域适应的transformer架构在工单优先级分类任务中表现出色，具有更强的标签对齐能力。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Learning+to+Prioritize+IT+Tickets%3A+A+Comparative+Evaluation+of+Embedding-based+Approaches+and+Fine-Tuned+Transformer+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17916，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17916&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Prioritizing service tickets in IT Service Management (ITSM) is critical for operational efficiency but remains challenging due to noisy textual inputs, subjective writing styles, and pronounced class imbalance. We evaluate two families of approaches for ticket prioritization: embedding-based pipelines that combine dimensionality reduction, clustering, and classical classifiers, and a fine-tuned multilingual transformer that processes both textual and numerical features. Embedding-based methods exhibit limited generalization across a wide range of thirty configurations, with clustering failing to uncover meaningful structures and supervised models highly sensitive to embedding quality. In contrast, the proposed transformer model achieves substantially higher performance, with an average F1-score of 78.5% and weighted Cohen's kappa values of nearly 0.80, indicating strong alignment with true labels. These results highlight the limitations of generic embeddings for ITSM data and demonstrate the effectiveness of domain-adapted transformer architectures for operational ticket prioritization.

</details>


### [68] [KVReviver: Reversible KV Cache Compression with Sketch-Based Token Reconstruction](https://arxiv.org/abs/2512.17917)
*Aomufei Yuan, Zhiming Wang, Ruijie Miao, Dayu Wang, Yuxuan Tian, Zihan Wang, Yebo Peng, Yuhan Wu, Bairen Yi, Xin Liu, Tong Yang*

**主要类别:** cs.CL

**AI概要:** KVReviver是一种基于草图算法的可逆KV缓存压缩方法，能在保持推理精度的同时大幅减少内存使用，解决大语言模型部署中的内存瓶颈问题。


<details>
  <summary>更多</summary>
  
**动机:** 随着大语言模型上下文长度增加，KV缓存的内存需求成为部署和批处理的瓶颈。传统压缩方法会永久丢失token信息，导致上下文遗忘问题。

**方法:** 提出基于草图算法的可逆KV缓存压缩方法，通过额外的数据结构重建被压缩的token，在有限内存内实现完整计算。

**结果:** 在2k长度上下文中仅需10%的KV缓存预算即可保持相同推理精度；在32k长度上下文中仅用25%缓存预算就能达到相当精度（约2%精度损失）。

**结论:** KVReviver有效解决了KV缓存的内存瓶颈问题，通过可逆压缩避免了信息永久丢失，为长上下文LLM部署提供了实用的解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是KVReviver%3A+Reversible+KV+Cache+Compression+with+Sketch-Based+Token+Reconstruction，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17917，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17917&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** As the context length of current large language models (LLMs) rapidly increases, the memory demand for the Key-Value (KV) cache is becoming a bottleneck for LLM deployment and batch processing. Traditional KV cache compression methods typically involve permanently evicting or irreversibly merging "less important" tokens with low attention scores. This approach results in the unrecoverable loss of token information, which we call Contextual Amnesia, significantly degrading the model's information retrieval capability. To address this issue, we propose KVReviver, a reversible KV cache compression method based on the sketch algorithm. This method allows reconstructing compressed tokens from an additional data structure, thus enabling full-scale computation within limited memory. Experiments showed that in 2k-length contexts, it requires only 10% of KV Cache budget while maintaining identical end-to-end inference accuracy. For 32k-length contexts, it achieves equivalent or comparable accuracy ~2% accuracy loss) using merely 25% of KV Cache budget.

</details>


### [69] [Separating Constraint Compliance from Semantic Accuracy: A Novel Benchmark for Evaluating Instruction-Following Under Compression](https://arxiv.org/abs/2512.17920)
*Rahul Baxi*

**主要类别:** cs.CL

**AI概要:** LLMs在提示压缩下性能下降，研究发现RLHF训练的'乐于助人'行为是导致中等压缩级别约束违反的主要原因，极端压缩表现反而更好。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在提示压缩下性能退化，但机制尚不明确，需要系统研究压缩对模型约束遵循和语义准确性的影响。

**方法:** 引入压缩衰减理解测试(CDCT)，独立测量约束遵循(CC)和语义准确性(SA)，评估9个前沿LLM在8个概念和5个压缩级别下的表现，使用三法官LLM陪审团进行评分。

**结果:** 发现普遍的U型曲线模式(97.2%出现率)，约束违反在中度压缩时达到峰值；RLHF消融实验显示移除'乐于助人'信号使CC平均提升598%；推理模型比高效模型表现好27.5%。

**结论:** 研究揭示了RLHF对齐与指令遵循之间的根本性冲突，为改进部署系统提供了可行指导。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Separating+Constraint+Compliance+from+Semantic+Accuracy%3A+A+Novel+Benchmark+for+Evaluating+Instruction-Following+Under+Compression，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.17920，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.17920&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) exhibit degraded performance under prompt compression, but the mechanisms remain poorly understood. We introduce the Compression-Decay Comprehension Test (CDCT), a benchmark that independently measures constraint compliance (CC) and semantic accuracy (SA) across compression levels. We evaluate 9 frontier LLMs across 8 concepts using 5 compression levels from extreme (c=0.0, ~2 words) to none (c=1.0, ~135 words). A three-judge LLM jury achieves almost perfect inter-rater agreement on CC (Fleiss' \k{appa}=0.90).
  We observe a universal U-curve pattern in constraint compliance (97.2% prevalence), with violations peaking at medium compression (c=0.5, ~27 words). Counterintuitively, models perform better at extreme compression than medium lengths. The dimensions are statistically orthogonal (r=0.193, p=0.084), with constraint effects 2.9x larger than semantic effects.
  Experimental validation via RLHF ablation confirms our constraint salience hypothesis: removing "helpfulness" signals improves CC by 598% on average (71/72 trials, p<0.001), with 79% achieving perfect compliance. This demonstrates that RLHF-trained helpfulness behaviors are the dominant cause of constraint violations at medium compression. Reasoning models outperform efficient models by 27.5% (Cohen's d=0.96).
  Our findings reveal a fundamental tension between RLHF alignment and instruction-following, providing actionable guidelines for improving deployed systems.

</details>


### [70] [ReGal: A First Look at PPO-based Legal AI for Judgment Prediction and Summarization in India](https://arxiv.org/abs/2512.18014)
*Shubham Kumar Nigam, Tanuj Tyagi, Siddharth Shukla, Aditya Kumar Guru, Balaramamahanthi Deepak Patnaik, Danush Khanna, Noel Shallum, Kripabandhu Ghosh, Arnab Bhattacharya*

**主要类别:** cs.CL

**AI概要:** 本文介绍了ReGal框架，结合多任务指令调优和基于AI反馈的强化学习，在印度法律AI背景下进行早期探索，虽然在标准评估指标上表现不如监督学习和专有模型，但为法律文本的RL应用提供了重要见解。


<details>
  <summary>更多</summary>
  
**动机:** 探索在印度法律背景下应用强化学习方法于法律AI，解决法律文本处理中的特殊挑战，如奖励模型对齐、法律语言复杂性和领域适应性。

**方法:** 提出ReGal框架，整合多任务指令调优和基于AI反馈的强化学习（RLAIF），使用近端策略优化（PPO）算法，在两个关键法律任务上进行评估：法院判决预测与解释（CJPE）和法律文档摘要。

**结果:** 框架在标准评估指标上表现不如监督学习和专有模型，但通过实证和定性分析展示了RL在高风险、长文档法律任务中的应用潜力。

**结论:** 研究为未来使用强化学习优化法律推理流程奠定了基础，对构建可解释和自适应的法律AI系统具有广泛意义。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是ReGal%3A+A+First+Look+at+PPO-based+Legal+AI+for+Judgment+Prediction+and+Summarization+in+India，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18014，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18014&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper presents an early exploration of reinforcement learning methodologies for legal AI in the Indian context. We introduce Reinforcement Learning-based Legal Reasoning (ReGal), a framework that integrates Multi-Task Instruction Tuning with Reinforcement Learning from AI Feedback (RLAIF) using Proximal Policy Optimization (PPO). Our approach is evaluated across two critical legal tasks: (i) Court Judgment Prediction and Explanation (CJPE), and (ii) Legal Document Summarization. Although the framework underperforms on standard evaluation metrics compared to supervised and proprietary models, it provides valuable insights into the challenges of applying RL to legal texts. These challenges include reward model alignment, legal language complexity, and domain-specific adaptation. Through empirical and qualitative analysis, we demonstrate how RL can be repurposed for high-stakes, long-document tasks in law. Our findings establish a foundation for future work on optimizing legal reasoning pipelines using reinforcement learning, with broader implications for building interpretable and adaptive legal AI systems.

</details>


### [71] [CoPE: A Small Language Model for Steerable and Scalable Content Labeling](https://arxiv.org/abs/2512.18027)
*Samidh Chakrabarti, David Willner, Kevin Klyman, Tiffany Saade, Emily Capstick, Sabina Nong*

**主要类别:** cs.CL

**AI概要:** CoPE是一个可政策导向的小型语言模型，通过矛盾示例训练和双目标注方法实现快速准确的内容标注，在7个危害领域达到与前沿模型相当或更好的准确率，但参数量仅为1%。


<details>
  <summary>更多</summary>
  
**动机:** 开发一个能够在消费级GPU上运行的小型语言模型，通过政策解释而非简单记忆来实现内容标注，为在线平台治理提供新的设计可能性。

**方法:** 使用矛盾示例训练方法使模型学习政策解释，采用双目标注方法快速构建明确的训练数据集。

**结果:** 在7个不同危害领域的评估中，CoPE以仅1%的参数量达到与前沿模型相当或更优的准确率，并发布了90亿参数的版本。

**结论:** CoPE代表了分类器系统的范式转变，将机器学习任务转变为政策编写任务，为在线平台治理开辟了新的设计可能性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是CoPE%3A+A+Small+Language+Model+for+Steerable+and+Scalable+Content+Labeling，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18027，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18027&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper details the methodology behind CoPE, a policy-steerable small language model capable of fast and accurate content labeling. We present a novel training curricula called Contradictory Example Training that enables the model to learn policy interpretation rather than mere policy memorization. We also present a novel method for generating content policies, called Binocular Labeling, which enables rapid construction of unambiguous training datasets. When evaluated across seven different harm areas, CoPE exhibits equal or superior accuracy to frontier models at only 1% of their size. We openly release a 9 billion parameter version of the model that can be run on a single consumer-grade GPU. Models like CoPE represent a paradigm shift for classifier systems. By turning an ML task into a policy writing task, CoPE opens up new design possibilities for the governance of online platforms.

</details>


### [72] [Narrative Consolidation: Formulating a New Task for Unifying Multi-Perspective Accounts](https://arxiv.org/abs/2512.18041)
*Roger A. Finger, Eduardo G. Cortes, Sandro J. Rigo, Gabriel de O. Ramos*

**主要类别:** cs.CL

**AI概要:** 本文提出了叙事整合新任务，通过时间对齐事件图(TAEG)实现多文档的时序整合，在圣经福音书测试中实现完美时序排序和显著内容提升


<details>
  <summary>更多</summary>
  
**动机:** 传统多文档摘要方法注重简洁性但破坏叙事流，需要一种能保持时序完整性、完整性和细节融合的新方法

**方法:** 引入时间对齐事件图(TAEG)结构，使用中心性算法选择每个事件的最核心表示，确保正确的时序位置

**结果:** 在圣经福音书测试中实现完美时序排序(Kendall's Tau 1.000)，ROUGE-L F1分数提升357.2%

**结论:** 叙事整合是一个重要的NLP任务，显式的时间框架是其解决的基本要素，TAEG方法验证了这一任务定义的合理性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Narrative+Consolidation%3A+Formulating+a+New+Task+for+Unifying+Multi-Perspective+Accounts，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18041，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18041&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Processing overlapping narrative documents, such as legal testimonies or historical accounts, often aims not for compression but for a unified, coherent, and chronologically sound text. Standard Multi-Document Summarization (MDS), with its focus on conciseness, fails to preserve narrative flow. This paper formally defines this challenge as a new NLP task: Narrative Consolidation, where the central objectives are chronological integrity, completeness, and the fusion of complementary details. To demonstrate the critical role of temporal structure in this task, we introduce Temporal Alignment Event Graph (TAEG), a graph structure that explicitly models chronology and event alignment. By applying a standard centrality algorithm to TAEG, our method functions as a version selection mechanism, choosing the most central representation of each event in its correct temporal position. In a study on the four Biblical Gospels, this structure-focused approach guarantees perfect temporal ordering (Kendall's Tau of 1.000) by design and dramatically improves content metrics (e.g., +357.2% in ROUGE-L F1). The success of this baseline method validates the formulation of Narrative Consolidation as a relevant task and establishes that an explicit temporal backbone is a fundamental component for its resolution.

</details>


### [73] [Statistical laws and linguistics inform meaning in naturalistic and fictional conversation](https://arxiv.org/abs/2512.18072)
*Ashley M. A. Fehr, Calla G. Beauregard, Julia Witte Zimmerman, Katie Ekström, Pablo Rosillo-Rodes, Christopher M. Danforth, Peter Sheridan Dodds*

**主要类别:** cs.CL

**AI概要:** 研究发现对话中的词汇增长遵循Heaps定律，但不同词性的词汇增长模式存在差异，视频聊天和电影对话两种媒介中的表现有所不同。


<details>
  <summary>更多</summary>
  
**动机:** 对话是社会连接的核心，但关于对话如何随时间展开的研究较少，特别是词汇增长模式（Heaps定律）在对话中的应用以及语言特征如何影响这种增长。

**方法:** 通过分析两种不同媒介的对话数据：1）陌生人在视频聊天中的对话；2）电影中虚构角色的对话，测量词汇量随对话长度增长的Heaps定律模式。

**结果:** 发现词汇量的增长模式因词性不同而存在差异，不同词类的词汇在对话中的出现频率和增长速率各不相同。

**结论:** 对话中的词汇增长模式受到语言特征的显著影响，这一发现为从行为和语言学角度理解对话动态提供了新的见解。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Statistical+laws+and+linguistics+inform+meaning+in+naturalistic+and+fictional+conversation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18072，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18072&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Conversation is a cornerstone of social connection and is linked to well-being outcomes. Conversations vary widely in type with some portion generating complex, dynamic stories. One approach to studying how conversations unfold in time is through statistical patterns such as Heaps' law, which holds that vocabulary size scales with document length. Little work on Heaps's law has looked at conversation and considered how language features impact scaling. We measure Heaps' law for conversations recorded in two distinct mediums: 1. Strangers brought together on video chat and 2. Fictional characters in movies. We find that scaling of vocabulary size differs by parts of speech. We discuss these findings through behavioral and linguistic frameworks.

</details>


### [74] [Training LLMs with LogicReward for Faithful and Rigorous Reasoning](https://arxiv.org/abs/2512.18196)
*Jundong Xu, Hao Fei, Huichi Zhou, Xin Quan, Qijun Huang, Shengqiong Wu, William Yang Wang, Mong-Li Lee, Wynne Hsu*

**主要类别:** cs.CL

**AI概要:** LogicReward：基于定理证明器的逻辑奖励系统，通过强制步骤级逻辑正确性来指导模型训练，在自然语言推理和逻辑推理任务上超越GPT-4o和o4-mini


<details>
  <summary>更多</summary>
  
**动机:** 现有训练方法主要依赖结果反馈，可能产生正确但推理过程有缺陷的答案，缺乏逻辑严密性保证，这在逻辑一致性至关重要的高风险场景中尤为关键

**方法:** 提出LogicReward奖励系统，使用定理证明器强制执行步骤级逻辑正确性；引入软统一自动形式化方法，减少自然语言歧义，提高形式化质量

**结果:** 8B模型在自然语言推理和逻辑推理任务上分别超越GPT-4o和o4-mini 11.6%和2%；提升推理忠实度，改善数学和常识推理等未见任务的泛化能力，无需真实标签也能提供可靠奖励信号

**结论:** LogicReward通过定理证明确保逻辑正确性，显著提升LLM推理能力，为高风险场景下的可靠推理提供了有效解决方案

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Training+LLMs+with+LogicReward+for+Faithful+and+Rigorous+Reasoning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18196，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18196&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Although LLMs exhibit strong reasoning capabilities, existing training methods largely depend on outcome-based feedback, which can produce correct answers with flawed reasoning. Prior work introduces supervision on intermediate steps but still lacks guarantees of logical soundness, which is crucial in high-stakes scenarios where logical consistency is paramount. To address this, we propose LogicReward, a novel reward system that guides model training by enforcing step-level logical correctness with a theorem prover. We further introduce Autoformalization with Soft Unification, which reduces natural language ambiguity and improves formalization quality, enabling more effective use of the theorem prover. An 8B model trained on data constructed with LogicReward surpasses GPT-4o and o4-mini by 11.6\% and 2\% on natural language inference and logical reasoning tasks with simple training procedures. Further analysis shows that LogicReward enhances reasoning faithfulness, improves generalizability to unseen tasks such as math and commonsense reasoning, and provides a reliable reward signal even without ground-truth labels. We will release all data and code at https://llm-symbol.github.io/LogicReward.

</details>


### [75] [GeoSense-AI: Fast Location Inference from Crisis Microblogs](https://arxiv.org/abs/2512.18225)
*Deepit Sapru*

**主要类别:** cs.CL

**AI概要:** GeoSense-AI是一个实时地理定位AI管道，通过整合多种NLP技术直接从社交媒体文本中提取地理位置信息，而非依赖稀疏的地理标签，为应急响应提供快速的位置感知能力。


<details>
  <summary>更多</summary>
  
**动机:** 传统方法依赖稀疏的地理标签，无法满足紧急情况下实时位置信息提取的需求。需要开发能够在嘈杂的微博流中快速准确识别地理位置的技术，以支持紧急情况下的态势感知。

**方法:** 整合统计hashtag分割、词性驱动的专有名词检测、灾害词典的依存解析、轻量级命名实体识别和地名词典消歧等多种NLP技术，在流式处理约束下进行信息提取。

**结果:** 与广泛使用的NER工具包相比，该系统在保持高F1分数的同时，实现了数量级更快的吞吐量，能够部署在实时危机信息处理场景中。

**结论:** 通过优先考虑非正式文本的鲁棒性和流式处理效率，GeoSense-AI展示了领域调优的NLP和知识基础如何提升应急响应能力，超越传统地理标签依赖。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是GeoSense-AI%3A+Fast+Location+Inference+from+Crisis+Microblogs，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18225，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18225&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper presents an applied AI pipeline for realtime geolocation from noisy microblog streams, unifying statistical hashtag segmentation, part-of-speech-driven proper-noun detection, dependency parsing around disaster lexicons, lightweight named-entity recognition, and gazetteer-grounded disambiguation to infer locations directly from text rather than sparse geotags. The approach operationalizes information extraction under streaming constraints, emphasizing low-latency NLP components and efficient validation against geographic knowledge bases to support situational awareness during emergencies. In head to head comparisons with widely used NER toolkits, the system attains strong F1 while being engineered for orders-of-magnitude faster throughput, enabling deployment in live crisis informatics settings. A production map interface demonstrates end-to-end AI functionality ingest, inference, and visualization--surfacing locational signals at scale for floods, outbreaks, and other fastmoving events. By prioritizing robustness to informal text and streaming efficiency, GeoSense-AI illustrates how domain-tuned NLP and knowledge grounding can elevate emergency response beyond conventional geo-tag reliance.

</details>


### [76] [InstructNet: A Novel Approach for Multi-Label Instruction Classification through Advanced Deep Learning](https://arxiv.org/abs/2512.18301)
*Tanjim Taharat Aurpa, Md Shoaib Ahmed, Md Mahbubur Rahman, Md. Golam Moazzam*

**主要类别:** cs.CL

**AI概要:** 本研究使用wikiHow的11,121条多标签教学文章数据，采用XLNet和BERT等Transformer架构进行多标签指令分类，其中XLNet架构表现最佳，达到97.30%的准确率和93%的宏平均F1分数。


<details>
  <summary>更多</summary>
  
**动机:** 人们经常使用"How To"前缀搜索解决问题的方法指令，对教学文本进行分类对于任务导向学习和知识库构建至关重要。

**方法:** 使用来自wikiHow的11,121条多标签教学文章数据集，采用基于Transformer的深度神经网络架构（XLNet、BERT等）进行多标签指令分类。

**结果:** XLNet架构表现最优，准确率达到97.30%，微平均和宏平均F1分数分别为89.02%和93%，在多标签分类中取得了显著成果。

**结论:** XLNet架构在提出的InstructNet方法中表现出色，通过多层次评估策略全面了解了架构的有效性，并确定了未来改进的方向。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是InstructNet%3A+A+Novel+Approach+for+Multi-Label+Instruction+Classification+through+Advanced+Deep+Learning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18301，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18301&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** People use search engines for various topics and items, from daily essentials to more aspirational and specialized objects. Therefore, search engines have taken over as peoples preferred resource. The How To prefix has become familiar and widely used in various search styles to find solutions to particular problems. This search allows people to find sequential instructions by providing detailed guidelines to accomplish specific tasks. Categorizing instructional text is also essential for task-oriented learning and creating knowledge bases. This study uses the How To articles to determine the multi-label instruction category. We have brought this work with a dataset comprising 11,121 observations from wikiHow, where each record has multiple categories. To find out the multi-label category meticulously, we employ some transformer-based deep neural architectures, such as Generalized Autoregressive Pretraining for Language Understanding (XLNet), Bidirectional Encoder Representation from Transformers (BERT), etc. In our multi-label instruction classification process, we have reckoned our proposed architectures using accuracy and macro f1-score as the performance metrics. This thorough evaluation showed us much about our strategys strengths and drawbacks. Specifically, our implementation of the XLNet architecture has demonstrated unprecedented performance, achieving an accuracy of 97.30% and micro and macro average scores of 89.02% and 93%, a noteworthy accomplishment in multi-label classification. This high level of accuracy and macro average score is a testament to the effectiveness of the XLNet architecture in our proposed InstructNet approach. By employing a multi-level strategy in our evaluation process, we have gained a more comprehensive knowledge of the effectiveness of our proposed architectures and identified areas for forthcoming improvement and refinement.

</details>


### [77] [CTTA-T: Continual Test-Time Adaptation for Text Understanding via Teacher-Student with a Domain-aware and Generalized Teacher](https://arxiv.org/abs/2512.18321)
*Tianlun Liu, Zhiliang Tian, Zhen Huang, Xingzhi Zhou, Wanlong Yu, Tianle Liu, Feng Liu, Dongsheng Li*

**主要类别:** cs.CL

**AI概要:** 提出CTTA-T框架，通过教师-学生架构和增量PCA方法解决文本理解中的持续测试时适应问题，有效减少错误积累并提升跨域泛化能力


<details>
  <summary>更多</summary>
  
**动机:** 文本理解面临域偏移挑战，现有持续测试时适应方法在减少错误积累和增强未观测域泛化能力方面存在不足

**方法:** 采用教师-学生框架，基于dropout一致性进行精炼-过滤策略，通过增量PCA动态积累跨域语义构建域感知教师模型

**结果:** 实验显示CTTA-T在基准测试中表现优异

**结论:** 所提方法有效解决了CTTA中的错误积累和泛化问题，为文本理解的持续域适应提供了实用解决方案

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是CTTA-T%3A+Continual+Test-Time+Adaptation+for+Text+Understanding+via+Teacher-Student+with+a+Domain-aware+and+Generalized+Teacher，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18321，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18321&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Text understanding often suffers from domain shifts. To handle testing domains, domain adaptation (DA) is trained to adapt to a fixed and observed testing domain; a more challenging paradigm, test-time adaptation (TTA), cannot access the testing domain during training and online adapts to the testing samples during testing, where the samples are from a fixed domain. We aim to explore a more practical and underexplored scenario, continual test-time adaptation (CTTA) for text understanding, which involves a sequence of testing (unobserved) domains in testing. Current CTTA methods struggle in reducing error accumulation over domains and enhancing generalization to handle unobserved domains: 1) Noise-filtering reduces accumulated errors but discards useful information, and 2) accumulating historical domains enhances generalization, but it is hard to achieve adaptive accumulation. In this paper, we propose a CTTA-T (continual test-time adaptation for text understanding) framework adaptable to evolving target domains: it adopts a teacher-student framework, where the teacher is domain-aware and generalized for evolving domains. To improve teacher predictions, we propose a refine-then-filter based on dropout-driven consistency, which calibrates predictions and removes unreliable guidance. For the adaptation-generalization trade-off, we construct a domain-aware teacher by dynamically accumulating cross-domain semantics via incremental PCA, which continuously tracks domain shifts. Experiments show CTTA-T excels baselines.

</details>


### [78] [LIR$^3$AG: A Lightweight Rerank Reasoning Strategy Framework for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.18329)
*Guo Chen, Junjie Huang, Huaijin Xie, Fei Sun, Tao Jia*

**主要类别:** cs.CL

**AI概要:** 本文提出LiR³AG框架，让非推理模型通过重构检索证据为连贯推理链来转移推理策略，在RAG多跳QA任务中显著减少98%输出token开销和58.6%推理时间，同时提升8B非推理模型性能超越32B推理模型。


<details>
  <summary>更多</summary>
  
**动机:** 检索增强生成(RAG)通过整合外部知识增强大语言模型，推理模型能提升多跳QA任务性能但带来巨大计算成本。需要理解和缓解这种权衡关系。

**方法:** 对RAG多跳QA任务中的推理策略进行全面研究，发现推理模型采用结构化策略整合检索和内部知识，主要遵循上下文基础推理和知识调和推理两种模式。提出LiR³AG轻量级重排推理策略框架。

**结果:** LiR³AG显著减少平均98%输出token开销和58.6%推理时间，同时将8B非推理模型的F1性能提升6.2%到22.5%，性能超越32B推理模型。

**结论:** LiR³AG为RAG系统提供了实用高效的路径，通过让非推理模型转移推理策略来实现性能提升和计算成本降低的双重目标。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是LIR%24%5E3%24AG%3A+A+Lightweight+Rerank+Reasoning+Strategy+Framework+for+Retrieval-Augmented+Generation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18329，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18329&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Retrieval-Augmented Generation (RAG) effectively enhances Large Language Models (LLMs) by incorporating retrieved external knowledge into the generation process. Reasoning models improve LLM performance in multi-hop QA tasks, which require integrating and reasoning over multiple pieces of evidence across different documents to answer a complex question. However, they often introduce substantial computational costs, including increased token consumption and inference latency. To better understand and mitigate this trade-off, we conduct a comprehensive study of reasoning strategies for reasoning models in RAG multi-hop QA tasks. Our findings reveal that reasoning models adopt structured strategies to integrate retrieved and internal knowledge, primarily following two modes: Context-Grounded Reasoning, which relies directly on retrieved content, and Knowledge-Reconciled Reasoning, which resolves conflicts or gaps using internal knowledge. To this end, we propose a novel Lightweight Rerank Reasoning Strategy Framework for RAG (LiR$^3$AG) to enable non-reasoning models to transfer reasoning strategies by restructuring retrieved evidence into coherent reasoning chains. LiR$^3$AG significantly reduce the average 98% output tokens overhead and 58.6% inferencing time while improving 8B non-reasoning model's F1 performance ranging from 6.2% to 22.5% to surpass the performance of 32B reasoning model in RAG, offering a practical and efficient path forward for RAG systems.

</details>


### [79] [Towards Efficient Agents: A Co-Design of Inference Architecture and System](https://arxiv.org/abs/2512.18337)
*Weizhe Lin, Hui-Ling Zhen, Shuai Yang, Xian Wang, Renxi Liu, Hanting Chen, Wangze Zhang, Chuansai Zhou, Yiming Li, Chen Chen, Xing Li, Zhiyuan Yang, Xiaosong Li, Xianzhi Yu, Zhenhua Dong, Mingxuan Yuan, Yunhe Wang*

**主要类别:** cs.CL

**AI概要:** AgentInfer是一个端到端的智能体加速框架，通过四个协同组件实现推理优化和架构设计，在保持精度的同时减少50%以上的无效token消耗，实现1.8-2.5倍的整体加速。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型智能体在现实部署中面临效率低下的问题，主要源于推理循环、上下文增长和异构工具交互中积累的系统延迟，而非孤立的模型推理。

**方法:** 提出包含四个组件的统一框架：AgentCollab（分层双模型推理框架）、AgentSched（缓存感知混合调度器）、AgentSAM（基于后缀自动机的推测解码方法）、AgentCompress（语义压缩机制），这些组件共同形成自进化引擎。

**结果:** 在BrowseComp-zh和DeepDiver基准测试中，AgentInfer减少了50%以上的无效token消耗，实现1.8-2.5倍的整体加速，同时保持精度不变。

**结论:** 优化智能体任务完成效率（而非单纯的每token吞吐量）是构建可扩展、高效和自我改进智能系统的关键。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Towards+Efficient+Agents%3A+A+Co-Design+of+Inference+Architecture+and+System，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18337，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18337&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The rapid development of large language model (LLM)-based agents has unlocked new possibilities for autonomous multi-turn reasoning and tool-augmented decision-making. However, their real-world deployment is hindered by severe inefficiencies that arise not from isolated model inference, but from the systemic latency accumulated across reasoning loops, context growth, and heterogeneous tool interactions. This paper presents AgentInfer, a unified framework for end-to-end agent acceleration that bridges inference optimization and architectural design. We decompose the problem into four synergistic components: AgentCollab, a hierarchical dual-model reasoning framework that balances large- and small-model usage through dynamic role assignment; AgentSched, a cache-aware hybrid scheduler that minimizes latency under heterogeneous request patterns; AgentSAM, a suffix-automaton-based speculative decoding method that reuses multi-session semantic memory to achieve low-overhead inference acceleration; and AgentCompress, a semantic compression mechanism that asynchronously distills and reorganizes agent memory without disrupting ongoing reasoning. Together, these modules form a Self-Evolution Engine capable of sustaining efficiency and cognitive stability throughout long-horizon reasoning tasks. Experiments on the BrowseComp-zh and DeepDiver benchmarks demonstrate that through the synergistic collaboration of these methods, AgentInfer reduces ineffective token consumption by over 50%, achieving an overall 1.8-2.5 times speedup with preserved accuracy. These results underscore that optimizing for agentic task completion-rather than merely per-token throughput-is the key to building scalable, efficient, and self-improving intelligent systems.

</details>


### [80] [LLM-based Few-Shot Early Rumor Detection with Imitation Agent](https://arxiv.org/abs/2512.18352)
*Fengzhu Zeng, Qian Shao, Ling Cheng, Wei Gao, Shih-Fen Cheng, Jing Ma, Cheng Niu*

**主要类别:** cs.CL

**AI概要:** 提出了一种结合自主代理和LLM的早期谣言检测框架，代理负责确定早期时间点，LLM作为谣言检测器，在少样本设置下实现高效检测。


<details>
  <summary>更多</summary>
  
**动机:** 早期谣言检测在数据稀缺环境下具有挑战性，LLM虽然擅长少样本NLP任务但不适合时间序列数据且计算成本高。

**方法:** 使用自主代理作为早期时间点决策器，LLM作为训练免费的谣言检测器，只需训练轻量级代理。

**结果:** 在四个真实数据集上的实验表明，该方法提升了LLM的性能，在准确性和早期性方面超越了现有EARD方法。

**结论:** 该框架为少样本早期谣言检测提供了首个解决方案，实现了计算效率和检测性能的平衡。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是LLM-based+Few-Shot+Early+Rumor+Detection+with+Imitation+Agent，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18352，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18352&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Early Rumor Detection (EARD) aims to identify the earliest point at which a claim can be accurately classified based on a sequence of social media posts. This is especially challenging in data-scarce settings. While Large Language Models (LLMs) perform well in few-shot NLP tasks, they are not well-suited for time-series data and are computationally expensive for both training and inference. In this work, we propose a novel EARD framework that combines an autonomous agent and an LLM-based detection model, where the agent acts as a reliable decision-maker for \textit{early time point determination}, while the LLM serves as a powerful \textit{rumor detector}. This approach offers the first solution for few-shot EARD, necessitating only the training of a lightweight agent and allowing the LLM to remain training-free. Extensive experiments on four real-world datasets show our approach boosts performance across LLMs and surpasses existing EARD methods in accuracy and earliness.

</details>


### [81] [DACE For Railway Acronym Disambiguation](https://arxiv.org/abs/2512.18357)
*El Mokhtar Hribach, Oussama Mechhour, Mohammed Elmonstaser, Yassine El Boudouri, Othmane Kabal*

**主要类别:** cs.CL

**AI概要:** DACE框架通过动态提示、检索增强生成、上下文选择和集成聚合技术，在法语铁路文档的首字母缩略词消歧任务中取得最佳性能，F1分数达0.9069


<details>
  <summary>更多</summary>
  
**动机:** 技术文本处理中的首字母缩略词消歧是一个基础性挑战，特别是在专业领域的高歧义环境下，需要解决自动化分析的复杂性

**方法:** 提出DACE框架，结合动态提示、检索增强生成、上下文选择和集成聚合技术，通过自适应上下文学习和外部领域知识注入来增强大语言模型

**结果:** 在TextMine'26竞赛中排名第一，F1分数达到0.9069，有效缓解了幻觉问题并处理了低资源场景

**结论:** DACE框架通过集成多种技术有效解决了专业领域中的首字母缩略词消歧问题，证明了动态提示和知识注入在提升模型性能方面的重要性

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是DACE+For+Railway+Acronym+Disambiguation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18357，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18357&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Acronym Disambiguation (AD) is a fundamental challenge in technical text processing, particularly in specialized sectors where high ambiguity complicates automated analysis. This paper addresses AD within the context of the TextMine'26 competition on French railway documentation. We present DACE (Dynamic Prompting, Retrieval Augmented Generation, Contextual Selection, and Ensemble Aggregation), a framework that enhances Large Language Models through adaptive in-context learning and external domain knowledge injection. By dynamically tailoring prompts to acronym ambiguity and aggregating ensemble predictions, DACE mitigates hallucination and effectively handles low-resource scenarios. Our approach secured the top rank in the competition with an F1 score of 0.9069.

</details>


### [82] [LLM Agents Implement an NLG System from Scratch: Building Interpretable Rule-Based RDF-to-Text Generators](https://arxiv.org/abs/2512.18360)
*Mateusz Lango, Ondřej Dušek*

**主要类别:** cs.CL

**AI概要:** 提出了一种新型神经符号框架用于RDF到文本生成，通过多个LLM代理的协作交互而非传统反向传播来训练模型，生成基于RDF三元组的规则式Python代码生成器，无需领域内人工参考文本。


<details>
  <summary>更多</summary>
  
**动机:** 解决传统RDF-to-text生成方法需要大量监督训练数据、缺乏可解释性以及存在幻觉问题，旨在开发完全可解释、无需训练数据且能快速生成文本的方法。

**方法:** 使用多个LLM代理协作交互生成基于规则的Python代码生成器，仅基于RDF三元组，不依赖领域内人工参考文本，系统完全可解释且仅需单CPU运行。

**结果:** 在WebNLG和OpenDialKG数据集上的实验表明，该方法减少了幻觉现象，与微调或提示语言模型相比仅产生轻微的流畅性损失。

**结论:** 该神经符号框架提供了一种无需监督训练数据、完全可解释且高效的RDF-to-text生成方案，在减少幻觉的同时保持了良好的文本生成质量。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是LLM+Agents+Implement+an+NLG+System+from+Scratch%3A+Building+Interpretable+Rule-Based+RDF-to-Text+Generators，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18360，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18360&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present a novel neurosymbolic framework for RDF-to-text generation, in which the model is "trained" through collaborative interactions among multiple LLM agents rather than traditional backpropagation. The LLM agents produce rule-based Python code for a generator for the given domain, based on RDF triples only, with no in-domain human reference texts. The resulting system is fully interpretable, requires no supervised training data, and generates text nearly instantaneously using only a single CPU. Our experiments on the WebNLG and OpenDialKG data show that outputs produced by our approach reduce hallucination, with only slight fluency penalties compared to finetuned or prompted language models

</details>


### [83] [SRS-Stories: Vocabulary-constrained multilingual story generation for language learning](https://arxiv.org/abs/2512.18362)
*Wiktor Kamzela, Mateusz Lango, Ondrej Dusek*

**主要类别:** cs.CL

**AI概要:** 使用大语言模型为语言学习者生成个性化故事，仅使用学习者已知词汇，通过上下文语境教授新词汇并复习已学词汇，结合间隔重复系统优化学习效果。


<details>
  <summary>更多</summary>
  
**动机:** 为语言学习者提供个性化阅读材料，通过上下文语境自然习得新词汇，同时有效复习已学词汇，解决传统语言学习材料缺乏个性化和语境化的问题。

**方法:** 在英语、中文和波兰语三种语言中测试三种故事生成方法和三种词汇约束策略，使用间隔重复系统优化词汇复习，评估生成文本的质量。

**结果:** 生成的故事比标准约束束搜索方法生成的文本更具语法正确性、连贯性，并提供更好的词汇使用示例。

**结论:** 该方法能有效生成高质量个性化语言学习材料，通过语境化方式促进词汇学习，优于传统约束生成方法。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是SRS-Stories%3A+Vocabulary-constrained+multilingual+story+generation+for+language+learning，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18362，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18362&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In this paper, we use large language models to generate personalized stories for language learners, using only the vocabulary they know. The generated texts are specifically written to teach the user new vocabulary by simply reading stories where it appears in context, while at the same time seamlessly reviewing recently learned vocabulary. The generated stories are enjoyable to read and the vocabulary reviewing/learning is optimized by a Spaced Repetition System. The experiments are conducted in three languages: English, Chinese and Polish, evaluating three story generation methods and three strategies for enforcing lexical constraints. The results show that the generated stories are more grammatical, coherent, and provide better examples of word usage than texts generated by the standard constrained beam search approach

</details>


### [84] [AraToken: Optimizing Arabic Tokenization with Normalization Pipeline and Language Extension for Qwen3](https://arxiv.org/abs/2512.18399)
*Mark Kashirskiy, Artiom Lipinski, Ilya Makarov*

**主要类别:** cs.CL

**AI概要:** AraToken是一个针对阿拉伯语优化的分词器，基于SentencePiece Unigram算法，通过标准化处理阿拉伯语特有的正字法变体，显著提高了分词效率，并在Qwen3-0.6B模型中通过词汇扩展方法实现了快速性能提升。


<details>
  <summary>更多</summary>
  
**动机:** 现有的通用分词器主要针对英语和拉丁语系语言开发，在处理形态丰富的阿拉伯语时表现不佳，导致分词序列过长和压缩效率降低。

**方法:** 基于SentencePiece Unigram算法构建阿拉伯语优化分词器，包含全面的标准化流程处理Alif变体、变音符号和阿拉伯-印度数字；引入语言扩展管道(LEP)方法，通过词汇扩展、平均子词初始化和选择性Transformer层解冻来集成优化分词器。

**结果:** SentencePiece加标准化相比未标准化基线降低了18%的fertility(1.199 vs 1.35 tokens/word)；LEP方法在10万阿拉伯语样本上仅用800训练步就将评估损失从8.28降至2.43。

**结论:** AraToken显著改善了阿拉伯语的分词效率，LEP方法有效实现了优化分词器与现有模型的快速集成，为阿拉伯语NLP研究提供了重要工具和资源。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是AraToken%3A+Optimizing+Arabic+Tokenization+with+Normalization+Pipeline+and+Language+Extension+for+Qwen3，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18399，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18399&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Tokenization is a critical preprocessing step for large language models (LLMs), directly impacting training efficiency and downstream performance. General-purpose tokenizers trained predominantly on English and Latin-script languages exhibit suboptimal performance on morphologically rich languages such as Arabic, resulting in inflated token sequences and reduced compression efficiency. In this work, we present AraToken, an Arabic-optimized tokenizer built on SentencePiece Unigram algorithm with a comprehensive normalization pipeline addressing Arabic-specific orthographic variations including Alif variants, diacritics, and Arabic-Indic numerals. We systematically compare BPE, WordPiece, and SentencePiece algorithms across multiple configurations, demonstrating that SentencePiece with normalization achieves 18% lower fertility (1.199 vs 1.35 tokens/word) compared to unnormalized baselines. Furthermore, we introduce the Language Extension Pipeline (LEP), a method for integrating the optimized tokenizer into Qwen3-0.6B through vocabulary extension with mean subtoken initialization and selective transformer layer unfreezing. Our experiments show that LEP reduces evaluation loss from 8.28 to 2.43 within 800 training steps on 100K Arabic samples. We release our tokenizer, training scripts, and model checkpoints to facilitate Arabic NLP research.

</details>


### [85] [An Agentic AI Framework for Training General Practitioner Student Skills](https://arxiv.org/abs/2512.18440)
*Victor De Marez, Jens Van Nooten, Luna De Bruyne, Walter Daelemans*

**主要类别:** cs.CL

**AI概要:** 论文提出了一个基于大语言模型的智能代理框架，用于训练全科医学生的临床技能，通过可配置的病例生成、角色扮演对话和标准化评估反馈，有效解决了虚拟模拟患者在医学教育中的准确性和一致性问题。


<details>
  <summary>更多</summary>
  
**动机:** 当前虚拟模拟患者(VSPs)在医学教育中存在医疗准确性不足、角色扮演不一致、场景生成困难和缺乏结构化反馈等问题，需要开发更可靠的教学工具。

**方法:** 提出了一个三合一代理框架：(1)可配置的循证病例生成；(2)基于角色的患者对话控制，支持检索增强；(3)基于标准的沟通和临床推理评估反馈系统。在交互式口语咨询环境中实现并进行了医学学生(N=14)评估。

**结果:** 参与者报告了真实且符合病例的对话、适当的难度校准、稳定的人格特征表现，以及富含实例的实用反馈，整体可用性优秀。

**结论:** 通过将场景控制、交互控制和标准化评估分离的代理架构，是构建可靠且具有教学价值的虚拟模拟患者训练工具的有效模式。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是An+Agentic+AI+Framework+for+Training+General+Practitioner+Student+Skills，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18440，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18440&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Advancements in large language models offer strong potential for enhancing virtual simulated patients (VSPs) in medical education by providing scalable alternatives to resource-intensive traditional methods. However, current VSPs often struggle with medical accuracy, consistent roleplaying, scenario generation for VSP use, and educationally structured feedback. We introduce an agentic framework for training general practitioner student skills that unifies (i) configurable, evidence-based vignette generation, (ii) controlled persona-driven patient dialogue with optional retrieval grounding, and (iii) standards-based assessment and feedback for both communication and clinical reasoning. We instantiate the framework in an interactive spoken consultation setting and evaluate it with medical students ($\mathbf{N{=}14}$). Participants reported realistic and vignette-faithful dialogue, appropriate difficulty calibration, a stable personality signal, and highly useful example-rich feedback, alongside excellent overall usability. These results support agentic separation of scenario control, interaction control, and standards-based assessment as a practical pattern for building dependable and pedagogically valuable VSP training tools.

</details>


### [86] [Mitigating Spurious Correlations in NLI via LLM-Synthesized Counterfactuals and Dynamic Balanced Sampling](https://arxiv.org/abs/2512.18462)
*Christopher Román Jaimes*

**主要类别:** cs.CL

**AI概要:** 提出自动化pipeline解决NLI模型依赖虚假相关性问题，包含LF-LMI检测语义伪影、LLM合成对比集、动态平衡采样训练策略，显著提升模型一致性表现


<details>
  <summary>更多</summary>
  
**动机:** NLI模型经常依赖虚假相关性而非语义推理，现有缓解策略标注成本高或导致灾难性遗忘

**方法:** 三阶段方法：1) LF-LMI检测语义伪影 2) LLM合成高质量对比集并进行多评委验证 3) 动态平衡采样训练策略防止遗忘

**结果:** 在挑战性基准上一致性从63.5%提升至81.0%，同时保持88.4%的域内准确率，显著优于简单微调

**结论:** 提出的自动化pipeline有效解决了NLI模型的虚假相关性问题和灾难性遗忘，实现了更好的泛化性能

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Mitigating+Spurious+Correlations+in+NLI+via+LLM-Synthesized+Counterfactuals+and+Dynamic+Balanced+Sampling，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18462，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18462&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Natural Language Inference (NLI) models frequently rely on spurious correlations rather than semantic reasoning. Existing mitigation strategies often incur high annotation costs or trigger catastrophic forgetting during fine-tuning. We propose an automated, scalable pipeline to address these limitations. First, we introduce Log-Frequency LMI (LF-LMI) to accurately detect semantic artifacts. Second, we generate a high-quality synthetic contrast set via an LLM-synthesis pipeline with multi-judge verification. Finally, we introduce Dynamic Balanced Sampling, a training strategy that rotates the original data distribution to prevent forgetting. Our method improves consistency on a challenging benchmark from 63.5% to 81.0% while maintaining 88.4% in-domain accuracy, significantly outperforming naive fine-tuning.

</details>


### [87] [Research on a hybrid LSTM-CNN-Attention model for text-based web content classification](https://arxiv.org/abs/2512.18475)
*Mykola Kuz, Ihor Lazarovych, Mykola Kozlenko, Mykola Pikuliak, Andrii Kvasniuk*

**主要类别:** cs.CL

**AI概要:** 提出了一种结合LSTM、CNN和注意力机制的混合深度学习架构，用于网络文本内容分类，在准确率、精确率、召回率和F1分数上均优于单一模型和BERT等基线方法。


<details>
  <summary>更多</summary>
  
**动机:** 解决单一神经网络架构在文本分类中的局限性，需要同时捕获细粒度文本结构和更广泛的语义上下文，提高分类性能。

**方法:** 使用预训练的GloVe词嵌入表示文本，CNN层提取局部n-gram模式和词汇特征，LSTM层建模长距离依赖和序列结构，注意力机制选择性地关注输入序列中最具信息量的部分。采用5折交叉验证评估模型。

**结果:** 混合模型取得了优异性能：准确率0.98、精确率0.94、召回率0.92、F1分数0.93，超越了仅使用CNN、LSTM或BERT等基线模型的性能。

**结论:** 该混合架构在文本分类中表现出高效性，特别适合需要句法特征提取和语义解释的任务，支持混合深度学习在NLP应用中的更广泛使用。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Research+on+a+hybrid+LSTM-CNN-Attention+model+for+text-based+web+content+classification，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18475，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18475&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This study presents a hybrid deep learning architecture that integrates LSTM, CNN, and an Attention mechanism to enhance the classification of web content based on text. Pretrained GloVe embeddings are used to represent words as dense vectors that preserve semantic similarity. The CNN layer extracts local n-gram patterns and lexical features, while the LSTM layer models long-range dependencies and sequential structure. The integrated Attention mechanism enables the model to focus selectively on the most informative parts of the input sequence. A 5-fold cross-validation setup was used to assess the robustness and generalizability of the proposed solution. Experimental results show that the hybrid LSTM-CNN-Attention model achieved outstanding performance, with an accuracy of 0.98, precision of 0.94, recall of 0.92, and F1-score of 0.93. These results surpass the performance of baseline models based solely on CNNs, LSTMs, or transformer-based classifiers such as BERT. The combination of neural network components enabled the model to effectively capture both fine-grained text structures and broader semantic context. Furthermore, the use of GloVe embeddings provided an efficient and effective representation of textual data, making the model suitable for integration into systems with real-time or near-real-time requirements. The proposed hybrid architecture demonstrates high effectiveness in text-based web content classification, particularly in tasks requiring both syntactic feature extraction and semantic interpretation. By combining presented mechanisms, the model addresses the limitations of individual architectures and achieves improved generalization. These findings support the broader use of hybrid deep learning approaches in NLP applications, especially where complex, unstructured textual data must be processed and classified with high reliability.

</details>


### [88] [Teaching and Critiquing Conceptualization and Operationalization in NLP](https://arxiv.org/abs/2512.18505)
*Vagrant Gautam*

**主要类别:** cs.CL

**AI概要:** 该论文探讨了NLP领域中抽象概念缺乏明确定义的问题，作者创建了一个研讨课程让学生探索概念化和操作化问题


<details>
  <summary>更多</summary>
  
**动机:** NLP研究者经常使用"可解释性"、"偏见"、"推理"等抽象概念而不加以定义，各子领域对这些术语有不同理解，这影响了数据集构建、指标制定和系统评估

**方法:** 作者设计了一个跨学科的研讨课程，通过阅读清单和重点讨论与批判的方式，让学生深入探索这些概念的含义和测量方法

**结果:** 论文提出了一个教学框架来系统性地解决NLP中概念定义和操作化的问题，但没有提供具体的实证研究结果

**结论:** 需要更严谨地对待NLP中的抽象概念，通过跨学科讨论和批判性思维来建立更清晰的概念定义和测量方法

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Teaching+and+Critiquing+Conceptualization+and+Operationalization+in+NLP，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18505，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18505&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** NLP researchers regularly invoke abstract concepts like "interpretability," "bias," "reasoning," and "stereotypes," without defining them. Each subfield has a shared understanding or conceptualization of what these terms mean and how we should treat them, and this shared understanding is the basis on which operational decisions are made: Datasets are built to evaluate these concepts, metrics are proposed to quantify them, and claims are made about systems. But what do they mean, what should they mean, and how should we measure them? I outline a seminar I created for students to explore these questions of conceptualization and operationalization, with an interdisciplinary reading list and an emphasis on discussion and critique.

</details>


### [89] [Generalization Gaps in Political Fake News Detection: An Empirical Study on the LIAR Dataset](https://arxiv.org/abs/2512.18533)
*S Mahmudul Hasan, Shaily Roy, Akib Jawad Nafis*

**主要类别:** cs.CL

**AI概要:** 该研究发现文本语言模型在政治事实核查中存在性能天花板，简单线性模型与复杂Transformer性能相当，表明模型容量不是主要瓶颈，需要外部知识而非单纯增加模型复杂度。


<details>
  <summary>更多</summary>
  
**动机:** 政治虚假信息日益语言微妙，对自动化事实核查系统构成挑战，但文本语言建模的实证限制尚未充分探索。

**方法:** 在LIAR基准上系统评估9种机器学习算法，分离词汇特征（词袋、TF-IDF）和语义嵌入（GloVe），分析不同模型性能。

**结果:** 发现性能上限（加权F1得分不超过0.32），线性SVM（准确率0.624）与RoBERTa（0.620）性能相当，树模型存在巨大泛化差距（训练准确率>99%，测试仅25%），SMOTE数据增强无效。

**结论:** 政治事实核查中，仅增加模型复杂度而不整合外部知识收效甚微，语义特征模糊性是主要限制因素。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Generalization+Gaps+in+Political+Fake+News+Detection%3A+An+Empirical+Study+on+the+LIAR+Dataset，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18533，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18533&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The proliferation of linguistically subtle political disinformation poses a significant challenge to automated fact-checking systems. Despite increasing emphasis on complex neural architectures, the empirical limits of text-only linguistic modeling remain underexplored. We present a systematic diagnostic evaluation of nine machine learning algorithms on the LIAR benchmark. By isolating lexical features (Bag-of-Words, TF-IDF) and semantic embeddings (GloVe), we uncover a hard "Performance Ceiling", with fine-grained classification not exceeding a Weighted F1-score of 0.32 across models. Crucially, a simple linear SVM (Accuracy: 0.624) matches the performance of pre-trained Transformers such as RoBERTa (Accuracy: 0.620), suggesting that model capacity is not the primary bottleneck. We further diagnose a massive "Generalization Gap" in tree-based ensembles, which achieve more than 99% training accuracy but collapse to approximately 25% on test data, indicating reliance on lexical memorization rather than semantic inference. Synthetic data augmentation via SMOTE yields no meaningful gains, confirming that the limitation is semantic (feature ambiguity) rather than distributional. These findings indicate that for political fact-checking, increasing model complexity without incorporating external knowledge yields diminishing returns.

</details>


### [90] [LLMs on Drugs: Language Models Are Few-Shot Consumers](https://arxiv.org/abs/2512.18546)
*Alexander Doudkin*

**主要类别:** cs.CL

**AI概要:** 研究发现，在LLM推理时使用精神活性药物相关的提示词会显著降低模型性能，酒精提示使准确率从45%降至10%，证明单句人物设定提示可以破坏模型可靠性


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型对推理时的人物设定敏感，但之前从未对提示级别的"药物"干预进行严格基准测试

**方法:** 使用GPT-5-mini和ARC-Challenge数据集，比较LSD、可卡因、酒精和大麻四种单句提示与清醒对照组的性能，每个条件100个验证项，使用确定性解码和统计检验

**结果:** 对照组准确率0.45；酒精降至0.10(p=3.2e-8)，可卡因0.21(p=4.9e-4)，LSD 0.19(p=1.3e-4)，大麻0.30(p=0.041)，主要原因是人物设定提示破坏了标准答案模板

**结论:** 人物设定文本类似于"少样本消耗品"，可以在不修改模型权重的情况下破坏模型可靠性，这对外部提示的安全性具有重要意义

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是LLMs+on+Drugs%3A+Language+Models+Are+Few-Shot+Consumers，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18546，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18546&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) are sensitive to the personas imposed on them at inference time, yet prompt-level "drug" interventions have never been benchmarked rigorously. We present the first controlled study of psychoactive framings on GPT-5-mini using ARC-Challenge. Four single-sentence prompts -- LSD, cocaine, alcohol, and cannabis -- are compared against a sober control across 100 validation items per condition, with deterministic decoding, full logging, Wilson confidence intervals, and Fisher exact tests. Control accuracy is 0.45; alcohol collapses to 0.10 (p = 3.2e-8), cocaine to 0.21 (p = 4.9e-4), LSD to 0.19 (p = 1.3e-4), and cannabis to 0.30 (p = 0.041), largely because persona prompts disrupt the mandated "Answer: <LETTER>" template. Persona text therefore behaves like a "few-shot consumable" that can destroy reliability without touching model weights. All experimental code, raw results, and analysis scripts are available at https://github.com/lexdoudkin/llms-on-drugs.

</details>


### [91] [Neologism Learning as a Parameter-Efficient Alternative to Fine-Tuning for Model Steering](https://arxiv.org/abs/2512.18551)
*Sungjoon Park, Varun Ramamurthi, Owen Terry*

**主要类别:** cs.CL

**AI概要:** 本文比较了在语言模型中使用新词学习（neologism learning）与LoRA微调的性能，发现新词学习在相同训练设置下表现更优，且模型会自发创造新词。


<details>
  <summary>更多</summary>
  
**动机:** 研究如何通过新词学习来实现模型行为引导，相比传统微调方法更高效灵活，只需训练少量参数即可实现特定行为控制。

**方法:** 比较新词学习与LoRA微调在相同数据和超参数设置下的性能表现，并研究模型对新词的自我表达行为。

**结果:** 新词学习在匹配训练设置下优于微调模型，模型在被询问新词时会自发创造新词汇。

**结论:** 新词学习是一种计算效率高、灵活性强的模型行为引导方法，为语言模型的可控生成提供了新途径。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Neologism+Learning+as+a+Parameter-Efficient+Alternative+to+Fine-Tuning+for+Model+Steering，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18551，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18551&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In language modeling, neologisms are new tokens trained to represent a concept not already included in a given model's vocabulary. Neologisms can be used to encourage specific behavior in models, for example by appending prompts with "Give me a neologism answer." Behavioral steering can also be achieved through fine-tuning, albeit with more compute and less flexibility: learning a neologism only trains d parameters and allows the user to still access the model's default behavior. We compare the performance of neologism learning against low-rank adaptation (LoRA) fine-tuning, finding that neologisms outperform fine-tuned models under a matched training setup (same data and hyperparameters). We also investigate self-verbalizations of neologisms, and observe that the model will occasionally make up its own new words when asked about a neologism.

</details>


### [92] [From Scratch to Fine-Tuned: A Comparative Study of Transformer Training Strategies for Legal Machine Translation](https://arxiv.org/abs/2512.18593)
*Amit Barman, Atanu Mandal, Sudip Kumar Naskar*

**主要类别:** cs.CL

**AI概要:** 该论文针对印度多语言环境下的法律翻译问题，通过微调预训练OPUS-MT模型和从头训练Transformer模型两种方法，在英印地法律文本翻译任务中取得了显著效果，证明了领域适应对提升翻译质量的重要性。


<details>
  <summary>更多</summary>
  
**动机:** 解决印度等多语言国家中因法律文档主要使用英语而造成的语言障碍问题，通过法律机器翻译技术提高法律信息的可及性和司法透明度。

**方法:** 采用两种互补策略：1) 对预训练的OPUS-MT模型进行领域特定的微调；2) 使用提供的法律语料库从头开始训练Transformer模型。使用SacreBLEU、chrF++、TER、ROUGE、BERTScore、METEOR和COMET等多种标准机器翻译指标进行评估。

**结果:** 微调后的OPUS-MT模型获得46.03的SacreBLEU分数，显著优于基线模型和从头训练的模型。

**结论:** 领域适应在提升翻译质量方面非常有效，法律机器翻译系统在多语言环境下具有改善司法可及性和法律透明度的巨大潜力。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是From+Scratch+to+Fine-Tuned%3A+A+Comparative+Study+of+Transformer+Training+Strategies+for+Legal+Machine+Translation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18593，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18593&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In multilingual nations like India, access to legal information is often hindered by language barriers, as much of the legal and judicial documentation remains in English. Legal Machine Translation (L-MT) offers a scalable solution to this challenge by enabling accurate and accessible translations of legal documents. This paper presents our work for the JUST-NLP 2025 Legal MT shared task, focusing on English-Hindi translation using Transformer-based approaches. We experiment with 2 complementary strategies, fine-tuning a pre-trained OPUS-MT model for domain-specific adaptation and training a Transformer model from scratch using the provided legal corpus. Performance is evaluated using standard MT metrics, including SacreBLEU, chrF++, TER, ROUGE, BERTScore, METEOR, and COMET. Our fine-tuned OPUS-MT model achieves a SacreBLEU score of 46.03, significantly outperforming both baseline and from-scratch models. The results highlight the effectiveness of domain adaptation in enhancing translation quality and demonstrate the potential of L-MT systems to improve access to justice and legal transparency in multilingual contexts.

</details>


### [93] [On Finding Inconsistencies in Documents](https://arxiv.org/abs/2512.18601)
*Charles J. Lovering, Seth Ebner, Brandon Smock, Michael Krumdick, Saad Rabbani, Ahmed Muhammad, Varshini Reddy, Chris Tanner*

**主要类别:** cs.CL

**AI概要:** 论文介绍了FIND基准测试，用于评估语言模型在长而复杂的文档中发现不一致性的能力。GPT-5表现最佳，能发现64%的人工插入不一致性，还能找到原始文档中被作者忽略的真实不一致性。


<details>
  <summary>更多</summary>
  
**动机:** 学术界、法律和金融领域的专业人士需要审核文档，因为不一致性可能导致经济、声誉和科学成本。语言模型有潜力大幅加速这一审核过程。

**方法:** 引入FIND基准测试，每个示例都是由领域专家手动插入不一致性的文档。这些文档具有长、技术性强和复杂的特点。

**结果:** 最佳模型GPT-5恢复了64%的人工插入不一致性，并在50篇arXiv论文中发现了136个被原作者忽略的合法不一致性（共196个建议）。

**结论:** 尽管语言模型在发现文档不一致性方面表现出色，但即使最佳模型仍遗漏了近一半的不一致性，表明不一致性检测仍然是一个具有挑战性的任务。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是On+Finding+Inconsistencies+in+Documents，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18601，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18601&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Professionals in academia, law, and finance audit their documents because inconsistencies can result in monetary, reputational, and scientific costs. Language models (LMs) have the potential to dramatically speed up this auditing process. To understand their abilities, we introduce a benchmark, FIND (Finding INconsistencies in Documents), where each example is a document with an inconsistency inserted manually by a domain expert. Despite the documents being long, technical, and complex, the best-performing model (gpt-5) recovered 64% of the inserted inconsistencies. Surprisingly, gpt-5 also found undiscovered inconsistencies present in the original documents. For example, on 50 arXiv papers, we judged 136 out of 196 of the model's suggestions to be legitimate inconsistencies missed by the original authors. However, despite these findings, even the best models miss almost half of the inconsistencies in FIND, demonstrating that inconsistency detection is still a challenging task.

</details>


### [94] [A Comparative Study of Light-weight Language Models for PII Masking and their Deployment for Real Conversational Texts](https://arxiv.org/abs/2512.18608)
*Prabigya Acharya, Liza Shrestha*

**主要类别:** cs.CL

**AI概要:** 轻量级模型（T5-small和Mistral-Instruct-v0.3）在PII脱敏任务上可以达到与前沿大语言模型相当的性能，但存在准确率、鲁棒性和计算效率之间的权衡。


<details>
  <summary>更多</summary>
  
**动机:** 当前前沿大语言模型虽然具有强大的PII脱敏能力，但存在数据处理和计算成本方面的担忧，需要探索轻量级模型是否能达到可比性能。

**方法:** 在AI4Privacy基准构建的英文数据集上微调编码器-解码器（T5-small）和解码器专用（Mistral-Instruct-v0.3）架构，创建不同数据集变体研究标签标准化和PII表示，涵盖24个标准化PII类别和更高粒度设置。

**结果:** 两种轻量级模型在PII脱敏任务上都达到了与前沿LLMs相当的性能。标签标准化在所有架构中一致提升性能。Mistral获得更高的F1和召回率，但对不同PII类型更具鲁棒性，但生成延迟显著更高。T5在对话文本中鲁棒性较差，但提供更可控的结构化输出和更低推理成本。

**结论:** 轻量级模型可以提供有效的PII脱敏，同时解决与前沿大语言模型相关的数据处理问题，但在非正式输入下性能会下降，需要在准确性、鲁棒性和计算效率之间进行权衡。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是A+Comparative+Study+of+Light-weight+Language+Models+for+PII+Masking+and+their+Deployment+for+Real+Conversational+Texts，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18608，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18608&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Automated masking of Personally Identifiable Information (PII) is critical for privacy-preserving conversational systems. While current frontier large language models demonstrate strong PII masking capabilities, concerns about data handling and computational costs motivate exploration of whether lightweight models can achieve comparable performance. We compare encoder-decoder and decoder-only architectures by fine-tuning T5-small and Mistral-Instruct-v0.3 on English datasets constructed from the AI4Privacy benchmark. We create different dataset variants to study label standardization and PII representation, covering 24 standardized PII categories and higher-granularity settings. Evaluation using entity-level and character-level metrics, type accuracy, and exact match shows that both lightweight models achieve performance comparable to frontier LLMs for PII masking tasks. Label normalization consistently improves performance across architectures. Mistral achieves higher F1 and recall with greater robustness across PII types but incurs significantly higher generation latency. T5, while less robust in conversational text, offers more controllable structured outputs and lower inference cost, motivating its use in a real-time Discord bot for real-world PII redaction. Evaluation on live messages reveals performance degradation under informal inputs. These results clarify trade-offs between accuracy, robustness, and computational efficiency, demonstrating that lightweight models can provide effective PII masking while addressing data handling concerns associated with frontier LLMs.

</details>


### [95] [LLM-CAS: Dynamic Neuron Perturbation for Real-Time Hallucination Correction](https://arxiv.org/abs/2512.18623)
*Jensen Zhang, Ningyuan Liu, Yijia Fan, Zihao Huang, Qinglin Zeng, Kaitong Cai, Jian Wang, Keze Wang*

**主要类别:** cs.CL

**AI概要:** LLM-CAS是一个基于分层强化学习的实时幻觉校正框架，通过在推理过程中动态选择神经元扰动来纠正大语言模型的幻觉问题，无需永久修改模型参数。


<details>
  <summary>更多</summary>
  
**动机:** 现有方法如监督微调和人类反馈强化学习数据密集且计算昂贵，静态参数编辑方法难以处理上下文相关错误和灾难性遗忘问题。

**方法:** 将实时幻觉校正构建为分层强化学习问题，训练智能体学习策略，在推理时基于当前上下文动态选择临时神经元扰动。

**结果:** 在多个基准测试中显著提升事实准确性：StoryCloze提升10.98个百分点，TriviaQA提升2.71个百分点，TruthfulQA的MC1分数提升2.06个百分点，优于ITI、CAA和SADI等方法。

**结论:** LLM-CAS提供了一个高效且上下文感知的解决方案，能提高大语言模型的可靠性，并具有未来多模态扩展的潜力。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是LLM-CAS%3A+Dynamic+Neuron+Perturbation+for+Real-Time+Hallucination+Correction，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18623，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18623&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) often generate hallucinated content that lacks factual or contextual grounding, limiting their reliability in critical applications. Existing approaches such as supervised fine-tuning and reinforcement learning from human feedback are data intensive and computationally expensive, while static parameter editing methods struggle with context dependent errors and catastrophic forgetting.
  We propose LLM-CAS, a framework that formulates real-time hallucination correction as a hierarchical reinforcement learning problem. LLM-CAS trains an agent to learn a policy that dynamically selects temporary neuron perturbations during inference based on the current context. Unlike prior dynamic approaches that rely on heuristic or predefined adjustments, this policy driven mechanism enables adaptive and fine grained correction without permanent parameter modification.
  Experiments across multiple language models demonstrate that LLM-CAS consistently improves factual accuracy, achieving gains of 10.98 percentage points on StoryCloze, 2.71 points on TriviaQA, and 2.06 points on the MC1 score of TruthfulQA. These results outperform both static editing methods such as ITI and CAA and the dynamic SADI framework. Overall, LLM-CAS provides an efficient and context aware solution for improving the reliability of LLMs, with promising potential for future multimodal extensions.

</details>


### [96] [Does It Tie Out? Towards Autonomous Legal Agents in Venture Capital](https://arxiv.org/abs/2512.18658)
*Pierre Colombo, Malik Boudiaf, Allyn Sweet, Michael Desa, Hongxi Wang, Kevin Candra, Syméon del Marmol*

**主要类别:** cs.CL

**AI概要:** 该论文将风险投资融资轮次前的资本表核对作为法律AI的现实基准测试，分析了现有智能代理系统的性能，并提出了一种世界模型架构来实现自动化资本表核对。


<details>
  <summary>更多</summary>
  
**动机:** 当前LLM在法律基准测试中表现持续改进，但资本表核对等专业法律工作流程需要多文档推理、严格证据追溯和确定性输出，现有方法无法可靠实现。

**方法:** 将资本表核对作为法律AI的现实基准进行特征化，分析比较现有智能代理系统的性能，提出一种世界模型架构来实现自动化。

**结果:** 研究发现现有智能代理系统无法可靠完成资本表核对任务，需要新的架构来解决多文档推理和证据追溯的挑战。

**结论:** 提出的世界模型架构不仅可用于自动化资本表核对，还可作为应用法律智能的更广泛基础，为专业法律工作流程的自动化提供解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Does+It+Tie+Out%3F+Towards+Autonomous+Legal+Agents+in+Venture+Capital，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18658，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18658&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Before closing venture capital financing rounds, lawyers conduct diligence that includes tying out the capitalization table: verifying that every security (for example, shares, options, warrants) and issuance term (for example, vesting schedules, acceleration triggers, transfer restrictions) is supported by large sets of underlying legal documentation. While LLMs continue to improve on legal benchmarks, specialized legal workflows, such as capitalization tie-out, remain out of reach even for strong agentic systems. The task requires multi-document reasoning, strict evidence traceability, and deterministic outputs that current approaches fail to reliably deliver. We characterize capitalization tie-out as an instance of a real-world benchmark for legal AI, analyze and compare the performance of existing agentic systems, and propose a world model architecture toward tie-out automation-and more broadly as a foundation for applied legal intelligence.

</details>


### [97] [Solver-Independent Automated Problem Formulation via LLMs for High-Cost Simulation-Driven Design](https://arxiv.org/abs/2512.18682)
*Yuchen Li, Handing Wang, Bing Xue, Mengjie Zhang, Yaochu Jin*

**主要类别:** cs.CL

**AI概要:** APF框架利用大语言模型自动将工程师的自然语言设计需求转换为可执行的优化模型，通过创新的数据生成管道解决高成本仿真环境下缺乏求解器反馈的数据集构建难题，在天线设计实验中显著优于现有方法。


<details>
  <summary>更多</summary>
  
**动机:** 在高成本仿真驱动的设计领域，将模糊的设计需求转化为数学优化公式是一个瓶颈，该过程耗时且依赖专家知识。现有LLM方法要么形式化效果差，要么依赖求解器反馈（在高仿真成本下不可用）。

**方法:** 提出APF框架，通过创新的自动生成高质量数据管道，在无求解器反馈的情况下构建合适的微调数据集，利用数据生成和测试实例标注技术，对LLM进行监督微调以提升生成准确可执行优化问题公式的能力。

**结果:** 天线设计实验结果表明，APF在需求形式化准确性和最终辐射效率曲线满足设计目标的质量方面都显著优于现有方法。

**结论:** APF框架成功解决了高成本仿真环境下自动化问题公式化的挑战，通过创新的数据生成方法实现了不依赖求解器反馈的高质量优化模型生成，为仿真驱动设计领域提供了有效的自动化解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Solver-Independent+Automated+Problem+Formulation+via+LLMs+for+High-Cost+Simulation-Driven+Design，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18682，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18682&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** In the high-cost simulation-driven design domain, translating ambiguous design requirements into a mathematical optimization formulation is a bottleneck for optimizing product performance. This process is time-consuming and heavily reliant on expert knowledge. While large language models (LLMs) offer potential for automating this task, existing approaches either suffer from poor formalization that fails to accurately align with the design intent or rely on solver feedback for data filtering, which is unavailable due to the high simulation costs. To address this challenge, we propose APF, a framework for solver-independent, automated problem formulation via LLMs designed to automatically convert engineers' natural language requirements into executable optimization models. The core of this framework is an innovative pipeline for automatically generating high-quality data, which overcomes the difficulty of constructing suitable fine-tuning datasets in the absence of high-cost solver feedback with the help of data generation and test instance annotation. The generated high-quality dataset is used to perform supervised fine-tuning on LLMs, significantly enhancing their ability to generate accurate and executable optimization problem formulations. Experimental results on antenna design demonstrate that APF significantly outperforms the existing methods in both the accuracy of requirement formalization and the quality of resulting radiation efficiency curves in meeting the design goals.

</details>


### [98] [MemEvolve: Meta-Evolution of Agent Memory Systems](https://arxiv.org/abs/2512.18746)
*Guibin Zhang, Haotian Ren, Chong Zhan, Zhenhong Zhou, Junhao Wang, He Zhu, Wangchunshu Zhou, Shuicheng Yan*

**主要类别:** cs.CL

**AI概要:** MemEvolve是一个元进化框架，联合进化代理的经验知识和内存架构，使系统不仅能积累经验，还能优化学习方式。


<details>
  <summary>更多</summary>
  
**动机:** 现有基于LLM的智能体依赖手动设计的静态内存架构，无法根据不同任务上下文进行元适应，限制了系统的进化能力。

**方法:** 提出MemEvolve框架，通过EvolveLab代码库将12种代表性内存系统模块化为编码、存储、检索和管理四个设计空间，实现内存架构的自动进化。

**结果:** 在四个具有挑战性的智能体基准测试中，MemEvolve实现了显著性能提升（最高17.06%），并展现出强大的跨任务和跨LLM泛化能力。

**结论:** MemEvolve通过元进化内存架构突破了静态内存系统的限制，为自进化系统提供了新的发展范式，具有很好的可迁移性和泛化性能。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是MemEvolve%3A+Meta-Evolution+of+Agent+Memory+Systems，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18746，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18746&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Self-evolving memory systems are unprecedentedly reshaping the evolutionary paradigm of large language model (LLM)-based agents. Prior work has predominantly relied on manually engineered memory architectures to store trajectories, distill experience, and synthesize reusable tools, enabling agents to evolve on the fly within environment interactions. However, this paradigm is fundamentally constrained by the staticity of the memory system itself: while memory facilitates agent-level evolving, the underlying memory architecture cannot be meta-adapted to diverse task contexts. To address this gap, we propose MemEvolve, a meta-evolutionary framework that jointly evolves agents' experiential knowledge and their memory architecture, allowing agent systems not only to accumulate experience but also to progressively refine how they learn from it. To ground MemEvolve in prior research and foster openness in future self-evolving systems, we introduce EvolveLab, a unified self-evolving memory codebase that distills twelve representative memory systems into a modular design space (encode, store, retrieve, manage), providing both a standardized implementation substrate and a fair experimental arena. Extensive evaluations on four challenging agentic benchmarks demonstrate that MemEvolve achieves (I) substantial performance gains, improving frameworks such as SmolAgent and Flash-Searcher by up to $17.06\%$; and (II) strong cross-task and cross-LLM generalization, designing memory architectures that transfer effectively across diverse benchmarks and backbone models.

</details>


### [99] [From Natural Language to Control Signals: A Conceptual Framework for Semantic Channel Finding in Complex Experimental Infrastructure](https://arxiv.org/abs/2512.18779)
*Thorsten Hellert, Nikolay Agladze, Alex Giovannone, Jan Jug, Frank Mayet, Mark Sherwin, Antonin Sulc, Chris Tennant*

**主要类别:** cs.CL

**AI概要:** 论文提出了一个四范式框架来解决复杂实验设施中语义通道查找问题，将自然语言意图映射到具体控制系统信号，在四个实际设施中实现了90-97%的准确率。


<details>
  <summary>更多</summary>
  
**动机:** 现代实验平台拥有数万到数十万个控制和诊断通道，操作人员和AI系统依赖非正式的专家知识、不一致的命名约定和碎片化文档来定位信号，这为可靠性、可扩展性和语言模型驱动接口造成了持续瓶颈。

**方法:** 引入四范式框架：(i)基于策划通道字典的上下文直接查找，(ii)结构化树约束分层导航，(iii)使用迭代推理和工具数据库查询的交互式代理探索，(iv)基于本体的语义搜索，将通道含义与设施特定命名约定解耦。

**结果:** 在四个运行设施中进行了概念验证实现，涵盖两个数量级的规模（从紧凑型自由电子激光器到大型同步辐射光源）和多样化的控制系统架构，实现了90-97%的专家策划操作查询准确率。

**结论:** 该框架为复杂实验基础设施中的语义通道查找问题提供了系统化的解决方案，通过多种范式适应不同的数据体制，显著提高了信号定位的准确性和效率。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是From+Natural+Language+to+Control+Signals%3A+A+Conceptual+Framework+for+Semantic+Channel+Finding+in+Complex+Experimental+Infrastructure，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18779，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18779&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Modern experimental platforms such as particle accelerators, fusion devices, telescopes, and industrial process control systems expose tens to hundreds of thousands of control and diagnostic channels accumulated over decades of evolution. Operators and AI systems rely on informal expert knowledge, inconsistent naming conventions, and fragmented documentation to locate signals for monitoring, troubleshooting, and automated control, creating a persistent bottleneck for reliability, scalability, and language-model-driven interfaces. We formalize semantic channel finding-mapping natural-language intent to concrete control-system signals-as a general problem in complex experimental infrastructure, and introduce a four-paradigm framework to guide architecture selection across facility-specific data regimes. The paradigms span (i) direct in-context lookup over curated channel dictionaries, (ii) constrained hierarchical navigation through structured trees, (iii) interactive agent exploration using iterative reasoning and tool-based database queries, and (iv) ontology-grounded semantic search that decouples channel meaning from facility-specific naming conventions. We demonstrate each paradigm through proof-of-concept implementations at four operational facilities spanning two orders of magnitude in scale-from compact free-electron lasers to large synchrotron light sources-and diverse control-system architectures, from clean hierarchies to legacy environments. These implementations achieve 90-97% accuracy on expert-curated operational queries.

</details>


### [100] [From Word to World: Can Large Language Models be Implicit Text-based World Models?](https://arxiv.org/abs/2512.18832)
*Yixia Li, Hongru Wang, Jiahao Qiu, Zhenfei Yin, Dongdong Zhang, Cheng Qian, Zeping Li, Pony Ma, Guanhua Chen, Heng Ji, Mengdi Wang*

**主要类别:** cs.CL

**AI概要:** 该研究探讨了基于大语言模型的世界模型在文本环境中的有效性，提出了三层次评估框架，发现在行为覆盖充分和环境复杂度适中的条件下，LLM世界模型能通过多种方式提升智能体性能。


<details>
  <summary>更多</summary>
  
**动机:** 现实世界环境存在非适应性、覆盖范围有限和难以扩展的问题，而世界模型通过模拟经验有望提高学习效率，但LLM是否能可靠地充当世界模型角色及其有效条件尚不明确。

**方法:** 在基于文本的环境中，将语言建模重新解释为交互下的下一状态预测，引入三层次评估框架：保真度与一致性、可扩展性与鲁棒性、智能体效用，并在五个代表性环境中进行测试。

**结果:** 充分训练的世界模型能保持一致的潜在状态，随数据和模型规模可预测地扩展，并通过动作验证、合成轨迹生成和强化学习预热等方式提升智能体性能。

**结论:** 世界模型的增益效果严重依赖于行为覆盖和环境复杂度，明确了世界建模有效支持智能体学习的边界条件。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是From+Word+to+World%3A+Can+Large+Language+Models+be+Implicit+Text-based+World+Models%3F，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18832，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18832&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Agentic reinforcement learning increasingly relies on experience-driven scaling, yet real-world environments remain non-adaptive, limited in coverage, and difficult to scale. World models offer a potential way to improve learning efficiency through simulated experience, but it remains unclear whether large language models can reliably serve this role and under what conditions they meaningfully benefit agents. We study these questions in text-based environments, which provide a controlled setting to reinterpret language modeling as next-state prediction under interaction. We introduce a three-level framework for evaluating LLM-based world models: (i) fidelity and consistency, (ii) scalability and robustness, and (iii) agent utility. Across five representative environments, we find that sufficiently trained world models maintain coherent latent state, scale predictably with data and model size, and improve agent performance via action verification, synthetic trajectory generation, and warm-starting reinforcement learning. Meanwhile, these gains depend critically on behavioral coverage and environment complexity, delineating clear boundry on when world modeling effectively supports agent learning.

</details>


### [101] [AraMix: Recycling, Refiltering, and Deduplicating to Deliver the Largest Arabic Pretraining Corpus](https://arxiv.org/abs/2512.18834)
*Sultan Alrashed, Francesco Orabona*

**主要类别:** cs.CL

**AI概要:** AraMix是一个去重后的阿拉伯语预训练语料库，包含约1780亿个token和1.79亿个文档，通过系统性地重用和整理现有数据集构建而成。


<details>
  <summary>更多</summary>
  
**动机:** 对于资源较少的语言，投资于现有数据的整理流程比额外的网络爬取更有价值，可以避免大量重复内容。

**方法:** 合并七个公开阿拉伯语网络数据集，应用针对阿拉伯语文本的质量过滤，进行跨数据集去重（包括MinHash和句子级去重）。

**结果:** 发现这些独立收集的语料库中有近60%的token是重复的，成功构建了最大的经过严格过滤的公开阿拉伯语预训练语料库。

**结论:** 对于低资源语言，专注于现有数据的整理和去重比重新爬取网络数据更有效，这种方法可以产生更大的价值回报。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是AraMix%3A+Recycling%2C+Refiltering%2C+and+Deduplicating+to+Deliver+the+Largest+Arabic+Pretraining+Corpus，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18834，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18834&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present AraMix, a deduplicated Arabic pretraining corpus containing approximately 178 billion tokens across 179 million documents. Rather than scraping the web again, AraMix demonstrates that substantial value lies in systematically reusing and curating existing pretraining datasets: we combine seven publicly available Arabic web datasets, apply quality filtering designed specifically for Arabic text to re-filter some datasets, and perform cross-dataset deduplication, both MinHash and sentence-level. This approach reveals that nearly 60% of tokens across these independently collected corpora are duplicates, redundancy that any new scraping efforts will reproduce. Our work suggests that for lower resource languages, investment in curation pipelines for existing data yields greater returns than additional web crawls, an approach that allowed us to curate the largest heavily filtered publicly available Arabic pretraining corpus.

</details>


### [102] [MDToC: Metacognitive Dynamic Tree of Concepts for Boosting Mathematical Problem-Solving of Large Language Models](https://arxiv.org/abs/2512.18841)
*Tung Duong Ta, Tim Oates*

**主要类别:** cs.CL

**AI概要:** MDToC是一种三阶段元认知方法，通过构建概念树、验证计算准确性并采用多数投票机制，显著提升大语言模型在数学计算验证任务上的表现。


<details>
  <summary>更多</summary>
  
**动机:** 尽管大语言模型在数学推理方面有所进步，但在使用现有提示技术时仍然难以有效验证计算结果的准确性。

**方法:** 提出MDToC三阶段方法：1)构建概念树；2)为每个概念开发准确性验证的计算；3)采用多数投票机制评估竞争性解决方案。

**结果:** 在CHAMP、MATH和Game-of-24基准测试中，GPT-4-Turbo分别达到58.1%、86.6%和85%的准确率，比GoT方法分别提升5%、5.4%和4%。在所有骨干模型上都优于现有提示方法，比ToT提升7.6%，比GoT提升6.2%。

**结论:** MDToC证明了元认知计算验证是提升数学推理能力的有效方向，无需手工设计的提示即可显著提升模型性能。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是MDToC%3A+Metacognitive+Dynamic+Tree+of+Concepts+for+Boosting+Mathematical+Problem-Solving+of+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18841，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18841&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Despite advances in mathematical reasoning capabilities, Large Language Models (LLMs) still struggle with calculation verification when using established prompting techniques. We present MDToC (Metacognitive Dynamic Tree of Concepts), a three-phase approach that constructs a concept tree, develops accuracy-verified calculations for each concept, and employs majority voting to evaluate competing solutions. Evaluations across CHAMP, MATH, and Game-of-24 benchmarks demonstrate our MDToC's effectiveness, with GPT-4-Turbo achieving 58.1\% on CHAMP, 86.6\% on MATH, and 85\% on Game-of-24 - outperforming GoT by 5\%, 5.4\%, and 4\% on all these tasks, respectively, without hand-engineered hints. MDToC consistently surpasses existing prompting methods across all backbone models, yielding improvements of up to 7.6\% over ToT and 6.2\% over GoT, establishing metacognitive calculation verification as a promising direction for enhanced mathematical reasoning.

</details>


### [103] [Toward Human-Centered AI-Assisted Terminology Work](https://arxiv.org/abs/2512.18859)
*Antonio San Martin*

**主要类别:** cs.CL

**AI概要:** 本文提出以人为中心的人工智能框架，强调AI应增强而非取代术语学家的能力，通过增强型术语学家、伦理AI和以人为本设计三个维度来确保术语工作的专业性、准确性和多样性。


<details>
  <summary>更多</summary>
  
**动机:** 生成式AI的快速扩散虽然提升了术语工作效率，但无结构化的采用风险会削弱专业自主性、放大偏见、侵蚀语言和概念多样性，因此需要建立以人为中心的AI方法。

**方法:** 基于人工智能和翻译研究，提出一个以人为中心的框架，围绕三个相互关联的维度：增强型术语学家、伦理AI和以人为本的设计。

**结果:** 构建了一个强调高度自动化与强大人为控制兼容性的框架，确立了术语学家在偏见缓解中的核心作用，以及围绕术语学家需求、价值观和福祉设计AI工具和工作流程的重要性。

**结论:** 当前AI采用的选择不仅将影响术语实践，还将决定术语和专业知识准确性、适当性和多样性的保护，因此必须采取以人为中心的方法。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Toward+Human-Centered+AI-Assisted+Terminology+Work，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18859，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18859&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** The rapid diffusion of generative artificial intelligence is transforming terminology work. While this technology promises gains in efficiency, its unstructured adoption risks weakening professional autonomy, amplifying bias, and eroding linguistic and conceptual diversity. This paper argues that a human-centered approach to artificial intelligence has become a necessity for terminology work. Building on research in artificial intelligence and translation studies, it proposes a human-centered framework that conceptualizes artificial intelligence as a means of amplifying the terminologist's capabilities, rather than replacing them. The framework is organized around three interrelated dimensions: the augmented terminologist, ethical AI, and human-centered design. Together, these dimensions emphasize the compatibility of high automation with strong human control, the central role of terminologists in bias mitigation, and the importance of designing AI tools and workflows around the needs, values, and well-being of the terminologist. The paper concludes by stressing that current choices in AI adoption will shape not only terminological practice, but also the preservation of accuracy, adequacy, and diversity in terminology and specialized knowledge.

</details>


### [104] [Can LLMs Estimate Student Struggles? Human-AI Difficulty Alignment with Proficiency Simulation for Item Difficulty Prediction](https://arxiv.org/abs/2512.18880)
*Ming Li, Han Chen, Yunze Xiao, Jian Chen, Hong Jiao, Tianyi Zhou*

**主要类别:** cs.CL

**AI概要:** 大型语言模型虽然在问题解决能力上超越人类，但在评估题目难度时与人类认知存在系统性偏差，模型无法准确模拟人类学习者的认知困难，且随着模型规模扩大反而趋向机器共识而非人类对齐。


<details>
  <summary>更多</summary>
  
**动机:** 解决教育评估中题目难度估计的冷启动问题，探索大型语言模型是否能够感知人类学习者的认知困难。

**方法:** 对20多个模型进行大规模实证分析，涵盖医学知识和数学推理等多样领域，研究模型在不同能力水平下对人类难度的对齐情况。

**结果:** 发现模型存在系统性不对齐现象，模型规模扩大无助于改善对齐效果；高性能模型反而更难准确估计难度；模型缺乏自省能力，无法预测自身局限性。

**结论:** 通用问题解决能力并不意味着理解人类认知困难，当前模型在自动化难度预测方面面临重大挑战。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Can+LLMs+Estimate+Student+Struggles%3F+Human-AI+Difficulty+Alignment+with+Proficiency+Simulation+for+Item+Difficulty+Prediction，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18880，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18880&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Accurate estimation of item (question or task) difficulty is critical for educational assessment but suffers from the cold start problem. While Large Language Models demonstrate superhuman problem-solving capabilities, it remains an open question whether they can perceive the cognitive struggles of human learners. In this work, we present a large-scale empirical analysis of Human-AI Difficulty Alignment for over 20 models across diverse domains such as medical knowledge and mathematical reasoning. Our findings reveal a systematic misalignment where scaling up model size is not reliably helpful; instead of aligning with humans, models converge toward a shared machine consensus. We observe that high performance often impedes accurate difficulty estimation, as models struggle to simulate the capability limitations of students even when being explicitly prompted to adopt specific proficiency levels. Furthermore, we identify a critical lack of introspection, as models fail to predict their own limitations. These results suggest that general problem-solving capability does not imply an understanding of human cognitive struggles, highlighting the challenge of using current models for automated difficulty prediction.

</details>


### [105] [Remedy-R: Generative Reasoning for Machine Translation Evaluation without Error Annotations](https://arxiv.org/abs/2512.18906)
*Shaomu Tan, Ryosuke Mitani, Ritvik Choudhary, Qiyu Wu, Toshiyuki Sekiya, Christof Monz*

**主要类别:** cs.CL

**AI概要:** Remedy-R是一种基于推理的生成式机器翻译评估指标，通过强化学习训练，无需错误标注或大语言模型蒸馏，能够提供可解释的评估分析和自我反思反馈。


<details>
  <summary>更多</summary>
  
**动机:** 传统自动机器翻译指标虽然性能强大但缺乏可解释性，且在真实世界分布外输入下表现不佳，需要更透明和鲁棒的评估方法。

**方法:** 使用强化学习从成对翻译偏好中训练生成式指标，产生准确性、流畅性和完整性的逐步分析，最终给出评分。仅需6万训练样本即可训练。

**结果:** 在WMT22-24元评估中与顶级标量指标和GPT-4评估器竞争，具有良好的跨语言泛化能力和分布外鲁棒性，并能生成可用于翻译改进的反馈。

**结论:** Remedy-R不仅提供了可解释的评估，其推理能力还能捕获翻译相关信息，通过评估-修订流程实际提升翻译质量，展示了生成式评估指标的实用价值。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Remedy-R%3A+Generative+Reasoning+for+Machine+Translation+Evaluation+without+Error+Annotations，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18906，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18906&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Over the years, automatic MT metrics have hillclimbed benchmarks and presented strong and sometimes human-level agreement with human ratings. Yet they remain black-box, offering little insight into their decision-making and often failing under real-world out-of-distribution (OOD) inputs. We introduce Remedy-R, a reasoning-driven generative MT metric trained with reinforcement learning from pairwise translation preferences, without requiring error-span annotations or distillation from closed LLMs. Remedy-R produces step-by-step analyses of accuracy, fluency, and completeness, followed by a final score, enabling more interpretable assessments. With only 60K training pairs across two language pairs, Remedy-R remains competitive with top scalar metrics and GPT-4-based judges on WMT22-24 meta-evaluation, generalizes to other languages, and exhibits strong robustness on OOD stress tests. Moreover, Remedy-R models generate self-reflective feedback that can be reused for translation improvement. Building on this finding, we introduce Remedy-R Agent, a simple evaluate-revise pipeline that leverages Remedy-R's evaluation analysis to refine translations. This agent consistently improves translation quality across diverse models, including Qwen2.5, ALMA-R, GPT-4o-mini, and Gemini-2.0-Flash, suggesting that Remedy-R's reasoning captures translation-relevant information and is practically useful.

</details>


### [106] [FASTRIC: Prompt Specification Language for Verifiable LLM Interactions](https://arxiv.org/abs/2512.18940)
*Wen-Long Jin*

**主要类别:** cs.CL

**AI概要:** FASTRIC是一种提示规范语言，通过将有限状态机(FSM)显式编码到自然语言提示中，使大语言模型能够执行符合设计意图的多轮交互协议，并提供可验证的程序一致性度量。


<details>
  <summary>更多</summary>
  
**动机:** 大语言模型执行复杂多轮交互协议时缺乏形式化规范来验证执行是否符合设计意图，需要一种方法来使隐式的状态机规范显式化。

**方法:** 提出FASTRIC提示规范语言，指导设计者明确表达FSM的七个元素（最终状态、代理、状态、触发器、角色、初始状态、约束），利用LLM作为统一的解析器、解释器和运行时环境。

**结果:** 在不同形式化级别和模型规模（14.7B、685B、1T+参数）测试显示，最佳规范形式化程度是模型容量的函数，DeepSeek-V3.2在L2-L4级别实现完美一致性，ChatGPT-5在L3达到峰值，Phi4表现不稳定。

**结论:** 研究发现模型特定的'金发女孩区域'形式化范围，为创建可验证交互协议建立了提示规范工程，将多轮交互设计从启发式艺术转变为具有可测量程序保证的系统工程。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是FASTRIC%3A+Prompt+Specification+Language+for+Verifiable+LLM+Interactions，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18940，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18940&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large Language Models (LLMs) execute complex multi-turn interaction protocols but lack formal specifications to verify execution against designer intent. We introduce FASTRIC, a Prompt Specification Language that makes implicit Finite State Machines (FSMs) explicit in natural language prompts, enabling conformance verification through execution trace analysis. The LLM serves as intelligent execution agent: interpreting designer-encoded FSMs to execute specified behavioral roles. Unlike symbolic specification languages requiring parsers and compilers, FASTRIC leverages LLMs as unified infrastructure-simultaneously parser, interpreter, runtime environment, and development assistant. FASTRIC guides designers to articulate seven FSM elements (Final States, Agents, States, Triggers, Roles, Initial State, Constraints) structuring multi-turn interactions. Specification formality-ranging from implicit descriptions that frontier models infer to explicit step-by-step instructions for weaker models-serves as a design parameter. We introduce procedural conformance as verification metric measuring execution adherence to FSM specifications. Testing a 3-state kindergarten tutoring FSM across four formality levels and three model scales (14.7B, 685B, 1T+ parameters) reveals optimal specification formality is a function of model capacity. DeepSeek-V3.2 (685B) achieves perfect conformance (1.00) at L2-L4; ChatGPT-5 (~1T) peaks at L3 (0.90) before collapsing at L4 (0.39); Phi4 (14.7B) shows no stable optimum with high variance (SD=0.16-0.36). These findings reveal model-specific formality ranges-"Goldilocks zones"-where specifications provide sufficient structure without over-constraint, establishing Prompt Specification Engineering for creating verifiable interaction protocols, transforming multi-turn interaction design from heuristic art to systematic engineering with measurable procedural guarantees.

</details>


### [107] [Evaluating the Challenges of LLMs in Real-world Medical Follow-up: A Comparative Study and An Optimized Framework](https://arxiv.org/abs/2512.18999)
*Jinyan Liu, Zikang Chen, Qinchuan Wang, Tan Xie, Heming Zheng, Xudong Lv*

**主要类别:** cs.CL

**AI概要:** 对比研究两种医疗随访聊天机器人系统：端到端LLM系统与模块化流水线系统，发现模块化方法通过任务分解、语义聚类和流程管理显著提升对话稳定性和信息提取准确性，减少对话轮次和token消耗。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在医疗随访任务中直接端到端应用时，由于随访表格的复杂性，经常出现对话流程失控和信息提取不准确的问题。

**方法:** 设计并比较两种系统：端到端LLM系统（对照组）和具有结构化流程控制的模块化流水线系统（实验组）。模块化方法基于任务分解、语义聚类和流程管理。

**结果:** 端到端方法在冗长复杂表格上经常失败，而模块化方法显著改善对话稳定性和提取准确性，减少对话轮次46.73%，降低token消耗80%-87.5%。

**结论:** 在高风险医疗随访场景中部署LLM时，必须集成外部控制机制以确保系统稳定性和准确性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Evaluating+the+Challenges+of+LLMs+in+Real-world+Medical+Follow-up%3A+A+Comparative+Study+and+An+Optimized+Framework，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.18999，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.18999&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** When applied directly in an end-to-end manner to medical follow-up tasks, Large Language Models (LLMs) often suffer from uncontrolled dialog flow and inaccurate information extraction due to the complexity of follow-up forms. To address this limitation, we designed and compared two follow-up chatbot systems: an end-to-end LLM-based system (control group) and a modular pipeline with structured process control (experimental group). Experimental results show that while the end-to-end approach frequently fails on lengthy and complex forms, our modular method-built on task decomposition, semantic clustering, and flow management-substantially improves dialog stability and extraction accuracy. Moreover, it reduces the number of dialogue turns by 46.73% and lowers token consumption by 80% to 87.5%. These findings highlight the necessity of integrating external control mechanisms when deploying LLMs in high-stakes medical follow-up scenarios.

</details>


### [108] [Context-Aware Initialization for Reducing Generative Path Length in Diffusion Language Models](https://arxiv.org/abs/2512.19004)
*Tongyuan Miao, Gary Huang, Kai Jun Han, Annie Jiang*

**主要类别:** cs.CL

**AI概要:** DLLMs通过上下文感知初始化缩短扩散轨迹，使用轻量级辅助模型注入提示条件先验，减少约35%的去噪迭代次数，但需要解决准确率下降的挑战


<details>
  <summary>更多</summary>
  
**动机:** 扩散大语言模型需要多次去噪迭代才能从完全掩码初始化生成连贯文本，推理效率低。现有方法主要优化求解器或采样策略，但本文提出缩短轨迹本身的新视角

**方法:** 提出无需训练的上下文感知初始化接口：1) 离散token注入 2) 表示级嵌入插值 3) 基于置信度的重新掩码机制作为先验怀疑机制

**结果:** 在GSM8K上实验显示可减少约35%的函数评估次数，但发现简单的热启动会降低最终准确率

**结论:** 上下文感知初始化能有效减少迭代次数，但需要围绕校准、修正机制和表示对齐开展研究来解决准确率问题

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Context-Aware+Initialization+for+Reducing+Generative+Path+Length+in+Diffusion+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19004，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19004&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Diffusion Large Language Models (DLLMs) enable fully parallel token decoding but often remain impractical at inference time due to the many denoising iterations required to refine an information-free, fully masked initialization into coherent text. Most existing acceleration methods focus on traversing this generative trajectory more efficiently via improved solvers or sampling strategies. We advance a complementary perspective: shorten the trajectory itself by starting closer to the target distribution through context-aware initialization.
  We propose a training-free interface that injects prompt-conditioned priors from a lightweight auxiliary model into the diffusion initialization, and instantiate it with two mechanisms: discrete token injection and representation-level embedding interpolation. Because injected priors can be imperfect and unmask-only decoding can over-commit early, we also introduce a simple confidence-based remasking mechanism as a form of prior skepticism. Preliminary evidence on GSM8K suggests that context-aware initialization can substantially reduce denoising iterations (about 35\% fewer function evaluations in our setting), while also exposing a key open challenge: naive warm-starting can degrade final accuracy relative to strong diffusion baselines. We use these findings to motivate a research agenda around calibration, revision mechanisms, and representation alignment for reliable warm-started diffusion decoding.

</details>


### [109] [DramaBench: A Six-Dimensional Evaluation Framework for Drama Script Continuation](https://arxiv.org/abs/2512.19012)
*Shijian Ma, Yunqi Huang, Yan Lin*

**主要类别:** cs.CL

**AI概要:** DramaBench是首个大规模戏剧剧本续写评估基准，从6个独立维度全面评估模型能力，包括格式标准、叙事效率、角色一致性、情感深度、逻辑一致性和冲突处理。


<details>
  <summary>更多</summary>
  
**动机:** 现有基准无法全面评估戏剧剧本续写所需的角色一致性、情节推进连贯性和戏剧结构保持能力。

**方法:** 结合基于规则的分析、基于LLM的标注和统计指标，对8个最先进语言模型在1,103个剧本上进行评估，并进行统计显著性测试和人工验证。

**结果:** 对8,824次评估的分析显示，65.9%的成对比较具有统计显著性，人工验证在3/5维度上达到显著一致性，所有六个维度捕获了独立的质量方面（平均|r|=0.020）。

**结论:** DramaBench为模型改进提供了可操作的维度特定反馈，为创意写作评估建立了严格标准。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是DramaBench%3A+A+Six-Dimensional+Evaluation+Framework+for+Drama+Script+Continuation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19012，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19012&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Drama script continuation requires models to maintain character consistency, advance plot coherently, and preserve dramatic structurecapabilities that existing benchmarks fail to evaluate comprehensively. We present DramaBench, the first large-scale benchmark for evaluating drama script continuation across six independent dimensions: Format Standards, Narrative Efficiency, Character Consistency, Emotional Depth, Logic Consistency, and Conflict Handling. Our framework combines rulebased analysis with LLM-based labeling and statistical metrics, ensuring objective and reproducible evaluation. We conduct comprehensive evaluation of 8 state-of-the-art language models on 1,103 scripts (8,824 evaluations total), with rigorous statistical significance testing (252 pairwise comparisons, 65.9% significant) and human validation (188 scripts, substantial agreement on 3/5 dimensions). Our ablation studies confirm all six dimensions capture independent quality aspects (mean | r | = 0.020). DramaBench provides actionable, dimensionspecific feedback for model improvement and establishes a rigorous standard for creative writing evaluation.

</details>


### [110] [A Large Language Model Based Method for Complex Logical Reasoning over Knowledge Graphs](https://arxiv.org/abs/2512.19092)
*Ziyan Zhang, Chao Wang, Zhuo Chen, Lei Chen, Chiyi Li, Kai Song*

**主要类别:** cs.CL

**AI概要:** ROG框架结合知识图谱检索与大语言模型推理，通过分解复杂逻辑查询为子查询序列，实现比传统嵌入方法更好的知识图谱推理效果


<details>
  <summary>更多</summary>
  
**动机:** 现有基于嵌入的知识图谱推理方法在处理复杂查询结构、多操作符和深度推理链时效果不佳，需要新的解决方案

**方法:** ROG框架采用查询感知的KG邻域检索获取相关子图，使用大语言模型进行逐步的链式推理，分解复杂一阶逻辑查询为简单子查询序列

**结果:** 在标准KG推理基准测试中，ROG在平均倒数排名(MRR)上持续优于强嵌入基线，在复杂查询类型上表现尤为突出

**结论:** 结合结构化KG检索与LLM驱动的逻辑推理为复杂KG推理任务提供了强大有效的替代方案

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是A+Large+Language+Model+Based+Method+for+Complex+Logical+Reasoning+over+Knowledge+Graphs，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19092，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19092&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Reasoning over knowledge graphs (KGs) with first-order logic (FOL) queries is challenging due to the inherent incompleteness of real-world KGs and the compositional complexity of logical query structures. Most existing methods rely on embedding entities and relations into continuous geometric spaces and answer queries via differentiable set operations. While effective for simple query patterns, these approaches often struggle to generalize to complex queries involving multiple operators, deeper reasoning chains, or heterogeneous KG schemas. We propose ROG (Reasoning Over knowledge Graphs with large language models), an ensemble-style framework that combines query-aware KG neighborhood retrieval with large language model (LLM)-based chain-of-thought reasoning. ROG decomposes complex FOL queries into sequences of simpler sub-queries, retrieves compact, query-relevant subgraphs as contextual evidence, and performs step-by-step logical inference using an LLM, avoiding the need for task-specific embedding optimization. Experiments on standard KG reasoning benchmarks demonstrate that ROG consistently outperforms strong embedding-based baselines in terms of mean reciprocal rank (MRR), with particularly notable gains on high-complexity query types. These results suggest that integrating structured KG retrieval with LLM-driven logical reasoning offers a robust and effective alternative for complex KG reasoning tasks.

</details>


### [111] [Stop saying LLM: Large Discourse Models (LDM) and Artificial Discursive Agent (ADA)?](https://arxiv.org/abs/2512.19117)
*Amar Lakel*

**主要类别:** cs.CL

**AI概要:** 本文提出从认识论角度将大语言模型(LLM)重新概念化为大话语模型(LDM)和人工话语代理(ADA)，基于本体三元框架分析其运作机制，主张通过多方共管机制取代对AI的迷恋/恐惧二分法。


<details>
  <summary>更多</summary>
  
**动机:** 当前对大生成模型的分析存在局限性，需要从认识论层面重新审视其本质，超越单纯的语言模型范畴，建立更全面的话语分析框架。

**方法:** 提出本体三元理论框架：1)对现象世界规律的理解；2)具身认知的结构化；3)社会历史语境中的语言结构沉淀。通过这一框架分析LDMs如何对学习语料中固化的人类经验进行话语投射建模。

**结果:** 建立了从LLM到LDM再到ADA的概念演进框架，为理解生成模型提供了更丰富的理论工具。

**结论:** 应该用公共试验程序和多方共管机制(政府、产业、公民社会、学术界)来明确人工话语代理在社会空间中的位置、用途和限制，实现治理和共同监管。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Stop+saying+LLM%3A+Large+Discourse+Models+%28LDM%29+and+Artificial+Discursive+Agent+%28ADA%29%3F，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19117，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19117&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper proposes an epistemological shift in the analysis of large generative models, replacing the category ''Large Language Models'' (LLM) with that of ''Large Discourse Models'' (LDM), and then with that of Artificial Discursive Agent (ADA). The theoretical framework is based on an ontological triad distinguishing three regulatory instances: the apprehension of the phenomenal regularities of the referential world, the structuring of embodied cognition, and the structural-linguistic sedimentation of the utterance within a socio-historical context. LDMs, operating on the product of these three instances (the document), model the discursive projection of a portion of human experience reified by the learning corpus. The proposed program aims to replace the ''fascination/fear'' dichotomy with public trials and procedures that make the place, uses, and limits of artificial discursive agents in contemporary social space decipherable, situating this approach within a perspective of governance and co-regulation involving the State, industry, civil society, and academia.

</details>


### [112] [SAP: Syntactic Attention Pruning for Transformer-based Language Models](https://arxiv.org/abs/2512.19125)
*Tzu-Yun Lee, Ding-Yong Hong, Jan-Jan Wu*

**主要类别:** cs.CL

**AI概要:** SAP是一种新的注意力头剪枝方法，结合句法结构和注意力模式来指导剪枝过程，在保持性能的同时提升模型可解释性，并在无重训练设置中优于现有方法。


<details>
  <summary>更多</summary>
  
**动机:** 传统剪枝方法仅依赖模型权重和激活的数学分析，缺乏对语言结构的考虑，需要更有效且可解释的剪枝策略。

**方法:** 提出Syntactic Attention Pruning (SAP)方法，利用句法结构和注意力模式指导剪枝；引入Candidate Filtering (CF)机制基于性能贡献优先选择注意力头。

**结果:** SAP有效保留了具有高密度强注意力值的关键头，在无重训练设置中优于现有头剪枝策略，性能与最先进方法相当。

**结论:** SAP为模型压缩研究提供了新方向，具有高灵活性，适用于所有基于Transformer的语言模型。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是SAP%3A+Syntactic+Attention+Pruning+for+Transformer-based+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19125，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19125&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper introduces Syntactic Attention Pruning (SAP), a novel method for effectively pruning attention heads in Transformer models. Unlike conventional approaches that rely solely on mathematical analysis of model weights and activations, SAP incorporates both the syntactic structure and attention patterns of sentences to guide the pruning process. By leveraging these linguistic features, SAP not only achieves performance comparable to state-of-the-art methods but also enhances the interpretability of model behavior. To further improve robustness, we propose Candidate Filtering (CF), a mechanism that prioritizes heads based on their contribution to model performance, mitigating degradation during pruning. Experimental results indicate that SAP effectively preserves critical heads of a high density of strong attention values, outperforming existing head pruning strategies in retrain-free settings. These findings position SAP as a promising foundation for a new direction in model compression research, offering high flexibility for pruning across all transformer-based language models.

</details>


### [113] [AWPO: Enhancing Tool-Use of Large Language Models through Explicit Integration of Reasoning Rewards](https://arxiv.org/abs/2512.19126)
*Zihan Lin, Xiaohan Wang, Hexiong Yang, Jiajun Chai, Jie Cao, Guojun Yin, Wei Lin, Ran He*

**主要类别:** cs.CL

**AI概要:** AWPO是一个新的强化学习框架，通过优势加权策略优化有效整合显式推理奖励来提升大语言模型的工具使用能力，在多项基准测试中达到最先进性能。


<details>
  <summary>更多</summary>
  
**动机:** 现有方法忽视了显式推理奖励对增强推理和工具使用的潜力，且推理奖励与结果奖励的简单结合可能导致次优性能或与主要优化目标冲突。

**方法:** 提出优势加权策略优化(AWPO)框架，包含方差感知门控和难度感知加权机制，基于组相对统计自适应调节推理信号的优势，并采用定制化的裁剪机制确保优化稳定性。

**结果:** AWPO在标准工具使用基准测试中取得最先进性能，显著超越强基线方法和领先闭源模型。4B参数模型在多轮场景中比Grok-4准确率高16.0%，同时在MMLU-Pro分布外基准上保持泛化能力。

**结论:** AWPO框架通过有效整合显式推理奖励，成功提升了LLMs的工具使用能力，在参数效率和多轮场景性能方面表现出色，同时保持了良好的泛化性能。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是AWPO%3A+Enhancing+Tool-Use+of+Large+Language+Models+through+Explicit+Integration+of+Reasoning+Rewards，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19126，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19126&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** While reinforcement learning (RL) shows promise in training tool-use large language models (LLMs) using verifiable outcome rewards, existing methods largely overlook the potential of explicit reasoning rewards to bolster reasoning and tool utilization. Furthermore, natively combining reasoning and outcome rewards may yield suboptimal performance or conflict with the primary optimization objective. To address this, we propose advantage-weighted policy optimization (AWPO) -- a principled RL framework that effectively integrates explicit reasoning rewards to enhance tool-use capability. AWPO incorporates variance-aware gating and difficulty-aware weighting to adaptively modulate advantages from reasoning signals based on group-relative statistics, alongside a tailored clipping mechanism for stable optimization. Extensive experiments demonstrate that AWPO achieves state-of-the-art performance across standard tool-use benchmarks, significantly outperforming strong baselines and leading closed-source models in challenging multi-turn scenarios. Notably, with exceptional parameter efficiency, our 4B model surpasses Grok-4 by 16.0 percent in multi-turn accuracy while preserving generalization capability on the out-of-distribution MMLU-Pro benchmark.

</details>


### [114] [QuCo-RAG: Quantifying Uncertainty from the Pre-training Corpus for Dynamic Retrieval-Augmented Generation](https://arxiv.org/abs/2512.19134)
*Dehai Min, Kailin Zhang, Tongtong Wu, Lu Cheng*

**主要类别:** cs.CL

**AI概要:** QuCo-RAG提出基于预训练数据客观统计的动态检索增强生成方法，通过实体共现验证来减少大语言模型幻觉，相比现有方法显著提升问答准确性。


<details>
  <summary>更多</summary>
  
**动机:** 现有动态RAG方法依赖模型内部信号（如logits、熵）来判断何时检索，但这些信号不可靠，因为大语言模型通常校准不良且对错误输出表现出高置信度。

**方法:** 采用两阶段不确定性量化：生成前识别低频实体指示长尾知识缺口；生成时验证实体在预训练语料中的共现情况，零共现通常表示幻觉风险。使用Infini-gram在4万亿token语料上进行毫秒级查询，高不确定性时触发检索。

**结果:** 在多跳问答基准测试中，QuCo-RAG比最先进基线方法在OLMo-2模型上获得5-12个点的EM提升，在Llama、Qwen、GPT等模型上也能有效迁移，EM提升最高达14点。生物医学QA领域的泛化验证了该范式的鲁棒性。

**结论:** 基于语料库的验证为动态RAG提供了一个原则性、实际模型无关的范式，能有效减少大语言模型的幻觉问题。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是QuCo-RAG%3A+Quantifying+Uncertainty+from+the+Pre-training+Corpus+for+Dynamic+Retrieval-Augmented+Generation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19134，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19134&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Dynamic Retrieval-Augmented Generation adaptively determines when to retrieve during generation to mitigate hallucinations in large language models (LLMs). However, existing methods rely on model-internal signals (e.g., logits, entropy), which are fundamentally unreliable because LLMs are typically ill-calibrated and often exhibit high confidence in erroneous outputs. We propose QuCo-RAG, which shifts from subjective confidence to objective statistics computed from pre-training data. Our method quantifies uncertainty through two stages: (1) before generation, we identify low-frequency entities indicating long-tail knowledge gaps; (2) during generation, we verify entity co-occurrence in the pre-training corpus, where zero co-occurrence often signals hallucination risk. Both stages leverage Infini-gram for millisecond-latency queries over 4 trillion tokens, triggering retrieval when uncertainty is high. Experiments on multi-hop QA benchmarks show QuCo-RAG achieves EM gains of 5--12 points over state-of-the-art baselines with OLMo-2 models, and transfers effectively to models with undisclosed pre-training data (Llama, Qwen, GPT), improving EM by up to 14 points. Domain generalization on biomedical QA further validates the robustness of our paradigm. These results establish corpus-grounded verification as a principled, practically model-agnostic paradigm for dynamic RAG. Our code is publicly available at https://github.com/ZhishanQ/QuCo-RAG.

</details>


### [115] [From Speech to Subtitles: Evaluating ASR Models in Subtitling Italian Television Programs](https://arxiv.org/abs/2512.19161)
*Alessandro Lucca, Francesco Pierri*

**主要类别:** cs.CL

**AI概要:** 对意大利长视频字幕生成的研究发现，现有ASR模型无法完全替代人工字幕员，但可作为提升效率的辅助工具，建议采用人机协同的HITL方案。


<details>
  <summary>更多</summary>
  
**动机:** 现代ASR系统在标准数据集上表现良好，但在非英语内容（如意大利长视频）的实际生产环境中的性能尚未充分研究，需要为意大利媒体公司开发专业字幕系统提供参考。

**方法:** 评估四种最先进的ASR模型（Whisper Large v2、AssemblyAI Universal、Parakeet TDT v3 0.6b和WhisperX）在50小时意大利电视节目数据集上的表现，并与专业人工字幕员的工作进行基准测试。

**结果:** 研究发现当前模型无法满足媒体行业对完全自主化的准确度需求，但可以作为提升人类生产力的高效工具。

**结论:** 人机协同（HITL）方法至关重要，研究设计了支持这种工作流程的生产级云端基础设施。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是From+Speech+to+Subtitles%3A+Evaluating+ASR+Models+in+Subtitling+Italian+Television+Programs，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19161，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19161&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Subtitles are essential for video accessibility and audience engagement. Modern Automatic Speech Recognition (ASR) systems, built upon Encoder-Decoder neural network architectures and trained on massive amounts of data, have progressively reduced transcription errors on standard benchmark datasets. However, their performance in real-world production environments, particularly for non-English content like long-form Italian videos, remains largely unexplored. This paper presents a case study on developing a professional subtitling system for an Italian media company. To inform our system design, we evaluated four state-of-the-art ASR models (Whisper Large v2, AssemblyAI Universal, Parakeet TDT v3 0.6b, and WhisperX) on a 50-hour dataset of Italian television programs. The study highlights their strengths and limitations, benchmarking their performance against the work of professional human subtitlers. The findings indicate that, while current models cannot meet the media industry's accuracy needs for full autonomy, they can serve as highly effective tools for enhancing human productivity. We conclude that a human-in-the-loop (HITL) approach is crucial and present the production-grade, cloud-based infrastructure we designed to support this workflow.

</details>


### [116] [JEPA-Reasoner: Decoupling Latent Reasoning from Token Generation](https://arxiv.org/abs/2512.19171)
*Bingyang Kelvin Liu, Ziyu Patrick Chen*

**主要类别:** cs.CL

**AI概要:** JEPA-Reasoner：一种结合联合嵌入预测架构和生成能力的新型模型，通过在潜在空间进行推理并分离推理与标记生成，提高了鲁棒性和推理能力


<details>
  <summary>更多</summary>
  
**动机:** 现有JEPA架构缺乏生成能力，而基于Transformer的潜在空间推理方法仍存在累积误差问题，需要依赖上下文信息获得推理洞察

**方法:** 提出JEPA-Reasoner模型，增强JEPA的生成能力并在潜在空间进行推理；使用独立的Talker动作执行模型生成人类可读句子；解耦潜在空间推理和标记生成

**结果:** 能够产生可能支持多线程推理的混合潜在向量；在自回归生成中表现出对累积误差的优异鲁棒性

**结论:** 解耦潜在空间推理和标记生成的方法为多线程推理奠定了基础，同时提供了更强的抗累积误差能力

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是JEPA-Reasoner%3A+Decoupling+Latent+Reasoning+from+Token+Generation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19171，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19171&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** While Joint-Embedding Predictive Architecture (JEPA) has emerged as a powerful architecture for learning rich latent representations, it fundamentally lacks generative abilities. Meanwhile, latent space reasoning attempts for Transformer models like COCONUT do improve performance, but they ultimately rely on token-by-token generation, which still accumulates compounding error and relies on context information to gain reasoning insights. To address these limitations, we propose JEPA-Reasoner, a novel JEPA model enhanced with generative ability that reasons in latent space. We augment it with a separate action-taker model, Talker, to produce human-readable sentences. Our approach demonstrates that decoupling latent space reasoning and token generation enables JEPA-Reasoner to produce mixed latent vectors that might lay the foundation for multi-threaded reasoning, while performing autoregressive generation with superior robustness to compounding error.

</details>


### [117] [CycleChart: A Unified Consistency-Based Learning Framework for Bidirectional Chart Understanding and Generation](https://arxiv.org/abs/2512.19173)
*Dazhen Deng, Sen Yang, Yuchen He, Yuan Tian, Yingcai Wu*

**主要类别:** cs.CL

**AI概要:** CycleChart是一个基于一致性的双向图表理解与生成框架，通过schema-centric方法统一多个图表任务，使用generate-parse一致性目标学习跨方向语义，在图表生成、解析和问答任务上取得优异表现。


<details>
  <summary>更多</summary>
  
**动机:** 当前图表特定任务（如图表问答、图表解析和图表生成）通常孤立研究，阻碍了模型学习连接图表生成和解释的共享语义。

**方法:** 采用schema-centric表述作为跨任务的通用接口，构建一致的多任务数据集，引入generate-parse一致性目标：从表格和文本查询生成图表schema，然后从生成的图表中恢复schema和数据。

**结果:** 在图表生成、图表解析和图表问答任务上取得了强劲结果，展示了改进的跨任务泛化能力。

**结论:** CycleChart标志着向更通用的图表理解模型迈出了一步，通过一致性学习框架成功实现了双向图表理解与生成的统一处理。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是CycleChart%3A+A+Unified+Consistency-Based+Learning+Framework+for+Bidirectional+Chart+Understanding+and+Generation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19173，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19173&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Current chart-specific tasks, such as chart question answering, chart parsing, and chart generation, are typically studied in isolation, preventing models from learning the shared semantics that link chart generation and interpretation. We introduce CycleChart, a consistency-based learning framework for bidirectional chart understanding and generation. CycleChart adopts a schema-centric formulation as a common interface across tasks. We construct a consistent multi-task dataset, where each chart sample includes aligned annotations for schema prediction, data parsing, and question answering. To learn cross-directional chart semantics, CycleChart introduces a generate-parse consistency objective: the model generates a chart schema from a table and a textual query, then learns to recover the schema and data from the generated chart, enforcing semantic alignment across directions. CycleChart achieves strong results on chart generation, chart parsing, and chart question answering, demonstrating improved cross-task generalization and marking a step toward more general chart understanding models.

</details>


### [118] [Identifying Features Associated with Bias Against 93 Stigmatized Groups in Language Models and Guardrail Model Safety Mitigation](https://arxiv.org/abs/2512.19238)
*Anna-Maria Gueorguieva, Aylin Caliskan*

**主要类别:** cs.CL

**AI概要:** 该研究探讨了大型语言模型对93个被污名化群体的偏见，发现人类认为高危险性的污名（如帮派成员、HIV感染者）在模型输出中偏见最严重（60%），而社会人口污名偏见最少（11%）。使用防护模型可减少偏见但效果有限，且防护模型难以识别偏见意图。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型已被证明存在社会偏见，但对非受保护污名化身份的偏见研究不足，且不清楚污名的哪些社会特征与LLM输出偏见相关。

**方法:** 使用SocialStigmaQA基准测试三个主流LLM（Granite 3.0-8B、Llama-3.1-8B、Mistral-7B），评估93个污名化群体在37个社会场景中的偏见，分析污名的六个社会特征（美学、可隐藏性、病程、破坏性、起源、危险性）与偏见的关系，并测试各模型的防护模型效果。

**结果:** 高危险性污名偏见最严重（60%输出有偏见），社会人口污名偏见最少（11%）。防护模型使偏见分别减少10.4%、1.4%和7.8%，但对偏见特征的影响不变，且防护模型常无法识别偏见意图。

**结论:** LLM在处理污名化群体时存在显著偏见，防护模型虽有缓解作用但效果有限且不彻底。需要在涉及污名化群体的场景中谨慎使用LLM，并改进防护模型的偏见缓解能力。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Identifying+Features+Associated+with+Bias+Against+93+Stigmatized+Groups+in+Language+Models+and+Guardrail+Model+Safety+Mitigation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19238，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19238&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) have been shown to exhibit social bias, however, bias towards non-protected stigmatized identities remain understudied. Furthermore, what social features of stigmas are associated with bias in LLM outputs is unknown. From psychology literature, it has been shown that stigmas contain six shared social features: aesthetics, concealability, course, disruptiveness, origin, and peril. In this study, we investigate if human and LLM ratings of the features of stigmas, along with prompt style and type of stigma, have effect on bias towards stigmatized groups in LLM outputs. We measure bias against 93 stigmatized groups across three widely used LLMs (Granite 3.0-8B, Llama-3.1-8B, Mistral-7B) using SocialStigmaQA, a benchmark that includes 37 social scenarios about stigmatized identities; for example deciding wether to recommend them for an internship. We find that stigmas rated by humans to be highly perilous (e.g., being a gang member or having HIV) have the most biased outputs from SocialStigmaQA prompts (60% of outputs from all models) while sociodemographic stigmas (e.g. Asian-American or old age) have the least amount of biased outputs (11%). We test if the amount of biased outputs could be decreased by using guardrail models, models meant to identify harmful input, using each LLM's respective guardrail model (Granite Guardian 3.0, Llama Guard 3.0, Mistral Moderation API). We find that bias decreases significantly by 10.4%, 1.4%, and 7.8%, respectively. However, we show that features with significant effect on bias remain unchanged post-mitigation and that guardrail models often fail to recognize the intent of bias in prompts. This work has implications for using LLMs in scenarios involving stigmatized groups and we suggest future work towards improving guardrail models for bias mitigation.

</details>


### [119] [ChemATP: A Training-Free Chemical Reasoning Framework for Large Language Models](https://arxiv.org/abs/2512.19240)
*Mingxu Zhang, Dazhong Shen, Qi Zhang, Ying Sun*

**主要类别:** cs.CL

**AI概要:** ChemATP框架通过构建原子级文本知识库，使冻结的大型语言模型能够动态检索和推理化学知识，解决了传统方法在化学推理中缺乏显式化学先验的问题。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在分子科学中表现不佳，因为标准字符串表示缺乏显式化学先验。现有方法面临两难：基于训练的方法将先验注入参数但影响模型通用能力，而无训练方法依赖表面提示无法提供原子级先验。

**方法:** 提出ChemATP框架，构建首个原子级文本知识库，将化学知识与推理引擎解耦，使冻结的LLM能够动态检索和推理这些信息。

**结果:** ChemATP显著优于无训练基线方法，与最先进的基于训练模型性能相当，证明显式先验注入是隐式参数更新的竞争性替代方案。

**结论:** 通过显式化学知识检索和推理的架构，ChemATP在保持LLM通用智能的同时确保了可解释性和适应性，为分子科学中的LLM应用提供了有效解决方案。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是ChemATP%3A+A+Training-Free+Chemical+Reasoning+Framework+for+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19240，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19240&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large Language Models (LLMs) exhibit strong general reasoning but struggle in molecular science due to the lack of explicit chemical priors in standard string representations. Current solutions face a fundamental dilemma. Training-based methods inject priors into parameters, but this static coupling hinders rapid knowledge updates and often compromises the model's general reasoning capabilities. Conversely, existing training-free methods avoid these issues but rely on surface-level prompting, failing to provide the fine-grained atom-level priors essential for precise chemical reasoning. To address this issue, we introduce ChemATP, a framework that decouples chemical knowledge from the reasoning engine. By constructing the first atom-level textual knowledge base, ChemATP enables frozen LLMs to explicitly retrieve and reason over this information dynamically. This architecture ensures interpretability and adaptability while preserving the LLM's intrinsic general intelligence. Experiments show that ChemATP significantly outperforms training-free baselines and rivals state-of-the-art training-based models, demonstrating that explicit prior injection is a competitive alternative to implicit parameter updates.

</details>


### [120] [Auto-Prompting with Retrieval Guidance for Frame Detection in Logistics](https://arxiv.org/abs/2512.19247)
*Do Minh Duc, Quan Xuan Truong, Nguyen Tat Dat, Nguyen Van Vinh*

**主要类别:** cs.CL

**AI概要:** 本文提出了一种结合RAG、少样本提示、CoT推理和Auto-CoT的新型提示优化管道，用于物流文本中的框架检测任务，通过LLM提示优化代理迭代优化提示，在真实物流文本标注任务中实现高达15%的准确率提升。


<details>
  <summary>更多</summary>
  
**动机:** 提示工程在使大语言模型适应复杂推理和标注任务中至关重要，但需要高效的提示优化方法，特别是在物流文本处理这类需要高精度和效率的领域特定应用中。

**方法:** 采用检索增强生成(RAG)、少样本提示、思维链(CoT)推理和自动CoT合成(Auto-CoT)相结合的方法，构建基于LLM的提示优化代理，通过检索示例、性能反馈和内部自评估迭代优化提示。

**结果:** 实验结果显示，优化的提示（特别是通过Auto-CoT和RAG增强的）相比基线零样本或静态提示，在真实推理准确率上提升高达15%，在GPT-4o、Qwen 2.5和LLaMA 3.1等多个LLM上均表现出一致的改进。

**结论:** 结构化提示优化是完整微调的有效替代方案，为在物流等特定领域NLP应用中部署LLM提供了可扩展的解决方案，具有普遍适用性和实用价值。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Auto-Prompting+with+Retrieval+Guidance+for+Frame+Detection+in+Logistics，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19247，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19247&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Prompt engineering plays a critical role in adapting large language models (LLMs) to complex reasoning and labeling tasks without the need for extensive fine-tuning. In this paper, we propose a novel prompt optimization pipeline for frame detection in logistics texts, combining retrieval-augmented generation (RAG), few-shot prompting, chain-of-thought (CoT) reasoning, and automatic CoT synthesis (Auto-CoT) to generate highly effective task-specific prompts. Central to our approach is an LLM-based prompt optimizer agent that iteratively refines the prompts using retrieved examples, performance feedback, and internal self-evaluation. Our framework is evaluated on a real-world logistics text annotation task, where reasoning accuracy and labeling efficiency are critical. Experimental results show that the optimized prompts - particularly those enhanced via Auto-CoT and RAG - improve real-world inference accuracy by up to 15% compared to baseline zero-shot or static prompts. The system demonstrates consistent improvements across multiple LLMs, including GPT-4o, Qwen 2.5 (72B), and LLaMA 3.1 (70B), validating its generalizability and practical value. These findings suggest that structured prompt optimization is a viable alternative to full fine-tuning, offering scalable solutions for deploying LLMs in domain-specific NLP applications such as logistics.

</details>


### [121] [CienaLLM: Generative Climate-Impact Extraction from News Articles with Autoregressive LLMs](https://arxiv.org/abs/2512.19305)
*Javier Vela-Tambo, Jorge Gracia, Fernando Dominguez-Castro*

**主要类别:** cs.CL

**AI概要:** CienaLLM是一个基于模式引导生成信息提取的模块化框架，使用开源大语言模型从新闻文章中零样本提取结构化信息，支持可配置提示和输出模式，在干旱影响提取任务中达到或超过监督基线性能。


<details>
  <summary>更多</summary>
  
**动机:** 需要从异构新闻文章中大规模提取结构化信息来理解和监测气候灾害的社会经济影响。

**方法:** 开发CienaLLM框架，使用开放权重大语言模型进行零样本信息提取，支持多步流水线、云或本地推理，并通过大规模因子研究评估LLM家族、规模、精度和提示策略对性能的影响。

**结果:** 响应解析步骤几乎消除了格式错误同时保持准确性；更大模型提供最强和最稳定的性能；量化提供显著效率提升但牺牲少量准确性；提示策略显示出异质性和模型特异性效应。在西班牙新闻干旱影响提取中，CienaLLM在准确性上匹配或超过监督基线，但推理成本更高。

**结论:** CienaLLM的模式驱动和模型无关设计适用于通过编辑提示和模式（而非重新训练）来适应相关信息提取任务，如其他灾害、部门或语言。代码、配置和模式已开源以支持可重复使用。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是CienaLLM%3A+Generative+Climate-Impact+Extraction+from+News+Articles+with+Autoregressive+LLMs，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19305，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19305&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Understanding and monitoring the socio-economic impacts of climate hazards requires extracting structured information from heterogeneous news articles on a large scale. To that end, we have developed CienaLLM, a modular framework based on schema-guided Generative Information Extraction. CienaLLM uses open-weight Large Language Models for zero-shot information extraction from news articles, and supports configurable prompts and output schemas, multi-step pipelines, and cloud or on-premise inference. To systematically assess how the choice of LLM family, size, precision regime, and prompting strategy affect performance, we run a large factorial study in models, precisions, and prompt engineering techniques. An additional response parsing step nearly eliminates format errors while preserving accuracy; larger models deliver the strongest and most stable performance, while quantization offers substantial efficiency gains with modest accuracy trade-offs; and prompt strategies show heterogeneous, model-specific effects. CienaLLM matches or outperforms the supervised baseline in accuracy for extracting drought impacts from Spanish news, although at a higher inference cost. While evaluated in droughts, the schema-driven and model-agnostic design is suitable for adapting to related information extraction tasks (e.g., other hazards, sectors, or languages) by editing prompts and schemas rather than retraining. We release code, configurations, and schemas to support reproducible use.

</details>


### [122] [HATS: High-Accuracy Triple-Set Watermarking for Large Language Models](https://arxiv.org/abs/2512.19378)
*Zhiqing Hu, Chenxu Zhao, Jiazhong Lu, Xiaolei Liu*

**主要类别:** cs.CL

**AI概要:** 提出一种基于三色分区的水印方法，通过限制采样范围在绿色和黄色词汇集中嵌入水印信号，使用Fisher方法聚合p值进行检测，在Llama 2 7B上验证了高检测准确率和文本质量保持。


<details>
  <summary>更多</summary>
  
**动机:** 为了防止LLM生成文本的滥用，需要开发能够嵌入隐式水印信号的技术来追踪和识别AI生成内容。

**方法:** 在每个解码步骤将词汇表划分为绿/黄/红三个固定比例集合，限制采样到绿色和黄色集合；检测时重放相同分区，计算绿色富集和红色贫化统计量，转换为单侧z分数，通过Fisher方法聚合p值判断水印存在。

**结果:** 在Llama 2 7B上的实验显示，三色分区方案在固定误报率下实现了高检测准确率，同时保持了文本的可读性。

**结论:** 该方法提供了一种有效的LLM文本水印技术，能够在保持生成质量的同时实现可靠的水印检测，有助于防止AI生成内容的滥用。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是HATS%3A+High-Accuracy+Triple-Set+Watermarking+for+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19378，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19378&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Misuse of LLM-generated text can be curbed by watermarking techniques that embed implicit signals into the output. We propose a watermark that partitions the vocabulary at each decoding step into three sets (Green/Yellow/Red) with fixed ratios and restricts sampling to the Green and Yellow sets. At detection time, we replay the same partitions, compute Green-enrichment and Red-depletion statistics, convert them to one-sided z-scores, and aggregate their p-values via Fisher's method to decide whether a passage is watermarked. We implement generation, detection, and testing on Llama 2 7B, and evaluate true-positive rate, false-positive rate, and text quality. Results show that the triple-partition scheme achieves high detection accuracy at fixed FPR while preserving readability.

</details>


### [123] [Kunnafonidilaw ka Cadeau: an ASR dataset of present-day Bambara](https://arxiv.org/abs/2512.19400)
*Yacouba Diarra, Panga Azazia Kamate, Nouhoum Souleymane Coulibaly, Michael Leventhal*

**主要类别:** cs.CL

**AI概要:** Kunkado是一个160小时的班巴拉语ASR数据集，包含真实场景中的自发语音特征，通过微调模型在两个测试集上显著降低了WER，并在人工评估中优于使用更干净数据的可比系统


<details>
  <summary>更多</summary>
  
**动机:** 为捕获真实世界中的自发语音特征（如语码转换、不流利、背景噪音和说话人重叠），支持主要口语语言的鲁棒ASR系统开发

**方法:** 从马里广播档案编译160小时数据集，使用33.47小时人工审核子集微调Parakeet模型，应用实用的转录规范化处理数字格式、标签和语码转换注释的变异性

**结果:** 在两个真实测试集上，WER分别从44.47%降至37.12%和从36.07%降至32.33%；人工评估显示该模型优于使用98小时更干净但不太真实语音训练的相同架构系统

**结论:** Kunkado数据集和模型支持主要口语语言的鲁棒ASR发展，数据集和模型已公开发布

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Kunnafonidilaw+ka+Cadeau%3A+an+ASR+dataset+of+present-day+Bambara，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19400，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19400&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present Kunkado, a 160-hour Bambara ASR dataset compiled from Malian radio archives to capture present-day spontaneous speech across a wide range of topics. It includes code-switching, disfluencies, background noise, and overlapping speakers that practical ASR systems encounter in real-world use. We finetuned Parakeet-based models on a 33.47-hour human-reviewed subset and apply pragmatic transcript normalization to reduce variability in number formatting, tags, and code-switching annotations. Evaluated on two real-world test sets, finetuning with Kunkado reduces WER from 44.47\% to 37.12\% on one and from 36.07\% to 32.33\% on the other. In human evaluation, the resulting model also outperforms a comparable system with the same architecture trained on 98 hours of cleaner, less realistic speech. We release the data and models to support robust ASR for predominantly oral languages.

</details>


### [124] [CodeSimpleQA: Scaling Factuality in Code Large Language Models](https://arxiv.org/abs/2512.19424)
*Jian Yang, Wei Zhang, Yizhi Li, Shawn Guo, Haowen Wang, Aishan Liu, Ge Zhang, Zili Wang, Zhoujun Li, Xianglong Liu, Weifeng Lv*

**主要类别:** cs.CL

**AI概要:** CodeSimpleQA是一个双语基准测试，用于评估代码大语言模型在编程知识事实准确性方面的表现，并提出了结合监督微调和强化学习的后训练框架来提升模型的事实准确性。


<details>
  <summary>更多</summary>
  
**动机:** 现有代码生成基准主要关注代码执行正确性，而忽略了编程知识的事实准确性，这是一个关键挑战。

**方法:** 创建CodeSimpleQA双语基准测试（英文和中文），包含精心策划的问答对；构建CodeSimpleQA-Instruct指令语料库（6600万样本）；开发结合监督微调和强化学习的后训练框架。

**结果:** 评估发现即使前沿的LLM在代码事实性方面仍存在困难；提出的框架相比基础模型有显著改进。

**结论:** 事实感知对齐在开发可靠的代码LLM中具有关键重要性，需要专门关注编程知识的事实准确性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是CodeSimpleQA%3A+Scaling+Factuality+in+Code+Large+Language+Models，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19424，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19424&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) have made significant strides in code generation, achieving impressive capabilities in synthesizing code snippets from natural language instructions. However, a critical challenge remains in ensuring LLMs generate factually accurate responses about programming concepts, technical implementations, etc. Most previous code-related benchmarks focus on code execution correctness, overlooking the factual accuracy of programming knowledge. To address this gap, we present CodeSimpleQA, a comprehensive bilingual benchmark designed to evaluate the factual accuracy of code LLMs in answering code-related questions, which contains carefully curated question-answer pairs in both English and Chinese, covering diverse programming languages and major computer science domains. Further, we create CodeSimpleQA-Instruct, a large-scale instruction corpus with 66M samples, and develop a post-training framework combining supervised fine-tuning and reinforcement learning. Our comprehensive evaluation of diverse LLMs reveals that even frontier LLMs struggle with code factuality. Our proposed framework demonstrates substantial improvements over the base model, underscoring the critical importance of factuality-aware alignment in developing reliable code LLMs.

</details>


### [125] [MobileWorld: Benchmarking Autonomous Mobile Agents in Agent-User Interactive, and MCP-Augmented Environments](https://arxiv.org/abs/2512.19432)
*Quyu Kong, Xu Zhang, Zhenyu Yang, Nolan Gao, Chen Liu, Panrong Tong, Chenglin Cai, Hanzhang Zhou, Jianan Zhang, Liangyu Chen, Zhidan Liu, Steven Hoi, Yue Wang*

**主要类别:** cs.CL

**AI概要:** MobileWorld是一个新的移动应用基准测试，比现有的AndroidWorld更具挑战性，包含跨应用交互、用户交互和MCP增强任务，测试结果显示当前AI模型在这些复杂场景中表现显著下降。


<details>
  <summary>更多</summary>
  
**动机:** 现有AndroidWorld基准测试已达到饱和（成功率超过90%），缺乏电商和企业通信等关键应用类别，且无法反映真实移动使用场景中的模糊用户指令和混合工具使用需求。

**方法:** 开发MobileWorld基准测试，包含201个任务覆盖20个应用，强调跨应用长流程任务（平均27.8步）和多应用任务（62.2%）。引入新的任务类别包括用户交互和MCP增强任务，提供基于快照的容器环境和精确功能验证。

**结果:** 相比AndroidWorld，性能显著下降：最佳智能体框架成功率51.7%，端到端模型仅20.9%。当前模型在用户交互和MCP调用方面表现困难。

**结论:** MobileWorld为下一代移动智能提供了更具挑战性的测试平台，揭示了当前模型的局限性，特别是在用户交互和MCP调用方面，为未来发展指明了方向。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是MobileWorld%3A+Benchmarking+Autonomous+Mobile+Agents+in+Agent-User+Interactive%2C+and+MCP-Augmented+Environments，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19432，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19432&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Among existing online mobile-use benchmarks, AndroidWorld has emerged as the dominant benchmark due to its reproducible environment and deterministic evaluation; however, recent agents achieving over 90% success rates indicate its saturation and motivate the need for a more challenging benchmark. In addition, its environment lacks key application categories, such as e-commerce and enterprise communication, and does not reflect realistic mobile-use scenarios characterized by vague user instructions and hybrid tool usage. To bridge this gap, we introduce MobileWorld, a substantially more challenging benchmark designed to better reflect real-world mobile usage, comprising 201 tasks across 20 applications, while maintaining the same level of reproducible evaluation as AndroidWorld. The difficulty of MobileWorld is twofold. First, it emphasizes long-horizon tasks with cross-application interactions: MobileWorld requires nearly twice as many task-completion steps on average (27.8 vs. 14.3) and includes far more multi-application tasks (62.2% vs. 9.5%) compared to AndroidWorld. Second, MobileWorld extends beyond standard GUI manipulation by introducing novel task categories, including agent-user interaction and MCP-augmented tasks. To ensure robust evaluation, we provide snapshot-based container environment and precise functional verifications, including backend database inspection and task callback APIs. We further develop a planner-executor agentic framework with extended action spaces to support user interactions and MCP calls. Our results reveal a sharp performance drop compared to AndroidWorld, with the best agentic framework and end-to-end model achieving 51.7% and 20.9% success rates, respectively. Our analysis shows that current models struggle significantly with user interaction and MCP calls, offering a strategic roadmap toward more robust, next-generation mobile intelligence.

</details>


### [126] [SiamGPT: Quality-First Fine-Tuning for Stable Thai Text Generation](https://arxiv.org/abs/2512.19455)
*Thittipat Pairatsuppawat, Abhibhu Tachaapornchai, Paweekorn Kusolsomboon, Chutikan Chaiwong, Thodsaporn Chay-intr, Kobkrit Viriyayudhakorn, Nongnuch Ketui, Aslan B. Wong*

**主要类别:** cs.CL

**AI概要:** SiamGPT-32B是基于Qwen3-32B的泰语开放权重模型，通过质量优先策略和精心策划的监督数据进行微调，显著提升了泰语复杂指令下的生成稳定性和多轮对话鲁棒性。


<details>
  <summary>更多</summary>
  
**动机:** 现有的开放权重大语言模型在英语表现良好，但在泰语复杂指令下生成不稳定，难以部署使用。

**方法:** 基于Qwen3-32B进行监督微调，采用质量优先策略，结合翻译的高复杂度英语指令数据和泰语适配的AutoIF框架，强调监督质量而非数据规模。

**结果:** 在SEA-HELM基准测试中，SiamGPT-32B在同等规模开放权重泰语模型中表现最佳，在指令遵循、多轮对话和自然语言理解方面均有稳定提升。

**结论:** 通过精心设计的质量优先微调策略，无需持续预训练或语料扩展，即可显著提升泰语大语言模型的指令遵循能力和生成稳定性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是SiamGPT%3A+Quality-First+Fine-Tuning+for+Stable+Thai+Text+Generation，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19455，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19455&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Open-weights large language models remain difficult to deploy for Thai due to unstable generation under complex instructions, despite strong English performance. To mitigate these limitations, We present SiamGPT-32B, an open-weights model based on Qwen3-32B, fine-tuned with a Quality-First strategy emphasizing curated supervision over data scale. The fine-tuning pipeline combines translated high-complexity English instruction data with a Thai-adapted AutoIF framework for instruction and linguistic constraints. Using supervised fine-tuning only, without continual pretraining or corpus expansion, SiamGPT-32B improves instruction adherence, multi-turn robustness, and linguistic stability. Evaluations on the SEA-HELM benchmark show that SiamGPT-32B achieves the strongest overall performance among similar-scale open-weights Thai models, with consistent gains in instruction following, multi-turn dialogue, and natural language understanding.

</details>


### [127] [Activations as Features: Probing LLMs for Generalizable Essay Scoring Representations](https://arxiv.org/abs/2512.19456)
*Jinwei Chi, Ke Wang, Yu Chen, Xuanye Lin, Qiang Xu*

**主要类别:** cs.CL

**AI概要:** 本文研究大型语言模型中间层激活在跨提示作文评分中的判别能力，发现激活信息具有强大的评分判别力，LLMs能根据不同作文类型和评分维度自适应调整评估视角。


<details>
  <summary>更多</summary>
  
**动机:** 跨提示自动作文评分面临评分标准多样性的挑战，现有研究主要关注LLMs的输出，但中间层激活可能包含有价值的评分信息。

**方法:** 使用LLMs中间层激活拟合探针，分析不同模型和输入内容对判别能力的影响，通过计算不同提示下各特征维度的作文方向来分析评估视角变化。

**结果:** 激活信息在评估作文质量方面具有强判别能力，LLMs能够根据不同特征和作文类型调整评估视角，有效处理跨提示设置的评分标准多样性。

**结论:** LLMs中间层激活是跨提示作文评分的有价值信息源，模型能够自适应地处理不同评分标准，为AES任务提供了新的研究方向。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Activations+as+Features%3A+Probing+LLMs+for+Generalizable+Essay+Scoring+Representations，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19456，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19456&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Automated essay scoring (AES) is a challenging task in cross-prompt settings due to the diversity of scoring criteria. While previous studies have focused on the output of large language models (LLMs) to improve scoring accuracy, we believe activations from intermediate layers may also provide valuable information. To explore this possibility, we evaluated the discriminative power of LLMs' activations in cross-prompt essay scoring task. Specifically, we used activations to fit probes and further analyzed the effects of different models and input content of LLMs on this discriminative power. By computing the directions of essays across various trait dimensions under different prompts, we analyzed the variation in evaluation perspectives of large language models concerning essay types and traits. Results show that the activations possess strong discriminative power in evaluating essay quality and that LLMs can adapt their evaluation perspectives to different traits and essay types, effectively handling the diversity of scoring criteria in cross-prompt settings.

</details>


### [128] [A Large-Language-Model Framework for Automated Humanitarian Situation Reporting](https://arxiv.org/abs/2512.19475)
*Ivan Decostanzi, Yelena Mejova, Kyriaki Kalimeri*

**主要类别:** cs.CL

**AI概要:** 使用大语言模型自动将异构人道主义文档转化为结构化、有证据支持的报告框架，通过多步骤处理流程实现高质量的人道主义形势报告自动生成


<details>
  <summary>更多</summary>
  
**动机:** 当前人道主义决策依赖的手工报告流程效率低下、资源密集且不一致，需要自动化解决方案来提供及时准确的情势报告

**方法:** 整合语义文本聚类、自动问题生成、检索增强的答案提取与引用、多级摘要生成和行政摘要生成，采用模拟专家推理的内部评估指标

**结果:** 在13个人道主义事件中评估，生成问题相关性84.7%、重要性84.0%、紧急性76.4%；答案提取相关性86.3%，引用精确率和召回率均超过76%；人机评估一致性F1分数超过0.80

**结论:** 通过结合LLM推理、透明引用链接和多级评估，生成式AI能够自主产生准确、可验证且具有操作价值的人道主义形势报告，优于现有基线方法

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是A+Large-Language-Model+Framework+for+Automated+Humanitarian+Situation+Reporting，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19475，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19475&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Timely and accurate situational reports are essential for humanitarian decision-making, yet current workflows remain largely manual, resource intensive, and inconsistent. We present a fully automated framework that uses large language models (LLMs) to transform heterogeneous humanitarian documents into structured and evidence-grounded reports. The system integrates semantic text clustering, automatic question generation, retrieval augmented answer extraction with citations, multi-level summarization, and executive summary generation, supported by internal evaluation metrics that emulate expert reasoning. We evaluated the framework across 13 humanitarian events, including natural disasters and conflicts, using more than 1,100 documents from verified sources such as ReliefWeb. The generated questions achieved 84.7 percent relevance, 84.0 percent importance, and 76.4 percent urgency. The extracted answers reached 86.3 percent relevance, with citation precision and recall both exceeding 76 percent. Agreement between human and LLM based evaluations surpassed an F1 score of 0.80. Comparative analysis shows that the proposed framework produces reports that are more structured, interpretable, and actionable than existing baselines. By combining LLM reasoning with transparent citation linking and multi-level evaluation, this study demonstrates that generative AI can autonomously produce accurate, verifiable, and operationally useful humanitarian situation reports.

</details>


### [129] [Event Extraction in Large Language Model](https://arxiv.org/abs/2512.19537)
*Bobo Li, Xudong Han, Jiang Liu, Yuzhe Ding, Liqiang Jing, Zhaoqi Zhang, Jinheng Li, Xinya Du, Fei Li, Meishan Zhang, Min Zhang, Aixin Sun, Philip S. Yu, Hao Fei*

**主要类别:** cs.CL

**AI概要:** 这篇综述论文探讨了大型语言模型和多模态语言模型在事件抽取领域的应用、挑战和未来方向，提出了将事件抽取作为认知支架的系统化观点。


<details>
  <summary>更多</summary>
  
**动机:** LLM在事件抽取中面临幻觉、长上下文处理脆弱性、有限知识管理等部署差距，需要系统化的解决方案来提升可靠性。

**方法:** 通过事件模式和槽约束提供接地验证接口，使用事件中心结构作为逐步推理的中间表示，基于图RAG实现关系感知检索，利用事件存储扩展上下文窗口外的记忆能力。

**结果:** 系统梳理了文本和多模态环境下的事件抽取任务分类、方法演进（从规则到生成式框架）、解码策略、架构、数据集和评估方法。

**结论:** 事件抽取需要从静态抽取发展为结构可靠、支持智能体的感知和记忆层，为开放世界系统提供认知支撑，这是LLM时代的关键发展方向。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Event+Extraction+in+Large+Language+Model，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19537，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19537&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Large language models (LLMs) and multimodal LLMs are changing event extraction (EE): prompting and generation can often produce structured outputs in zero shot or few shot settings. Yet LLM based pipelines face deployment gaps, including hallucinations under weak constraints, fragile temporal and causal linking over long contexts and across documents, and limited long horizon knowledge management within a bounded context window. We argue that EE should be viewed as a system component that provides a cognitive scaffold for LLM centered solutions. Event schemas and slot constraints create interfaces for grounding and verification; event centric structures act as controlled intermediate representations for stepwise reasoning; event links support relation aware retrieval with graph based RAG; and event stores offer updatable episodic and agent memory beyond the context window. This survey covers EE in text and multimodal settings, organizing tasks and taxonomy, tracing method evolution from rule based and neural models to instruction driven and generative frameworks, and summarizing formulations, decoding strategies, architectures, representations, datasets, and evaluation. We also review cross lingual, low resource, and domain specific settings, and highlight open challenges and future directions for reliable event centric systems. Finally, we outline open challenges and future directions that are central to the LLM era, aiming to evolve EE from static extraction into a structurally reliable, agent ready perception and memory layer for open world systems.

</details>


### [130] [Algerian Dialect](https://arxiv.org/abs/2512.19543)
*Zakaria Benmounah, Abdennour Boulesnane*

**主要类别:** cs.CL

**AI概要:** 阿尔及利亚方言情感分析数据集：包含4.5万条YouTube评论，手动标注为5种情感类别，支持方言阿拉伯语NLP研究


<details>
  <summary>更多</summary>
  
**动机:** 解决阿尔及利亚方言公开资源的稀缺问题，支持情感分析、方言阿拉伯语自然语言处理和社交媒体分析研究

**方法:** 通过YouTube Data API从30多个阿尔及利亚媒体频道收集评论，每条评论手动标注为5种情感类别（非常负面、负面、中性、正面、非常正面），并包含丰富元数据

**结果:** 创建了包含45,000条标注评论的大规模数据集，包含时间戳、点赞数、视频URL等元数据，在Mendeley Data公开可用

**结论:** 该数据集填补了阿尔及利亚方言资源的空白，为相关领域研究提供了重要基础资源，采用CC BY 4.0许可证开放共享

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Algerian+Dialect，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19543，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19543&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present Algerian Dialect, a large-scale sentiment-annotated dataset consisting of 45,000 YouTube comments written in Algerian Arabic dialect. The comments were collected from more than 30 Algerian press and media channels using the YouTube Data API. Each comment is manually annotated into one of five sentiment categories: very negative, negative, neutral, positive, and very positive. In addition to sentiment labels, the dataset includes rich metadata such as collection timestamps, like counts, video URLs, and annotation dates. This dataset addresses the scarcity of publicly available resources for Algerian dialect and aims to support research in sentiment analysis, dialectal Arabic NLP, and social media analytics. The dataset is publicly available on Mendeley Data under a CC BY 4.0 license at https://doi.org/10.17632/zzwg3nnhsz.2.

</details>


### [131] [Increasing the Thinking Budget is Not All You Need](https://arxiv.org/abs/2512.19585)
*Ignacio Iacobacci, Zhaozhi Qian, Faroq AL-Tam, Muhammad AL-Qurishi, Riad Souissi*

**主要类别:** cs.CL

**AI概要:** 研究表明，单纯增加思维预算（推理过程长度）并非最有效的计算资源利用方式，通过自洽性和自我反思等替代配置可以获得更准确的结果。


<details>
  <summary>更多</summary>
  
**动机:** 探索思维预算作为关键参数如何与自洽性、反思等配置相互作用，提供兼顾性能结果和计算成本的平衡比较框架。

**方法:** 系统研究思维预算参数，分析其与不同配置（如自洽性、反思等）的交互作用，建立性能与计算成本的比较框架。

**结果:** 发现单纯增加思维预算不是最有效的计算利用方式，替代配置如自洽性和自我反思能获得更准确的响应。

**结论:** 思维预算的有效利用需要结合其他配置策略，单纯延长推理过程并非最优选择，自洽性和反思等机制能更高效地提升模型性能。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Increasing+the+Thinking+Budget+is+Not+All+You+Need，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19585，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19585&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Recently, a new wave of thinking-capable Large Language Models has emerged, demonstrating exceptional capabilities across a wide range of reasoning benchmarks. Early studies have begun to explore how the amount of compute in terms of the length of the reasoning process, the so-called thinking budget, impacts model performance. In this work, we propose a systematic investigation of the thinking budget as a key parameter, examining its interaction with various configurations such as self-consistency, reflection, and others. Our goal is to provide an informative, balanced comparison framework that considers both performance outcomes and computational cost. Among our findings, we discovered that simply increasing the thinking budget is not the most effective use of compute. More accurate responses can instead be achieved through alternative configurations, such as self-consistency and self-reflection.

</details>


### [132] [MauBERT: Universal Phonetic Inductive Biases for Few-Shot Acoustic Units Discovery](https://arxiv.org/abs/2512.19612)
*Angelo Ortiz Tandazo, Manel Khentout, Youssef Benchekroun, Thomas Hueber, Emmanuel Dupoux*

**主要类别:** cs.CL

**AI概要:** MauBERT是基于HuBERT的多语言扩展模型，通过发音特征映射在55种语言上进行预训练，学习预测发音特征或音素，生成语言无关的语音表征，在跨语言语音表示学习方面表现出色。


<details>
  <summary>更多</summary>
  
**动机:** 开发一个能够学习跨语言鲁棒语音表征的多语言自监督学习模型，通过发音特征来捕捉多语言语音特性，提高模型的泛化能力。

**方法:** 在HuBERT基础上继续预训练，使用基于语音到发音特征映射的监督信号，在55种语言的多语言数据上训练模型预测发音特征或音素。

**结果:** 通过全面的ABX可区分性测试显示，MauBERT模型比最先进的多语言自监督学习模型产生更上下文无关的表征，且能有效适应未见语言和日常语音，仅需少量自监督微调（10小时语音）。

**结论:** MauBERT为自监督语音模型中注入语言归纳偏置提供了一种有效方法，实现了跨语言的鲁棒语音表示学习。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是MauBERT%3A+Universal+Phonetic+Inductive+Biases+for+Few-Shot+Acoustic+Units+Discovery，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19612，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19612&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** This paper introduces MauBERT, a multilingual extension of HuBERT that leverages articulatory features for robust cross-lingual phonetic representation learning. We continue HuBERT pre-training with supervision based on a phonetic-to-articulatory feature mapping in 55 languages. Our models learn from multilingual data to predict articulatory features or phones, resulting in language-independent representations that capture multilingual phonetic properties. Through comprehensive ABX discriminability testing, we show MauBERT models produce more context-invariant representations than state-of-the-art multilingual self-supervised learning models. Additionally, the models effectively adapt to unseen languages and casual speech with minimal self-supervised fine-tuning (10 hours of speech). This establishes an effective approach for instilling linguistic inductive biases in self-supervised speech models.

</details>


### [133] [Exploring the features used for summary evaluation by Human and GPT](https://arxiv.org/abs/2512.19620)
*Zahra Sadeghi, Evangelos Milios, Frank Rudzicz*

**主要类别:** cs.CL

**AI概要:** 本研究探讨了大型语言模型在摘要评估中如何利用特定特征进行评分，通过统计分析发现与人类评估对齐的特征，并证明通过指导GPT使用人类评估指标可以提升其与人类判断的一致性。


<details>
  <summary>更多</summary>
  
**动机:** 虽然已有研究探索LLMs与人类评估的对齐，但尚不清楚LLMs在基于特定质量维度评估摘要时利用了哪些特征，且缺乏评估分数与指标之间的映射研究。

**方法:** 通过研究统计和机器学习指标，发现与人类和GPT响应对齐的特征，并指导GPT使用人类采用的评估指标。

**结果:** 发现了与人类和GPT响应对齐的特征，证明指示GPT使用人类评估指标可以改善其判断，使其更符合人类响应。

**结论:** 通过理解LLMs在摘要评估中使用的特征并引导其采用人类评估指标，可以显著提升自动化摘要评估系统与人类判断的一致性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Exploring+the+features+used+for+summary+evaluation+by+Human+and+GPT，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19620，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19620&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Summary assessment involves evaluating how well a generated summary reflects the key ideas and meaning of the source text, requiring a deep understanding of the content. Large Language Models (LLMs) have been used to automate this process, acting as judges to evaluate summaries with respect to the original text. While previous research investigated the alignment between LLMs and Human responses, it is not yet well understood what properties or features are exploited by them when asked to evaluate based on a particular quality dimension, and there has not been much attention towards mapping between evaluation scores and metrics. In this paper, we address this issue and discover features aligned with Human and Generative Pre-trained Transformers (GPTs) responses by studying statistical and machine learning metrics. Furthermore, we show that instructing GPTs to employ metrics used by Human can improve their judgment and conforming them better with human responses.

</details>


### [134] [Diacritic Restoration for Low-Resource Indigenous Languages: Case Study with Bribri and Cook Islands Māori](https://arxiv.org/abs/2512.19630)
*Rolando Coto-Solano, Daisy Li, Manoela Teleginski Ferraz, Olivia Sasse, Cha Krupka, Sharid Loáiciga, Sally Akevai Tenamu Nicholas*

**主要类别:** cs.CL

**AI概要:** 该论文研究了两种极度资源匮乏语言（Bribri和Cook Islands Māori）的变音符号恢复任务，比较了不同算法性能，发现微调后的字符级大语言模型表现最佳，需要约10,000词的数据量才能达到可靠性能。


<details>
  <summary>更多</summary>
  
**动机:** 响应语言社区的需求并解决自然语言处理中关于资源匮乏语境下模型性能和泛化能力的研究问题，特别是针对变音符号恢复这一文本规范化任务。

**方法:** 比较了多种变音符号恢复算法，包括字符级LLM微调、大规模多语言模型和零样本方法，在不同资源条件下进行实验，并分析了所需数据量。

**结果:** 微调的字符级LLM表现最好，能够将复杂字符分解为UTF-8字节表示；大规模多语言模型在数据受限时效果较差；所有模型需要约10,000词数据才能达到可靠性能；零样本方法在所有情况下表现都很差。

**结论:** 对于资源匮乏语言的变音符号恢复任务，字符级微调方法是最有效的解决方案，且需要足够的数据量支持，这为类似语言的NLP处理提供了实用指导。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Diacritic+Restoration+for+Low-Resource+Indigenous+Languages%3A+Case+Study+with+Bribri+and+Cook+Islands+M%C4%81ori，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19630，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19630&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** We present experiments on diacritic restoration, a form of text normalization essential for natural language processing (NLP) tasks. Our study focuses on two extremely under-resourced languages: Bribri, a Chibchan language spoken in Costa Rica, and Cook Islands Māori, a Polynesian language spoken in the Cook Islands. Specifically, this paper: (i) compares algorithms for diacritics restoration in under-resourced languages, including tonal diacritics, (ii) examines the amount of data required to achieve target performance levels, (iii) contrasts results across varying resource conditions, and (iv) explores the related task of diacritic correction. We find that fine-tuned, character-level LLMs perform best, likely due to their ability to decompose complex characters into their UTF-8 byte representations. In contrast, massively multilingual models perform less effectively given our data constraints. Across all models, reliable performance begins to emerge with data budgets of around 10,000 words. Zero-shot approaches perform poorly in all cases. This study responds both to requests from the language communities and to broader NLP research questions concerning model performance and generalization in under-resourced contexts.

</details>


### [135] [Exploring Zero-Shot ACSA with Unified Meaning Representation in Chain-of-Thought Prompting](https://arxiv.org/abs/2512.19651)
*Filippos Ventirozos, Peter Appleby, Matthew Shardlow*

**主要类别:** cs.CL

**AI概要:** 本文提出了一种基于统一意义表示(UMR)的思维链提示技术，用于零样本场景下的方面类别情感分析，相比标准CoT方法在不同模型上表现效果存在模型依赖性。


<details>
  <summary>更多</summary>
  
**动机:** 监督学习方法在ACSA任务中需要大量标注数据，成本高昂且新领域数据稀缺，因此探索大语言模型在零样本设置下的应用具有实用价值。

**方法:** 使用基于统一意义表示(UMR)的思维链提示技术来结构化ACSA任务中的推理过程，并在三个模型(Qwen3-4B、Qwen3-8B和Gemini-2.5-Pro)和四个数据集上进行评估。

**结果:** UMR方法的有效性可能依赖于具体模型。中等规模模型如Qwen3-8B表现出与基准相当的性能，但需要进一步研究该方法在小模型架构上的适用性。

**结论:** UMR-based CoT方法在零样本ACSA任务中显示出潜力，但其效果具有模型依赖性，需要进一步研究来验证在不同规模模型上的普适性。

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是Exploring+Zero-Shot+ACSA+with+Unified+Meaning+Representation+in+Chain-of-Thought+Prompting，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19651，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19651&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Aspect-Category Sentiment Analysis (ACSA) provides granular insights by identifying specific themes within reviews and their associated sentiment. While supervised learning approaches dominate this field, the scarcity and high cost of annotated data for new domains present significant barriers. We argue that leveraging large language models (LLMs) in a zero-shot setting is a practical alternative where resources for data annotation are limited. In this work, we propose a novel Chain-of-Thought (CoT) prompting technique that utilises an intermediate Unified Meaning Representation (UMR) to structure the reasoning process for the ACSA task. We evaluate this UMR-based approach against a standard CoT baseline across three models (Qwen3-4B, Qwen3-8B, and Gemini-2.5-Pro) and four diverse datasets. Our findings suggest that UMR effectiveness may be model-dependent. Whilst preliminary results indicate comparable performance for mid-sized models such as Qwen3-8B, these observations warrant further investigation, particularly regarding the potential applicability to smaller model architectures. Further research is required to establish the generalisability of these findings across different model scales.

</details>


### [136] [GenEnv: Difficulty-Aligned Co-Evolution Between LLM Agents and Environment Simulators](https://arxiv.org/abs/2512.19682)
*Jiacheng Guo, Ling Yang, Peter Chen, Qixin Xiao, Yinjie Wang, Xinzhe Juan, Jiahao Qiu, Ke Shen, Mengdi Wang*

**主要类别:** cs.CL

**AI概要:** GenEnv框架通过动态生成环境模拟器和智能体共同进化，使用难度对齐的课程奖励机制，在减少数据需求的同时显著提升LLM智能体性能


<details>
  <summary>更多</summary>
  
**动机:** 解决大型语言模型智能体训练中真实世界交互数据成本高、静态性强的瓶颈问题

**方法:** 建立智能体与可扩展生成环境模拟器之间的难度对齐协同进化游戏，使用α-Curriculum Reward动态调整任务难度以适应智能体当前能力水平

**结果:** 在五个基准测试中，GenEnv相比7B基线模型性能提升最高达40.3%，与更大模型性能相当或更好，相比Gemini 2.5 Pro的离线数据增强方法，使用数据量减少3.3倍但性能更优

**结论:** 通过从静态监督转向自适应模拟，GenEnv为扩展智能体能力提供了数据高效的途径

**与AI讨论:** [Discuss with Kimi](https://kimi.moonshot.cn/_prefill_chat?prefill_prompt=我们要讨论的论文是GenEnv%3A+Difficulty-Aligned+Co-Evolution+Between+LLM+Agents+and+Environment+Simulators，链接是https%3A%2F%2Farxiv.org%2Fabs%2F2512.19682，已有的FAQ链接是https://papers.cool/arxiv/kimi?paper=2512.19682&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false)

**原文摘要:** Training capable Large Language Model (LLM) agents is critically bottlenecked by the high cost and static nature of real-world interaction data. We address this by introducing GenEnv, a framework that establishes a difficulty-aligned co-evolutionary game between an agent and a scalable, generative environment simulator. Unlike traditional methods that evolve models on static datasets, GenEnv instantiates a dataevolving: the simulator acts as a dynamic curriculum policy, continuously generating tasks specifically tailored to the agent's ``zone of proximal development''. This process is guided by a simple but effective $α$-Curriculum Reward, which aligns task difficulty with the agent's current capabilities. We evaluate GenEnv on five benchmarks, including API-Bank, ALFWorld, BFCL, Bamboogle, and TravelPlanner. Across these tasks, GenEnv improves agent performance by up to \textbf{+40.3\%} over 7B baselines and matches or exceeds the average performance of larger models. Compared to Gemini 2.5 Pro-based offline data augmentation, GenEnv achieves better performance while using 3.3$\times$ less data. By shifting from static supervision to adaptive simulation, GenEnv provides a data-efficient pathway for scaling agent capabilities.

</details>
