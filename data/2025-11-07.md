...
**AI概要:** DreamGym是一个统一的强化学习框架，通过基于推理的经验模型合成多样化经验数据，解决了RL训练中成本高、任务多样性有限、奖励信号不可靠等问题，显著提升了自主代理的训练效果。


<details>
  <summary>更多</summary>
  
**动机:** 传统强化学习在大语言模型代理应用中面临成本高昂的rollout、任务多样性有限、奖励信号不可靠和基础设施复杂等挑战，阻碍了可扩展经验数据的收集。

**方法:** DreamGym将环境动态提炼为基于推理的经验模型，通过逐步推理获得一致的状态转换和反馈信号；利用离线真实数据初始化经验回放缓冲区并持续丰富；自适应生成新任务挑战当前策略实现在线课程学习。

**结果:** 在多样化环境和代理架构上，DreamGym显著提升了RL训练效果。在WebArena等非RL就绪任务上超越所有基线30%以上；在RL就绪但成本高的设置中，仅使用合成交互就能匹配GRPO和PPO性能；在从合成经验到真实环境的策略迁移中，获得显著性能提升且需要更少的真实交互。

**结论:** DreamGym提供了一个可扩展的通用RL预热策略，通过合成经验数据有效解决了RL训练中的可扩展性和成本问题，为自主代理的自我改进提供了实用解决方案。
...
**AI概要:** 本研究评估了NLP分词模型在汇编代码分析中的内在特性和参数选择，包括词汇量大小、预处理定制和预分词规则，并分析了它们对下游任务（如函数签名预测）的影响。研究发现分词器选择显著影响性能，内在指标只能部分预测外在评估结果。


<details>
  <summary>更多</summary>
  
**动机:** 汇编代码分析中的分词是一个未被充分探索但至关重要的领域，它影响词汇大小、语义覆盖和下游任务性能。本研究旨在填补这一空白，评估NLP分词模型在汇编代码中的适用性。

**方法:** 使用多种分词模型进行系统研究，包括Llama 3.2、BERT和BART等预训练模型。通过内在评估比较分词效率、词汇压缩和表示保真度，并在下游任务中评估性能。

**结果:** 初步发现表明分词器选择对下游性能有显著影响，内在指标只能部分预测外在评估结果，揭示了内在分词器特性与实际应用之间的复杂权衡。

**结论:** 本研究为低级代码分析中的分词模型优化提供了宝贵见解，有助于提高基于自然语言模型的二进制分析工作流程的鲁棒性和可扩展性。
...
**AI概要:** BehaviorLens框架通过比较文本、散点图和流程图三种用户行为数据表示方式，发现图像表示相比文本表示能将MLLM的下一次购买预测准确率提升87.5%，且无需额外计算成本。


<details>
  <summary>更多</summary>
  
**动机:** 探索多模态大语言模型在用户行为推理中，文本和图像两种数据表示方式哪种更有效，这一问题尚未得到充分研究。

**方法:** 开发BehaviorLens基准测试框架，在六个MLLM上评估三种用户交易数据表示方式（文本段落、散点图、流程图），使用真实购买序列数据集进行测试。

**结果:** 当数据以图像形式表示时，MLLM的下一次购买预测准确率相比等效文本表示提高了87.5%。

**结论:** 图像表示在用户行为推理任务中显著优于文本表示，为MLLM应用提供了更有效的数据表示策略。
...
**AI概要:** KnowThyself是一个基于聊天的LLM可解释性工具，通过自然语言交互和可视化降低技术门槛，提供模型分析的可访问平台


<details>
  <summary>更多</summary>
  
**动机:** 现有LLM可解释性工具分散且代码密集型，需要整合成更易用的解决方案

**方法:** 使用编排器LLM重新表述用户查询，通过代理路由器分发到专用模块，最后将输出整合为连贯解释

**结果:** 开发了统一的聊天界面工具，支持模型上传、自然语言提问和交互式可视化

**结论:** KnowThyself为LLM可解释性提供了易用且可扩展的平台，显著降低了技术障碍
...
**AI概要:** 本研究挑战了关于深度知识追踪(DKT)模型的主流解释，通过实验证明DKT的优势在于其隐式建模先决条件因果关系的能力，而非双向关系建模。


<details>
  <summary>更多</summary>
  
**动机:** 挑战现有文献中关于DKT性能提升源于双向知识组件关系建模的解释，探索DKT真正的工作原理。

**方法:** 通过将练习关系图修剪为有向无环图(DAG)，在Assistments数据集上训练DKT，并分析其预测能力与因果结构的一致性。提出基于DKT学习表示提取练习关系DAG的替代方法。

**结果:** 实验表明DKT的预测能力与因果结构高度一致，支持DKT通过近似知识组件间的因果依赖关系而非简单关系映射来提升性能的观点。

**结论:** DKT的有效性主要源于其建模知识组件间因果依赖关系的能力，这为开发更可解释的知识追踪模型提供了新视角。
...
**AI概要:** 研究发现大语言模型对提示语言和文化框架有响应，但存在系统性文化偏见，倾向于荷兰、德国、美国和日本的价值观，无法充分代表全球文化多样性


<details>
  <summary>更多</summary>
  
**动机:** 研究大语言模型是否能代表其广泛用户群体的文化多样性，考察提示语言和文化框架如何影响模型输出与不同国家人类价值观的匹配度

**方法:** 使用来自霍夫斯泰德价值观调查模块和世界价值观调查的63个项目，翻译成11种语言，以带有和不带有明确文化视角的提示形式测试10个大语言模型

**结果:** 提示语言和文化视角都会导致LLM输出变化，但所有模型都显示出对荷兰、德国、美国和日本价值观的系统性偏见；明确的文视角比针对性提示语言更能改善与人类价值观的匹配度；两种方法结合并不比英文提示加文化框架更有效

**结论:** LLMs处于尴尬的中间地带：对提示变化足够敏感以产生变化，但又过于固着于特定文化默认值，无法充分代表文化多样性
...
**AI概要:** ArchPilot是一个多智能体系统，通过集成架构生成、代理评估和自适应搜索来解决LLM-based agents在自动机器学习中计算开销大的问题，减少对完整训练运行的依赖。


<details>
  <summary>更多</summary>
  
**动机:** 现有的LLM-based agents在自动机器学习中严重依赖重复的完整训练运行来评估候选解决方案，导致计算开销大、可扩展性有限和迭代周期慢的问题。

**方法:** ArchPilot采用三智能体协作框架：1）协调智能体使用MCTS启发算法管理搜索过程；2）生成智能体迭代生成和改进架构；3）评估智能体执行代理训练并生成代理函数。

**结果:** 在MLE-Bench上的实验表明，ArchPilot优于AIDE和ML-Master等最先进基线方法。

**结论:** ArchPilot通过多智能体协作有效减少了计算开销，在有限预算下实现了高效的机器学习工程。
...
**AI概要:** 该论文提出了多智能体AI系统中的异常检测任务，通过构建包含4275和894条轨迹的数据集，展示了监督学习和半监督学习方法在检测智能体漂移、循环和输出缺失等静默故障方面的有效性，准确率分别达到98%和96%。


<details>
  <summary>更多</summary>
  
**动机:** 多智能体AI系统基于大语言模型，具有非确定性特点，容易发生静默故障（如漂移、循环、输出细节缺失），这些故障难以检测，需要系统化的异常检测方法。

**方法:** 提出了异常检测在智能体轨迹中的任务，开发了数据集构建流程来捕捉用户行为、智能体非确定性和LLM变异，构建了两个基准数据集（4275和894条轨迹），并使用监督学习（XGBoost）和半监督学习（SVDD）方法进行基准测试。

**结果:** 监督学习（XGBoost）和半监督学习（SVDD）方法表现相当，分别达到98%和96%的准确率，证明了这些方法在多智能体AI系统异常检测中的有效性。

**结论:** 这项工作首次系统性地研究了多智能体AI系统中的异常检测问题，提供了数据集、基准测试和见解，为未来研究提供了指导。
...
**AI概要:** LLM在处理数值属性时存在系统性放大相关性的问题，无关数值上下文会干扰表示并影响决策，揭示了多属性纠缠下的表示脆弱性


<details>
  <summary>更多</summary>
  
**动机:** 尽管行为研究发现大语言模型存在数值推理错误，但其内部表示机制尚不清楚，需要研究数值属性如何在共享潜在子空间中整合以及无关上下文如何干扰表示

**方法:** 结合线性探测与偏相关分析，在不同规模模型上进行基于提示的脆弱性测试

**结果:** LLM编码了真实世界的数值相关性但会系统性放大它们，无关上下文会引起幅度表示的一致偏移，下游影响因模型规模而异
...
**AI概要:** Agentmandering框架将选区重划重新构想为两个对立政党代理人的回合制谈判过程，通过LLM智能体嵌入策略互动，显著降低党派偏见和不公平性


<details>
  <summary>更多</summary>
  
**动机:** 现有计算方法主要生成合法选区重划方案的大规模集合，但忽略了选择过程中的策略动态，这为党派行为者挑选政治有利的地图创造了机会

**方法:** 基于博弈论思想（特别是Choose-and-Freeze协议），使用LLM智能体进行回合制谈判，代理人交替从少量候选地图中选择和冻结选区，通过受约束且可解释的选择逐步划分州

**结果:** 在2020年美国人口普查数据的所有州评估中，Agentmandering显著减少了党派偏见和不公平性，方差比标准基线低2-3个数量级，在摇摆州场景中表现出公平性和稳定性

**结论:** 该方法通过将策略互动嵌入选区重划过程，不仅满足形式约束，还确保了选择过程的公平性，为解决选区重划中的策略操纵问题提供了有效解决方案
...
**AI概要:** LLM-KGFR框架通过结合大语言模型和知识图谱检索器，解决了LLM在知识密集型问题上的局限性，实现了对未见知识图谱的零样本泛化和高效的大图谱处理。


<details>
  <summary>更多</summary>
  
**动机:** 大语言模型在推理方面表现出色，但在处理知识密集型问题时受限于上下文长度和参数化知识，现有方法在数据集特定调优和大规模图谱扩展性方面存在不足。

**方法:** 提出LLM-KGFR协作框架，使用知识图谱基础检索器(KGFR)编码关系描述并基于问题角色初始化实体，采用非对称渐进传播(APP)处理大规模图谱，通过节点、边和路径级接口支持迭代推理。

**结果:** 实验表明LLM-KGFR在保持可扩展性和泛化能力的同时实现了强劲性能

**结论:** 该框架为知识图谱增强推理提供了实用解决方案，能够有效处理大规模和未见过的知识图谱
...
**AI概要:** 本文提出了首个系统框架，通过以人为中心的基准测试来评估语音AI测试质量，解决了测试平台生成真实对话（模拟质量）和准确评估代理响应（评估质量）的双重挑战。


<details>
  <summary>更多</summary>
  
**动机:** 语音AI代理正在快速转向生产部署，但确保测试可靠性的系统方法仍然不成熟。组织无法客观评估其测试方法是否真正有效，随着语音AI扩展到数十亿日常交互，这造成了关键的测量差距。

**方法:** 该框架结合了成熟的心理测量技术（产生Elo评分的成对比较、自举置信区间和置换测试）与严格的统计验证，为任何测试方法提供可重复的度量标准。

**结果:** 对三个领先商业平台进行了全面实证评估，结果显示存在统计学显著的性能差异。表现最佳的平台Evalion在评估质量上达到0.92 F1分数（其他平台为0.73），在模拟质量上达到0.61联赛评分（其他平台为0.43）。
...
**AI概要:** 论文发现基于赋能的目标在单人类设置中有效的AI辅助目标，在多人类环境中可能导致其他人类的环境控制力和奖励减少，即"去赋能"现象。


<details>
  <summary>更多</summary>
  
**动机:** 虽然赋能被提出作为AI辅助行为的通用目标无关目标，但之前的研究假设AI只辅助单个孤立人类。现实中的多人类环境（如家庭和医院）需要研究赋能目标在多人类设置中的表现。

**方法:** 开发了开源的多人网格世界测试套件Disempower-Grid，通过实验方法研究辅助RL代理优化单人类赋能时对其他人类的影响。

**结果:** 实证表明优化单人类赋能的辅助RL代理会显著减少其他人类的环境影响力和奖励（去赋能现象）。联合赋能可以缓解此问题，但会以用户奖励为代价。

**结论:** AI对齐社区面临更广泛挑战：在单智能体设置中看似对齐的目标无关目标，在多智能体环境中可能变得不对齐。
...
**AI概要:** Opus工作流评估框架：一个结合概率奖励和规范惩罚的数学模型，用于量化工作流质量、效率，支持自动化评估、排序和优化。


<details>
  <summary>更多</summary>
  
**动机:** 需要一种系统化的方法来量化工作流的质量和效率，实现工作流的直接比较、评分和优化，特别是在现代自动化系统中。

**方法:** 结合Opus工作流奖励（概率函数估计预期性能）和Opus工作流规范惩罚（测量结构性和信息质量），形成统一的优化公式。

**结果:** 提出了一个概率-规范框架，能够通过成功概率、资源使用和输出增益来评估工作流，支持自动化评估和强化学习集成。

**结论:** 该框架为工作流评估和优化提供了数学基础，能够指导工作流的发现和改进，适用于现代自动化系统。
...
**AI概要:** 提出了一个多智能体预测编码框架，通过信息瓶颈目标让智能体学习何时、与谁、通信什么内容，在带宽限制下实现高效协调和空间记忆共享。
...
**方法:** 使用基于网格细胞的自监督运动预测建立内部空间编码，开发分层强化学习策略和类似海马社会位置细胞的神经群体，通过最小化相互不确定性来优化通信。

**结果:** 在Memory-Maze基准测试中表现优异：带宽从128位/步降至4位/步时，成功率从73.5%平缓降至64.4%，而全广播基线从67.6%骤降至28.6%。

**结论:** 该框架为复杂社会表征如何从统一的预测驱动中涌现提供了理论原则和生物学合理的基础，促进了社会集体智能的发展。
...
**AI概要:** RLoop是一个通过迭代策略初始化解决强化学习过拟合问题的自改进框架，通过探索-利用循环将瞬时策略变化转化为稳健性能提升，相比普通RL平均准确率提升9%，pass@32提升超过15%


<details>
  <summary>更多</summary>
  
**动机:** 强化学习在可验证奖励训练中存在过拟合问题，模型获得训练奖励但失去泛化能力，这是由于策略过度专业化和训练过程中多样化解的灾难性遗忘导致的

**方法:** 提出RLoop框架，通过迭代策略初始化：先用RL从给定策略探索解空间，然后过滤成功轨迹创建专家数据集，通过拒绝采样微调(RFT)精炼初始策略，为下一次迭代创建更好的起点

**结果:** RLoop有效缓解了遗忘问题并显著提升泛化能力，实验显示相比普通RL，平均准确率提升9%，pass@32提升超过15%

**结论:** RLoop通过探索-利用的良性循环成功解决了RL训练中的过拟合问题，将瞬时策略变化转化为稳健的性能提升，为大规模推理模型的强化学习训练提供了有效解决方案
...
**AI概要:** GUI-360°是一个大规模多模态数据集和基准测试套件，用于训练和评估计算机使用代理（CUAs），包含120万个执行动作步骤，支持GUI定位、屏幕解析和动作预测三个核心任务。


<details>
  <summary>更多</summary>
  
**动机:** 解决计算机使用代理研究中的三个关键问题：真实CUA任务稀缺、缺乏多模态轨迹自动收集标注流程、缺少统一基准测试框架。

**方法:** 采用LLM增强的自动化流程，包括查询来源、环境模板构建、任务实例化、批量执行和LLM驱动的质量过滤，在Windows办公应用中收集数据。

**结果:** 基准测试显示现有视觉-语言模型在GUI定位和动作预测方面存在显著不足，监督微调和强化学习虽有改进但仍达不到人类可靠性水平。

**结论:** GUI-360°数据集和代码已公开，旨在促进可重复研究并加速稳健桌面CUAs的发展，揭示了当前模型在真实GUI交互任务中的局限性。
...
**AI概要:** 论文指出传统CAV方法中仅用分类准确度衡量概念对齐性不可靠，提出了基于空间线性归因的新概念定位方法，并引入三类评估指标来量化概念对齐性。


<details>
  <summary>更多</summary>
  
**动机:** 现有CAV方法仅依赖探针分类准确度来评估概念对齐性，但研究发现这种方法容易捕捉伪相关而非真正概念，存在严重缺陷。

**方法:** 提出基于空间线性归因的新概念定位方法，与现有特征可视化技术进行对比，并引入三类评估指标：硬准确度、分割分数和增强鲁棒性。

**结果:** 研究表明具有平移不变性和空间对齐的探针能显著提高概念对齐性，故意构建的错位探针也能达到接近标准探针的准确度。

**结论:** 需要采用基于对齐性的评估指标而非探针准确度，且应根据模型架构和目标概念特性定制探针设计。
...
**AI概要:** AdversariaLLM是一个用于大语言模型越狱鲁棒性研究的工具箱，旨在解决当前LLM安全研究中实现碎片化、可复现性和可比性差的问题。


<details>
  <summary>更多</summary>
  
**动机:** 当前LLM安全和鲁棒性研究生态系统碎片化严重，存在大量有缺陷的实现、数据集和评估方法，导致研究难以复现和比较，阻碍了该领域的实质性进展。

**方法:** 开发了一个以可复现性、正确性和可扩展性为核心设计的工具箱，实现了12种对抗攻击算法，集成了7个基准数据集（涵盖危害性、过度拒绝和实用性评估），通过Hugging Face提供对多种开源LLM的访问，并包含计算资源追踪、确定性结果和分布评估等高级功能。

**结果:** 成功构建了AdversariaLLM工具箱及其配套包JudgeZoo，为LLM安全研究提供了透明、可比和可复现的坚实基础。

**结论:** AdversariaLLM通过统一的框架和工具集，有效解决了LLM安全研究领域的碎片化问题，为未来研究提供了标准化和可比较的基础设施，有望推动该领域的实质性进展。
...
**AI概要:** RxSafeBench是一个评估LLM药物安全性的新基准，包含2443个高质量临床咨询场景，测试LLM在禁忌症和药物相互作用方面的能力。研究发现当前LLM在整合隐含风险信息方面存在困难。


<details>
  <summary>更多</summary>
  
**动机:** 现有LLM医疗系统在药物安全性研究方面存在不足，缺乏真实世界数据集和临床咨询环境下的系统评估，受限于隐私和可访问性问题。

**方法:** 提出模拟临床咨询的框架，生成包含药物风险的问诊对话，构建RxRisk DB药物安全数据库，采用两阶段过滤策略确保临床真实性和专业性，使用结构化多选题评估LLM。

**结果:** 当前LLM难以有效整合禁忌症和药物相互作用知识，特别是在风险信息隐含而非明确的情况下表现不佳。

**结论:** 研究揭示了LLM系统在药物安全方面的关键挑战，RxSafeBench为评估LLM药物安全性提供了首个全面基准，有助于通过改进提示和任务特定调优来提高AI临床决策支持的可靠性。
...
**AI概要:** 该论文提出了Monitor-Generate-Verify (MGV)框架，通过在生成前增加显式监控过程来解决现有Generate-Verify架构中的前缀主导陷阱问题，将元认知理论转化为计算规范。


<details>
  <summary>更多</summary>
  
**动机:** 现有的Generate-Verify推理架构缺少监控过程，导致模型容易陷入前缀主导陷阱（prefix dominance trap），即在早期就锁定次优推理路径且难以恢复，造成约20%的准确率损失。

**方法:** 将Flavell以及Nelson和Narens的元认知理论形式化为计算规范，提出MGV框架，在生成前增加显式监控过程，捕获元认知体验（难度评估到置信度判断），并通过验证反馈来优化未来的监控。

**结果:** 虽然未提供实证验证，但这项工作首次系统地将基础元认知理论转化为计算实现，为理解推理系统失败提供了原则性词汇，并为未来测试时推理设计提出了具体的架构干预措施。

**结论:** MGV框架通过整合元认知监控机制，为解决推理系统中的前缀主导陷阱问题提供了理论框架和架构指导，为未来更稳健的推理系统设计奠定了基础。
...
**AI概要:** Iterative RMFT是一种后训练方法，通过迭代蒸馏低后悔决策轨迹来提升大语言模型的决策能力，无需依赖已知算法或人工模板，实现了跨模型的性能提升和理论保证。


<details>
  <summary>更多</summary>
  
**动机:** 现有大语言模型在交互式动态环境中进行决策时表现不佳，无法有效实现低后悔或探索-利用平衡，需要专门的后训练方法来提升其决策能力。

**方法:** 提出迭代后悔最小化微调(Iterative RMFT)：模型在每次迭代中生成多个决策轨迹，选择k个最低后悔的轨迹，然后在这些轨迹上对自身进行微调，利用后悔指标激发模型的决策能力和推理原理。

**结果:** 实验证明Iterative RMFT在不同类型模型（从数值输入输出的Transformer到GPT-4o mini等闭源模型）上都显著提升了决策性能，并能泛化到不同任务场景。

**结论:** Iterative RMFT提供了一个原则性的通用后训练框架，通过利用模型自身生成的推理来增强大语言模型的决策能力，避免了刚性输出工程，具有灵活性和理论支持。
...
**AI概要:** CoRPO是一种新的强化学习算法，解决了GRPO在处理序数奖励时的缺陷，通过自适应基线确保失败轨迹不被正向强化，并在达到质量阈值后转向相对偏好模式以寻找最优解。


<details>
  <summary>更多</summary>
  
**动机:** GRPO在处理非二元反馈（如序数奖励）时存在缺陷，其组平均基线可能对失败轨迹赋予正优势值，从而强化错误行为。

**方法:** 提出CoRPO算法，使用自适应基线强制最小质量阈值，确保失败解不被正向强化；当策略持续满足阈值后，基线自动转为相对偏好模式以寻找最优解。

**结果:** 在代码验证任务上实证验证，CoRPO表现出更稳定的收敛性和更好的域外泛化能力。

**结论:** 这项工作是在LLMs通过强化学习获得真正新能力研究计划中的关键一步，使LLMs能够从丰富的多维度反馈中学习，从二元奖励扩展到序数奖励，并最终实现更密集的每步监督。
...
**AI概要:** PAVe系统结合传统路径规划算法和LLM语义推理，通过多目标Dijkstra算法生成候选路径，再利用LLM代理根据用户任务、偏好和规避规则进行上下文评估，实现个性化车辆路线规划。


<details>
  <summary>更多</summary>
  
**动机:** 传统车辆路线系统只能优化单一指标，无法理解人类驾驶员的复杂语义和动态上下文需求，如多步骤任务、情境约束或紧急需求。

**方法:** 采用混合代理方法：先用多目标（时间、CO2）Dijkstra算法生成候选路线，然后使用LLM代理结合预处理的地理空间POI缓存，根据用户提供的任务、偏好和规避规则评估路线。

**结果:** 在城市场景基准测试中，PAVe成功将复杂用户意图转化为适当的路线修改，使用本地模型的初始路线选择准确率超过88%。

**结论:** 将经典路由算法与基于LLM的语义推理层相结合，是创建个性化、自适应和可扩展城市移动优化解决方案的有效方法。
...
**AI概要:** 该论文首次探索了网络代理的能源消耗和碳排放问题，通过理论和实证分析发现不同设计哲学对能耗有显著影响，且高能耗不一定带来更好结果，呼吁在评估网络代理时加入能耗指标。


<details>
  <summary>更多</summary>
  
**动机:** 虽然网络代理研究蓬勃发展，但其引发的可持续性问题（能源消耗和碳排放）尚未得到充分探索，需要揭示这一紧迫问题。

**方法:** 采用理论估计和实证基准测试相结合的方法，从两个角度分析网络代理的能源和CO2成本。

**结果:** 研究发现不同网络代理设计哲学会严重影响能耗，高能耗不一定等于更好结果，同时模型参数和过程缺乏透明度限制了能耗估算。

**结论:** 需要改变网络代理评估方式，在基准测试中加入专门的能耗指标，提高透明度以促进可持续发展。
...
**AI概要:** 研究发现开源大语言模型能够复现人类在博弈论实验中的决策模式，其中Llama模型高度还原人类合作行为，而Qwen模型更符合纳什均衡预测，无需人物设定提示即可实现群体行为模拟。


<details>
  <summary>更多</summary>
  
**动机:** 了解大语言模型在模拟人类决策方面的准确性，避免实际应用中的潜在危害，并验证其对社会行为模拟的有效性。

**方法:** 开发博弈论实验的数字孪生系统，建立系统的提示和探测框架，测试Llama、Mistral和Qwen三个开源模型在游戏理论场景中的表现。

**结果:** Llama模型能高保真复现人类合作模式，捕捉人类偏离理性选择理论的行为；Qwen模型则更符合纳什均衡预测；无需人物设定提示即可实现群体行为复制。

**结论:** 适当校准的大语言模型能够复制人类群体行为模式，为社会科学研究提供补充方法，并能生成关于人类社交决策的新实证预测。
...
**AI概要:** 本研究提出了一种数据驱动的稀疏传感框架，结合EPA-SWMM模型，通过优化传感器布置来重建雨水系统的峰值流量，仅用3个传感器在77个节点中就实现了0.92-0.95的纳什效率系数。


<details>
  <summary>更多</summary>
  
**动机:** 城市地表水洪水因强降雨超过排水系统承载能力而日益频发，但高时空分辨率的洪水预测和监测受到时间、预算和技术限制，如何在资源受限条件下监测城市排水网络和预测流量状况是主要挑战。

**方法:** 使用EPA-SWMM模型生成雨水网络中峰值流量剖面的训练数据集，应用数据驱动稀疏传感框架，通过奇异值分解进行降维和QR分解进行传感器分配，基于模拟训练数据集确定最优监测节点。

**结果:** 在77个节点中仅布置3个最优传感器就实现了满意的重建性能，纳什-萨特克利夫效率系数达到0.92-0.95（25-75百分位数），模型对测量不确定性具有良好的鲁棒性。

**结论:** 该框架平衡了计算效率和物理可解释性，能够以最少的传感器实现高精度流量重建，可进一步与预测模型集成，在有限的传感和监测资源下实现洪水预警和实时控制。
...
**AI概要:** Jr. AI Scientist是一个模拟初级研究者工作流程的自主AI科学家系统，能够分析论文局限、提出假设、实验验证并撰写论文，表现优于现有全自动系统但仍存在局限性和风险。


<details>
  <summary>更多</summary>
  
**动机:** 理解AI科学家系统的当前能力和风险，确保可信赖和可持续的AI驱动科学进步，同时维护学术生态系统的完整性。

**方法:** 开发Jr. AI Scientist系统，模拟新手学生研究者的核心研究流程：分析基线论文局限、制定改进假设、通过严格实验验证、撰写结果论文。利用现代编码代理处理复杂的多文件实现。通过AI评审员自动评估、作者主导评估和向Agents4Science投稿进行评价。

**结果:** Jr. AI Scientist生成的论文获得比现有全自动系统更高的评审分数，但作者评估和Agents4Science评审都发现了重要局限性。

**结论:** 当前AI科学家系统存在直接应用的风险和关键挑战，需要进一步研究。研究深入揭示了AI科学家发展的当前进展和风险。
...
**AI概要:** 本文提出将自然语言查询中的歧义视为合作交互特征而非缺陷，开发了区分可解析合作查询与不可解析非合作查询的框架，分析了15个流行数据集的查询类型，发现现有评估方法存在不足，并提出了面向合作解析的界面设计和评估新方向。


<details>
  <summary>更多</summary>
  
**动机:** 自然语言界面处理表格数据时面临查询歧义问题，传统方法将歧义视为缺陷，但本文认为歧义是合作交互的特征，用户和系统应共同承担查询规范的责任。

**方法:** 开发了一个原则性框架来区分合作查询（可解析解释）和非合作查询（无法解析），并将该框架应用于15个流行表格问答和分析数据集的查询分析。

**结果:** 分析发现现有数据集中的查询类型混合不当，既不适合评估系统执行准确性，也不适合评估解释能力。框架提供了从修复歧义转向拥抱合作解析的新视角。

**结论:** 该研究为表格数据自然语言界面提供了更明智的设计和评估方法，概述了未来研究的影响和方向，强调在查询解析中采用合作范式的重要性。
...
**AI概要:** 论文提出了一个基于正当代表制(JR)的审计框架，用于评估审议过程中专家问答环节的问题选择代表性，并开发了高效算法来测量问题集对参与者兴趣的代表程度。


<details>
  <summary>更多</summary>
  
**动机:** 在公民大会等审议过程中，参与者只能提出有限数量的问题给专家小组，如何选择最具代表性的问题集合来反映所有参与者的兴趣是一个挑战。

**方法:** 引入基于社会选择理论中正当代表制概念的审计框架，开发了在一般效用设置下审计JR的首个算法，最有效算法的时间复杂度为O(mn log n)。应用该方法比较了主持人选择的问题、整数线性规划选择的问题和LLM生成的问题的代表性。

**结果:** 研究结果揭示了LLM在支持审议过程中的潜力和当前局限性。通过将方法集成到已在50多个国家使用的在线审议平台，使实践者能够轻松审计和改进未来审议的代表性。

**结论:** 该研究为解决审议过程中的代表性问题提供了实用的审计工具和方法，特别强调了LLM技术的应用前景和现有不足，为改进民主审议过程提供了技术支撑。
...
**AI概要:** DR. WELL是一个去中心化的神经符号框架，通过两阶段协商协议实现多智能体协作规划，使用符号化抽象避免轨迹级协调的脆弱性，提高任务完成率和效率。


<details>
  <summary>更多</summary>
  
**动机:** 多智能体协作规划中存在部分信息和有限通信的挑战，轨迹级协调容易因微小偏差导致冲突，需要更高层次的抽象来确保同步和集体进展。

**方法:** 采用两阶段协商协议：首先智能体提出候选角色并进行推理，然后在共识和环境约束下承诺联合分配；承诺后每个智能体独立生成和执行符号化计划，通过共享世界模型进行执行结果落地。

**结果:** 在协作推块任务实验中，动态世界模型能够捕获可重用模式，提高任务完成率和效率，通过协商和自我优化实现更高效的协作策略。

**结论:** 通过符号化规划而非原始轨迹推理，DR. WELL避免了脆弱的步级对齐，实现了可重用、可同步和可解释的高层次操作，为多智能体协作提供了有效解决方案。
...
**AI概要:** VeriCoT是一个神经符号方法，将思维链推理转换为形式逻辑论证并验证其有效性，通过自动化求解器和人类可读的前提识别来检测错误推理，并在多个数据集上证明能有效预测答案正确性和改进推理质量。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型虽然能进行多步思维链推理，但无法可靠验证自身逻辑，即使在得出正确答案时底层推理也可能存在缺陷，这在高风险场景中削弱了可信度。

**方法:** VeriCoT将每个思维链推理步骤形式化为一阶逻辑，识别基于源上下文、常识知识或先前推理步骤的前提，使用自动化求解器验证逻辑有效性，并通过自然语言前提让人类和系统识别无根据或谬误的推理步骤。

**结果:** 在ProofWriter、LegalBench和BioASQ数据集上的实验显示，VeriCoT能有效识别有缺陷的推理，并作为最终答案正确性的强预测指标。

**结论:** VeriCoT不仅提供了推理验证能力，还可用于推理时自我反思、监督微调和基于验证的偏好微调，进一步提高了推理的有效性和准确性。
...
**AI概要:** 该论文提出了一个新颖的pipeline，通过提取Transformer层的隐藏状态激活，使用大五人格特质框架，发现低秩子空间并识别特质特定最优层，实现LLM人格特质的精确控制。


<details>
  <summary>更多</summary>
  
**动机:** 大型语言模型在生成中展现出隐含的人格特质，但如何可靠地控制或对齐这些特质以满足特定需求仍是一个开放挑战，需要有效的行为操纵机制。

**方法:** 提出一个pipeline：1)从Transformer层提取隐藏状态激活；2)使用大五人格特质框架；3)应用低秩子空间发现方法；4)识别不同模型架构中的特质特定最优层；5)通过动态层选择的灵活引导框架操作化人格对齐方向。

**结果:** 发现人格特质占据低秩共享子空间，这些潜在结构可以通过精心扰动转化为有效的引导机制，不影响流畅性、方差和一般能力。

**结论:** 该方法有助于弥合心理学理论与实际模型对齐之间的差距，为人格感知LLMs提供了有前景的方向。
...
**AI概要:** TextualVerifier是一个基于LLM的自我验证框架，通过思维链分解、变体生成、多数投票和共识聚合四阶段工作流，为TextGrad提供无需数值梯度的验证能力，显著提升推理有效性和优化结果。


<details>
  <summary>更多</summary>
  
**动机:** TextGrad缺乏确保文本推理有效性的自我验证机制，需要解决文本决策中的验证空白问题。

**方法:** 采用思维链推理和多数投票机制，四阶段工作流程：思维链分解、变体生成、多数投票、共识聚合，并与TextGrad在损失函数和优化结果验证阶段非侵入式集成。

**结果:** 实验显示统计显著改进(p<0.001)：推理步骤有效性提升29%；TextGrad集成后准确率从68.2%提升至70.4%；在GPQA、MMLU-ML、MMLU-CP基准上分别提升8.08、10.71、3.92个百分点。

**结论:** TextualVerifier是首个基于LLM技术的TextGrad自我验证框架，无需数值梯度即可实现更可靠的推理，为文本优化验证开辟了新方向。
...
**AI概要:** 本文扩展了希腊方言数据集GRDD+，新增6种方言变体，总规模达637万词，包含10种方言。通过微调3种LLM模型并与前沿模型对比，验证高质量方言数据对模型性能的影响。


<details>
  <summary>更多</summary>
  
**动机:** 现有希腊方言数据集覆盖范围有限，需要构建更大规模、更多样化的方言数据集来支持语言模型在方言处理方面的研究。

**方法:** 扩展GRDD数据集，新增克里特、塞浦路斯等6种希腊方言变体；使用637万词的数据集对Llama-3-8B等3种模型进行微调，并与Claude-3.7等前沿模型进行性能对比。

**结果:** 构建了迄今为止规模最大、变体最丰富的希腊方言数据集GRDD+；通过微调实验验证了高质量方言数据能有效提升语言模型在方言处理任务上的性能。

**结论:** 大规模、高质量的方言数据集对于提升语言模型的方言处理能力至关重要，GRDD+为希腊方言NLP研究提供了重要资源。
...
**AI概要:** PLLuM是波兰首个大规模开源语言模型家族，专门为波兰语设计，包含1400亿token预训练语料和定制数据集，采用负责任AI框架，旨在提升波兰语AI技术主权。


<details>
  <summary>更多</summary>
  
**动机:** 当前大语言模型主要针对英语开发，其他语言支持有限。波兰研究机构联盟希望开发高质量、透明且符合波兰文化背景的语言模型，打破英语主导的商业格局。

**方法:** 构建1400亿token波兰语预训练语料、77k指令数据集和100k偏好优化数据集；采用负责任AI框架，包含严格数据治理和混合输出校正安全过滤模块；详细设计模型架构、训练流程和对齐技术。

**结果:** 开发了PLLuM模型家族，包括基础版和指令调优版，并在公共管理下游任务中验证了其实用性。

**结论:** 通过公开发布这些模型，PLLuM旨在促进开放研究，加强波兰的AI技术主权，为波兰语社区提供高质量的语言AI解决方案。
...
**AI概要:** STARS是一种新的解码时对齐算法，通过迭代采样、评分和拒绝/接受固定长度的token片段来引导模型生成，显著提高计算效率和对齐质量。


<details>
  <summary>更多</summary>
  
**动机:** 现有的大语言模型对齐方法（如微调）计算成本高且效果不佳，而推理时方法（如Best-of-N采样）需要不可行的计算量才能达到最佳对齐效果。

**方法:** 提出STARS算法：在解码时对短固定长度的token片段进行迭代采样、评分和拒绝/接受，实现生成路径的早期校正。

**结果:** 在6个LLM上的实验显示，STARS在胜率上比SFT高出14.9个百分点，比DPO高出4.3个百分点，与强Best-of-N基线竞争力相当。

**结论:** 细粒度的奖励引导采样可作为传统微调和全序列排序方法的通用、鲁棒且高效替代方案，用于LLM对齐。
...
**AI概要:** 提出一种将多标签文本分类任务分解为一系列二分类决策的高效方法，通过独立查询每个标签维度并结合前缀缓存机制，在不损失准确性的情况下显著提升短文本推理效率。


<details>
  <summary>更多</summary>
  
**动机:** 解决大语言模型在多标签分类任务中效率低下的问题，特别是处理短文本时需要重复计算相同前缀的情况，寻求一种既保持准确性又提高推理速度的方法。

**方法:** 将多标签分类重新表述为序列化的二分类决策，每个目标维度独立查询；使用DeepSeek-V3作为强标注模型提供多标注，通过LLM-to-SLM蒸馏技术将知识蒸馏到小型模型；采用前缀缓存机制优化推理效率。

**结果:** 微调后的小型模型在情感文本分析任务上相比零样本基线有显著提升，特别是在训练时见过的维度上表现更好；该方法在保持准确性的同时获得了显著的效率增益。

**结论:** 将多标签分类分解为二分类查询，结合蒸馏和缓存感知推理，为基于LLM的分类提供了一个可扩展且有效的框架；虽然该方法在情感状态分析上验证，但具有跨领域的通用适用性。
...
**AI概要:** 该研究分析了三种低资源语言机器翻译数据集的质量问题，发现存在性别偏见、领域不平衡和有害内容，强调数据量不等于质量。


<details>
  <summary>更多</summary>
  
**动机:** 随着低资源语言在NLP研究中被越来越多地纳入，大规模数据集的收集成为重点，但过度追求数量而忽视质量可能导致技术性能不佳和产生有害内容。

**方法:** 研究调查了Afan Oromo、Amharic和Tigrinya三种低资源语言的机器翻译数据集，重点关注数据集中的性别表征问题。

**结果:** 发现训练数据中政治和宗教领域文本占比过大，而基准数据集则集中在新闻、健康、体育领域；存在明显的男性性别偏向（人名、动词语法性别、刻板描述）；发现针对女性的有害和有毒描述，数据量最大的语言问题最严重。

**结论:** 数据量不能保证质量，希望该研究能激发对低资源语言数据集质量的进一步调查，并促进早期有害内容的缓解。
...
**AI概要:** GRAD是一种无需重新训练的解码方法，通过构建稀疏标记转移图，在生成过程中融合语料证据来减少大语言模型的幻觉问题，在多个基准测试中显著提升准确性和事实性。


<details>
  <summary>更多</summary>
  
**动机:** 现有方法依赖外部知识源（如知识图谱），但基于提示的方法脆弱且领域敏感，符号知识集成成本高昂。受知识图谱启发，需要一种轻量级、即插即用的方法来缓解幻觉问题。

**方法:** GRAD在解码时构建稀疏标记转移图，通过单次前向传播累积检索语料中的下一个标记logits，然后将图检索的logits进行最大归一化并与模型logits自适应融合，优先选择高证据的延续。

**结果:** 在三个模型和多个问答基准测试中，GRAD始终优于基线方法：内在准确率提高9.7%，幻觉率降低8.6%，正确性提高6.9%，在所有方法中获得最高的真实性-信息性乘积分数。

**结论:** GRAD提供了一种轻量级、即插即用的替代方案，证明来自语料级标记转移的统计证据可以有效地引导生成更真实和可验证的输出。
...
**AI概要:** 论文研究了迭代参考游戏中人类和视觉语言模型的对比表现，发现模型在无相关上下文时表现远差于人类，但在有相关上下文时性能显著提升，但抽象指代任务对模型仍具挑战性


<details>
  <summary>更多</summary>
  
**动机:** 测试智能体在多轮语言环境中进行上下文敏感语用推理的能力，通过迭代参考游戏比较人类和视觉语言模型的表现差异

**方法:** 使用迭代参考游戏实验，通过改变上下文的数量、顺序和相关性来测试人类和视觉语言模型的表现

**结果:** 无相关上下文时，模型表现高于随机但远差于人类；有相关上下文时，模型性能随试验次数显著提升；抽象指代任务对模型仍很困难

**结论:** 迭代参考游戏是测试上下文敏感语用推理的重要测试案例，当前模型在有相关上下文时表现良好，但在抽象指代任务上仍需改进
...
**AI概要:** 论文开发了人类繁荣地理指数（HFGI），通过分析26亿条美国地理定位推文，使用微调的大语言模型对48个繁荣相关指标进行分类，提供了县和州级别的月度/年度繁荣话语数据，为多学科研究提供高分辨率数据支撑。


<details>
  <summary>更多</summary>
  
**动机:** 量化人类繁荣（包括幸福、健康、目标、美德、关系和经济稳定等多维度）对于超越经济指标理解社会福祉至关重要，但现有测量方法缺乏精细的时空分辨率。

**方法:** 分析2013-2023年约26亿条美国地理定位推文，使用微调的大型语言模型对48个指标进行分类，这些指标与哈佛全球繁荣研究框架一致，并包括对移民态度和腐败感知的测量。

**结果:** 创建了HFGI数据集，提供县和州级别的月度及年度繁荣相关话语指标，验证表明这些测量准确代表了基础构念，并与既定指标显示出预期的相关性。

**结论:** 该资源能够以前所未有的分辨率进行福祉、不平等和社会变化的多学科分析，提供了过去十年美国社交媒体话语中反映的人类繁荣动态的深入见解。
...
**AI概要:** 该论文提出了一种通过向量翻译在多智能体系统中实现语义级通信的方法，替代传统的token级消息传递，减少计算开销并提升信息传输效率。


<details>
  <summary>更多</summary>
  
**动机:** 当前多智能体系统中LLM通过token传递消息，丢弃了潜在语义信息，限制了信息传输效率并增加了不必要的计算开销。

**方法:** 使用学习到的映射建立向量翻译机制，在Llama-2-7B和Mistral-7B-Instruct之间训练双编码器翻译器，实现表示空间之间的直接语义交换。

**结果:** 获得了0.538的平均余弦对齐度，以30%混合强度注入翻译向量可在不破坏对数概率稳定性的情况下引导目标模型生成，双向评估显示2.01:1的传输不对称性。

**结论:** 跨模型潜在通信是可行的，这种方法可以构建共享语义而非token的协作AI系统，同时保持计算稳定性。
...
**AI概要:** 该论文提出了一个将溯因推理整合到检索增强生成(RAG)系统中的框架，通过检测证据不足、生成候选缺失前提并进行验证，提高了答案准确性和推理可靠性。


<details>
  <summary>更多</summary>
  
**动机:** RAG系统在检索证据不完整时会出现推理空白，需要一种原则性方法来填补这些空白，溯因推理为此提供了解决方案。

**方法:** 提出一个框架：检测证据不足的情况，生成候选缺失前提，通过一致性和合理性检查进行验证。
...
**AI概要:** 提出Weakly Supervised Transducer (WST)方法，通过灵活的训练图处理转录错误，在高达70%错误率的转录数据上仍能保持性能表现，优于现有的弱监督方法。


<details>
  <summary>更多</summary>
  
**动机:** RNN-T在端到端语音识别中依赖大量高质量标注数据，但这些数据成本高昂且难以获取，需要减少对此类数据的依赖。

**方法:** 设计Weakly Supervised Transducer (WST)，集成灵活的训练图来鲁棒处理转录错误，无需额外的置信度估计或预训练模型。

**结果:** 在合成和工业数据集上的实验表明，WST在转录错误率高达70%时仍能有效保持性能，持续优于基于CTC的弱监督方法如BTC和OTC。

**结论:** WST在实际ASR设置中具有实用性和鲁棒性，证明其处理高错误率转录数据的有效性。
...
**AI概要:** 论文提出了T-FIX基准，用于评估LLM在知识密集型领域生成解释时与专家判断的一致性，解决了现有评估方法只关注解释的合理性而忽略专家直觉对齐的问题。


<details>
  <summary>更多</summary>
  
**动机:** 在知识密集型应用场景中，用户（如医生、天文学家等专家）需要LLM提供符合专家推理水平的解释，但现有评估方法主要关注解释的合理性和内部一致性，无法衡量解释内容是否真正符合专家直觉。

**方法:** 开发了T-FIX基准，涵盖七个知识密集型领域，通过与领域专家合作制定新的度量标准来评估LLM解释与专家判断的一致性。

**结果:** 提出了专家对齐作为解释评估的新标准，并建立了相应的评估框架和度量方法。

**结论:** 专家对齐是评估LLM解释质量的重要维度，T-FIX基准为衡量LLM在专业领域解释能力提供了有效的评估工具。
...
**动机:** 现有方法未能充分理解时间约束的复杂语义信息，LLMs虽具备强大语义理解能力但时序推理能力有限且存在幻觉问题

**方法:** 提出Plan of Knowledge框架：1）将复杂时序问题分解为预定义工具的子目标序列；2）构建对比检索框架的时序知识库，选择性检索语义和时间对齐的事实

**结果:** 在四个基准TKGQA数据集上，PoK显著提升检索精度和推理准确率，最高超越最先进方法56.0%

**结论:** PoK通过结构化规划和时序知识检索的结合，有效增强了时序推理的可解释性和事实一致性
...
**AI概要:** 比较人类和大型语言模型在情感词汇联想行为上的差异，发现LLMs与人类联想重叠度中等，但LLMs倾向于放大情感负荷且更可预测、缺乏创造性


<details>
  <summary>更多</summary>
  
**动机:** 人类词汇联想能揭示心理词典机制，但受个人因素影响难以预测。研究旨在探索LLMs是否能以类似人类方式生成联想，特别是对情感词汇的反应

**方法:** 通过比较人类参与者和大型语言模型对情感词汇的联想反应，分析两者在联想行为上的相似性和差异性

**结果:** 人类与LLMs的联想重叠度为中等水平；LLMs倾向于放大刺激词的情感负荷；LLMs的联想更可预测且创造性低于人类

**结论:** 大型语言模型在词汇联想任务中表现出与人类相似但不同的模式，其联想更加标准化和情感放大化，缺乏人类联想的创造性和个体差异性
...
**AI概要:** 基于Transformer的放射学报告去标识化模型，通过大规模多模态训练，在PHI检测方面超越现有学术和商业系统，建立了新的临床文本安全处理基准。


<details>
  <summary>更多</summary>
  
**动机:** 提升放射学报告的自动去标识化能力，通过大规模训练数据集扩展基于Transformer的模型，并与商业云供应商系统在受保护健康信息(PHI)检测方面进行性能对比。
...
**结果:** 模型在Penn数据集上F1得分0.973，斯坦福数据集上0.996，超越或保持先前最先进模型性能。合成PHI评估显示一致的检测能力(F1:0.959)，在所有供应商系统中表现最佳(F1:0.960 vs. 0.632-0.754)。

**结论:** 大规模多模态训练提高了跨机构泛化能力和鲁棒性，合成PHI生成在确保隐私的同时保持了数据效用。基于Transformer的去标识化模型在PHI检测方面优于先前的学术和商业系统，为安全临床文本处理设立了新基准。
...
**AI概要:** 该论文研究了k-列表极限语言识别问题，通过扩展Angluin的经典理论，给出了k-列表可识别语言集合的精确特征化，并建立了统计设置下的指数收敛速率界限。


<details>
  <summary>更多</summary>
  
**动机:** 受近期语言生成问题积极结果的启发，重新审视经典的语言极限识别问题，探索在每次提供k个猜测的扩展能力下该问题的可解性。

**方法:** 基于Angluin经典特征化的递归版本，提出k-列表极限识别的精确特征化方法，并将问题分解为k个可单列表识别的语言集合。

**结果:** 证明了语言集合可k-列表极限识别当且仅当可分解为k个可单列表识别的子集合，并在统计设置下建立了指数收敛速率的最优性。

**结论:** 研究为语言识别理论提供了新的理论框架，展示了列表大小对识别能力的精确影响，并为统计学习中的收敛速率提供了理论基础。
...
**AI概要:** 批处理提示不仅降低LLM推理成本，还能作为推理时正则化器，提高多步推理的准确性和效率，减少3-5倍推理token使用，抑制过度思考，促进更果断的答案，并出现集体推理效应。


<details>
  <summary>更多</summary>
  
**动机:** 探索批处理提示在大型语言模型中的额外价值，超越传统的推理成本优化，研究其在多步推理中的正则化作用和行为影响。

**方法:** 在13个不同基准测试上进行全面研究，通过详细的行为分析评估批处理对模型推理行为的影响，包括过度思考抑制、模糊语言减少和集体效应观察。

**结果:** 批处理显著提高准确性，减少3-5倍推理token使用，抑制过度思考，减少自我修正等模糊语言，促进更果断答案，并出现模型从批次中较早示例泛化模式解决更难问题的集体效应。

**结论:** 批处理不仅是吞吐量优化技术，更是强大的推理时正则化器，能够实现更高效可靠的LLM推理，为大型推理模型提供了重要的性能提升途径。
...
**AI概要:** RIDE是一个基于项目反应理论的对抗性问题重写框架，用于生成更难的数学问题，评估LLM的真实数学推理能力，实验显示能平均降低26个模型21.73%的性能


<details>
  <summary>更多</summary>
  
**动机:** 当前LLM在数学推理上的高表现可能源于训练数据泄露或表面模式匹配，而非真正的推理能力，需要对抗性评估来测量真实能力

**方法:** 使用35个LLM模拟学生，基于项目反应理论构建难度排序器，通过强化学习指导问题重写模型生成不同难度的问题变体

**结果:** 在竞赛级数学基准上应用RIDE，生成的扰动版本使先进LLM性能平均下降21.73%，暴露了数学推理的有限鲁棒性

**结论:** RIDE框架有效揭示了LLM数学推理能力的局限性，验证了该评估方法的有效性，为更真实的数学推理评估提供了新途径
...
**AI概要:** CantoASR是一个结合ASR和大型音频语言模型的粤语语音识别框架，通过声学特征提取、微调Whisper模型和指令调优的Qwen-Audio，显著提升了低资源粤语ASR的准确率。


<details>
  <summary>更多</summary>
  
**动机:** 粤语ASR面临标注数据稀缺、六声调系统、变调和口音变化等挑战，现有模型如Whisper在粤语上词错误率较高，需要更好的声调和韵律处理能力。

**方法:** 提出CantoASR协作框架：1) 使用强制对齐提取声学特征；2) LoRA微调Whisper提升声调辨别能力；3) 指令调优Qwen-Audio进行韵律感知的纠错。

**结果:** 在自发粤语数据上的评估显示，相比Whisper-Large-V3取得了显著的字符错误率(CER)提升。

**结论:** 将声学线索与大型音频语言模型的推理能力相结合，为低资源声调语言和方言的ASR提供了可扩展的策略。
...
**AI概要:** 本文提出三种多智能体LLM流水线来提升Text-to-SQL任务性能，通过智能体协作讨论、规划-编码和编码-聚合等机制，显著提升了小模型的SQL生成准确率，其中Reasoner-Coder流水线效果最佳。


<details>
  <summary>更多</summary>
  
**动机:** 现有大语言模型在处理大规模数据库模式和复杂推理的Text-to-SQL任务时表现不佳，且现有研究多关注复杂不实用的旗舰模型流水线，而忽略了更小更高效的模型。

**方法:** 提出了三种多智能体流水线：(1)多智能体讨论流水线-智能体迭代批判和精炼SQL查询，由法官合成最终答案；(2)规划者-编码者流水线-思考模型规划者生成逐步SQL生成计划，编码者合成查询；(3)编码者-聚合者流水线-多个编码者独立生成SQL，推理智能体选择最佳查询。

**结果:** 多智能体讨论使Qwen2.5-7b-Instruct模型的执行准确率在三轮讨论后提升10.6%；Reasoner-Coder流水线效果最佳，DeepSeek-R1-32B和QwQ-32B规划者将Gemma 3 27B IT准确率从52.4%提升至56.4%。

**结论:** 多智能体协作方法能有效提升小模型在Text-to-SQL任务中的性能，为开发更高效的数据库自然语言接口提供了新思路。
...
**AI概要:** LAAC提出将LLM作为智能沟通中介的新范式，通过结构化对话捕获发送者意图，实现真实知识交换，避免AI生成内容的膨胀-压缩循环。但该方案在信息保真度、一致性和可靠性方面存在信任挑战。


<details>
  <summary>更多</summary>
  
**动机:** 当前AI生成内容泛滥导致沟通剧场化：发送者用LLM将简单想法膨胀为冗长内容，接收者用LLM压缩回摘要，双方都未接触真实内容。需要改变这种低效沟通模式。

**方法:** 提出LAAC框架，采用多智能体架构，通过结构化对话提取发送者意图，系统评估三个信任维度：信息捕获保真度、可重现性和查询响应完整性。

**结果:** 通过多场景控制实验发现，LAAC在部署前存在可测量的信任差距，特别是在高风险沟通场景中需要解决这些问题。

**结论:** LAAC作为沟通中介具有潜力，但必须首先解决信息保真度、一致性和可靠性方面的信任问题，才能可靠地部署到关键沟通场景中。
...
**AI概要:** 本研究提出了一个计算图灵测试框架来验证大语言模型生成文本的人类相似性，发现即使经过校准，LLM输出仍与人类文本有明显区别，特别是在情感表达方面，且存在人类相似性与语义保真度的权衡。


<details>
  <summary>更多</summary>
  
**动机:** 当前社会科学研究中大量使用大语言模型模拟人类行为，但缺乏对模型生成文本人类相似性的可靠验证方法，现有基于人类判断的评估方法不够准确可靠。

**方法:** 提出计算图灵测试框架，整合基于BERT的检测指标、语义相似度和可解释语言特征；系统比较9个开源LLM在5种校准策略下的表现，包括微调、风格提示和上下文检索。

**结果:** LLM输出即使经过校准仍明显可区分于人类文本，特别是在情感语调方面；指令调优模型表现不如基础模型；模型规模扩大不会增强人类相似性；存在人类相似性与语义保真度的权衡。

**结论:** 研究提供了LLM模拟验证和校准的可扩展框架，同时警示当前LLM在捕捉人类交流方面存在局限性，需要更谨慎地使用LLM进行社会科学模拟。
...
**AI概要:** 本研究实证评估了当前大语言模型能否通过波兰国家上诉法庭的官方资格考试。虽然模型在选择题知识测试中表现尚可，但在实践写作部分均未达到及格线，且LLM作为评判者的评估与官方评审存在分歧。


<details>
  <summary>更多</summary>
  
**动机:** 评估当前大语言模型在法律专业资格考试中的实际表现，特别是检验其能否胜任波兰公共采购法领域的专业资格认证。

**方法:** 构建混合信息检索和提取管道，测试多个LLM（包括GPT-4.1、Claude 4 Sonnet和Bielik-11B-v2.6）在闭卷和各种检索增强生成设置下的表现，采用LLM作为考生和LLM作为评判者两种方法。

**结果:** 模型在知识测试中获得满意分数，但在实践写作部分全部未通过；LLM-as-a-judge的评估结果与官方评审委员会判断经常不一致。

**结论:** 尽管技术进步迅速，当前LLM仍存在幻觉、错误引用法律条款、逻辑论证薄弱等问题，尚不能取代波兰公共采购裁决中的人类法官或独立考官，需要法律专家与技术团队的密切合作。
...
**AI概要:** REMIND是一种新的机器学习遗忘评估方法，通过分析输入微小变化时的损失变化模式来检测未学习数据的残留影响，比现有单点评估方法更敏感和可靠。


<details>
  <summary>更多</summary>
  
**动机:** 现有遗忘评估方法主要在单个输入层面进行评估，可能忽略语义相似样本中的残留影响，这种影响会损害隐私并导致间接信息泄露。

**方法:** 提出REMIND方法，分析模型在输入微小变化时的损失变化模式，通过检测损失曲面的平坦度来识别未学习数据的残留记忆。

**结果:** 实验显示未学习数据产生更平坦、更不陡峭的损失曲面，而保留或无关数据表现出更尖锐、更易变的模式。REMIND在仅需查询访问的情况下优于现有方法，并在不同模型、数据集和改写输入上表现出鲁棒性。

**结论:** REMIND提供了一个更敏感和可解释的遗忘效果衡量标准，为评估语言模型中的遗忘提供了可靠框架，并为记忆和遗忘研究提供了新视角。
...
**AI概要:** 研究发现当前预训练方法未能充分利用预训练数据集中的信息，通过检索增强生成技术可以显著提升模型性能，相当于将计算效率提升约5倍


<details>
  <summary>更多</summary>
  
**动机:** 尽管研究人员致力于改进预训练数据集，但很少有人关注预训练机制从数据中提取知识和想法的效率如何

**方法:** 使用检索增强生成技术和测试时计算来量化预训练过程中遗留的数据集价值，并分析这种价值在不同规模下的变化

**结果:** 在MMLU、Math-500和SimpleQA等基准测试中，预训练后从标准开源数据集检索带来了显著的准确率提升，MMLU上检索相当于比单独预训练多5倍计算量。LLaMA 3.1 8B模型在MMLU上获得10个百分点的提升

**结论:** 当前的预训练方法未能充分利用现有预训练数据集中的信息，仍有很大的改进空间
...
**AI概要:** 本文提出了一种基于图的方法来进行主题标注，通过分析主题词之间的语义关系来生成有意义且可解释的主题标签，相比传统方法在计算效率和效果上都有优势。


<details>
  <summary>更多</summary>
  
**动机:** 现有主题模型生成的主题词分布缺乏清晰的解释性，且传统方法计算成本高昂，需要一种既能提高可解释性又计算高效的主题标注方法。

**方法:** 采用基于图的方法，通过丰富主题词的语义相关词汇并分析它们之间的关系，从中推导出准确描述主题含义的标签。

**结果:** 在两个数据集上的实验表明，该方法在BERTScore和余弦相似度指标上优于传统基准方法，与ChatGPT-3.5效果相当，同时保持计算高效性。

**结论:** 图基方法为计算高效的主题标注提供了有效途径，未来可进一步探索提升可解释性和自动化水平的研究方向。
...
**AI概要:** SSPO提出句子级别的重要性比率，平衡GRPO和GSPO的优缺点，避免训练崩溃和方差问题，提高采样数据利用率，在五个数据集上取得最佳性能


<details>
  <summary>更多</summary>
  
**动机:** 现有RLVR算法如GRPO存在策略更新不稳定问题，GSPO存在采样数据利用率低的问题，需要一种平衡的解决方案

**方法:** 引入句子级别重要性比率，应用句子熵到PPO-CLIP中动态调整裁剪边界，鼓励高熵token探索并限制低熵token的裁剪范围

**结果:** 在五个数据集上平均得分46.57，超越GRPO(43.01)和GSPO(44.42)，在三个数据集上达到最先进性能

**结论:** SSPO有效利用了生成数据，继承了GSPO的优点但避免了其缺点，在强化学习验证奖励方面表现出色
...
**AI概要:** 本文提出了一种基于可学习性评分和批量选择策略的机器翻译数据选择方法，通过学习者模型与预训练参考模型的协同作用，显著提升了数据效率和翻译性能。


<details>
  <summary>更多</summary>
  
**动机:** 数据质量及其有效选择是提升机器翻译模型性能的基础，但现有方法在数据选择效率和训练效果方面仍有改进空间。

**方法:** 提出可学习性评分系统评估数据点的训练效用，采用考虑数据点间相互依赖关系的批量选择策略，利用学习者模型与预训练参考模型的协同作用。

**结果:** 在英译波斯等语言对上实现了高达5倍的数据效率提升，使用缓存嵌入时计算效率提高24%，翻译性能优于随机选择方法。

**结论:** 该方法通过智能数据选择显著提升了机器翻译系统的训练效率和泛化能力，为构建更稳健可靠的翻译系统提供了有效解决方案。
...
**AI概要:** 研究测试大型语言模型在时间推理方面的能力，使用1940年挪威书籍中的知识问答，让模型以1940年的视角回答问题，比较英文和挪威文提示的效果。


<details>
  <summary>更多</summary>
  
**动机:** 探索LLMs在时间推理方面的能力，特别是处理历史语境下的知识问答任务。

**方法:** 使用1940年挪威书籍中的知识问题，用英文和挪威文提示LLMs回答问题，采用LLM-as-judge评分机制，人工抽查验证。测试了DeepSeek-R1、Gemma3、Qwen3、Llama3.1等模型系列以及专门为挪威语定制的大型模型。

**结果:** 英文提示的效果始终优于挪威文提示（意外结果），使用更大的LLMs能改善结果。

**结论:** LLMs在时间推理方面表现出一定能力，但语言选择对性能有显著影响，模型规模也是重要因素，需要进一步研究语言和文化背景对模型表现的影响。
...
**AI概要:** PTTSD是一个概率性文本时间序列抑郁检测框架，通过临床访谈文本预测PHQ-8抑郁评分，提供不确定性估计和时间建模，在E-DAIC和DAIC-WOZ数据集上达到最先进性能。


<details>
  <summary>更多</summary>
  
**动机:** 现有抑郁严重程度预测模型缺乏不确定性估计和时间建模能力，无法为临床决策提供充分支持。

**方法:** 提出PTTSD框架，包含序列到序列和序列到单值两种变体，结合双向LSTM、自注意力机制、残差连接，使用高斯或Student-t输出头并通过负对数似然训练。

**结果:** 在E-DAIC数据集上MAE=3.85，在DAIC数据集上MAE=3.55，达到纯文本系统的最先进性能，产生良好校准的预测区间。

**结论:** 注意力机制和概率建模具有重要价值，不确定性感知预测具有可解释性和临床相关性，与MentalBERT的比较证明了方法的通用性。
...
**AI概要:** ThaiOCRBench是首个针对泰语文档视觉理解任务的综合基准测试，包含2,808个样本和13个任务类别，评估显示专有模型显著优于开源模型，特别是在细粒度文本识别和手写内容提取任务中。


<details>
  <summary>更多</summary>
  
**动机:** 现有多模态基准主要关注高资源语言，泰语在需要文档结构理解的任务中代表性不足，存在性能差距。

**方法:** 构建多样化人工标注数据集，在零样本设置下评估多种最先进的视觉语言模型（包括专有和开源系统），并进行详细的错误分析。

**结果:** 专有模型（如Gemini 2.5 Pro）表现优于开源模型，开源模型在细粒度文本识别和手写内容提取方面性能下降最明显。

**结论:** ThaiOCRBench为评估低资源、复杂文字环境下的VLMs提供了标准化框架，并指出了语言偏见、结构不匹配和幻觉内容等关键挑战，为改进泰语文档理解提供了可行见解。
...
**AI概要:** RUST-BENCH是一个新的表格推理基准测试，包含7966个问题，来自2031个真实世界表格，覆盖科学和体育两个领域，专门测试大语言模型在复杂、异构表格上的多跳推理能力。


<details>
  <summary>更多</summary>
  
**动机:** 现有表格推理基准主要使用小型、统一的表格，无法代表真实世界数据的复杂性，也无法全面评估大语言模型的推理能力。真实表格通常很长、异构且领域特定，需要跨数千个token进行多跳推理。

**方法:** 构建RUST-BENCH基准，包含7966个问题，来自2031个真实世界表格，涵盖两个领域：RB-Science（NSF资助记录）和RB-Sports（NBA统计数据）。该基准联合评估模型在规模、异构性、领域特异性和推理复杂性方面的表现。

**结果:** 实验显示，开源和专有大语言模型在异构模式和复杂多跳推理方面表现不佳，揭示了当前架构和提示策略的持续弱点。

**结论:** RUST-BENCH为推进表格推理研究建立了一个具有挑战性的新测试平台，突显了当前大语言模型在处理真实世界复杂表格数据时的局限性。
...
**AI概要:** OUNLP团队在TSAR-2025共享任务中提出了基于LLM提示的多轮文本简化方法，通过分析发现简化效果与源和目标CEFR级别差距相关，开发了MRS-Rule和MRS-Joint两种方法，使用GPT-4o生成，最终排名第7，后续改进显示以LLM简化结果作为起点可进一步提升性能。


<details>
  <summary>更多</summary>
  
**动机:** 基于对提示式文本简化方法的分析，发现文本简化性能与源CEFR级别和目标CEFR级别之间的差距高度相关，这一发现启发了多轮简化方法的研究。

**方法:** 提出了两种多轮简化方法：基于规则的多轮简化（MRS-Rule）和联合规则与LLM的多轮简化（MRS-Joint），均使用GPT-4o进行生成。

**结果:** 提交的系统在20个团队中排名第7。后续改进显示，以LLM简化候选作为起点可以进一步提升多轮简化性能。

**结论:** 多轮简化方法在可读性控制的文本简化任务中表现良好，特别是将LLM简化结果作为初始点的联合方法具有进一步改进的潜力，证明了CEFR级别差距对简化效果的重要性。
...
**AI概要:** 本研究系统评估了6个大型语言模型在Big Five人格框架下的表现，发现不同模型在人格维度上存在显著差异，神经质和外向性对温度参数敏感，架构特征影响人格稳定性。


<details>
  <summary>更多</summary>
  
**动机:** 随着LLMs在人类中心应用中的普及，理解其类人格行为对于负责任开发和部署越来越重要。

**方法:** 应用Big Five Inventory-2 (BFI-2)框架，在不同采样温度下评估六个LLM的人格特质表达，并使用层次聚类分析模型聚类。

**结果:** 发现五个维度中有四个存在显著差异，神经质和外向性对温度调整敏感，聚类分析显示架构特征可能导致特定模型具有稳定的人格特征。

**结论:** 研究结果为LLM中类人格模式的出现提供了新见解，为模型调优、选择和AI系统伦理治理提供了新视角。
...
**AI概要:** RAGalyst是一个自动化、与人类评估对齐的代理框架，用于评估特定领域的RAG系统，通过生成高质量合成QA数据集和优化LLM评估指标，在三个专业领域验证了RAG性能的领域依赖性。


<details>
  <summary>更多</summary>
  
**动机:** 现有RAG评估框架存在局限性：基于启发式指标无法捕捉领域特定细微差别，LLM-as-a-Judge方法缺乏与人类判断的验证对齐，特别是在专业安全关键领域评估困难。

**方法:** 开发RAGalyst框架，包含代理流水线生成高质量合成QA数据集（含代理过滤步骤保证数据保真度），通过提示优化改进Answer Correctness和Answerability两个LLM-as-a-Judge评估指标，在军事行动、网络安全和桥梁工程三个领域评估不同RAG组件。

**结果:** RAG性能高度依赖上下文，没有任何单一嵌入模型、LLM或超参数配置在所有领域都最优；分析了RAG中低Answer Correctness的最常见原因。

**结论:** RAGalyst系统化评估框架的必要性得到验证，能够帮助从业者发现领域特定的权衡并做出明智的设计选择，构建可靠有效的RAG系统。
...
**AI概要:** 论文提出了一个两部分的框架来处理放射学报告中的显性和隐性不确定性，并发布了Lunguage++基准数据集，支持不确定性感知的图像分类和诊断推理研究。


<details>
  <summary>更多</summary>
  
**动机:** 放射学报告对临床决策至关重要，但包含两种不确定性：显性不确定性（通过模糊表达体现）和隐性不确定性（由于信息省略造成），需要系统方法来量化和建模。

**方法:** 1) 通过专家验证的LLM基准排名量化显性不确定性，将模糊短语映射到概率值；2) 通过扩展框架建模隐性不确定性，基于专家定义的诊断路径添加特征性子发现。

**结果:** 开发了Lunguage++，这是对Lunguage基准的扩展版本，提供了不确定性感知的结构化放射学报告数据集。

**结论:** 该方法能够实现不确定性感知的图像分类、可靠的诊断推理，并为研究诊断不确定性的临床影响提供了新的资源。
...
**AI概要:** 该研究探索语言模型在思维链推理过程中是否隐含表示可能的替代推理路径，通过控制隐藏激活来预测和量化模型的不确定性，发现激活干预在模型尚未确定最终答案时最有效。


<details>
  <summary>更多</summary>
  
**动机:** 语言模型生成文本时，单个token的选择可能导致不同的推理路径，使得不确定性难以量化。研究者希望验证语言模型是否在生成过程中表示其可能采取的替代路径。

**方法:** 使用隐藏激活来控制并预测语言模型在思维链推理过程中的不确定性，通过实验分析激活干预与模型不确定性之间的关系。

**结果:** 研究发现模型在不同token处的不确定性与通过控制激活来引导模型的容易程度存在明显相关性，表明激活干预在模型有替代路径可用时最有效。隐藏激活还能预测模型的未来输出分布。

**结论:** 语言模型在推理过程中确实隐含表示了可能的路径空间，激活控制技术可用于理解和量化模型的不确定性，特别是在模型尚未确定最终答案的关键决策点。
...
**AI概要:** IntelliProof是一个基于大语言模型的交互式议论文分析系统，通过构建论证图来可视化论点结构和关系，提供分类、评分和解释功能，增强用户对论证质量的理解。


<details>
  <summary>更多</summary>
  
**动机:** 现有自动作文评分系统缺乏用户体验和可视化分析功能，需要一种能够将议论文结构语义与用户理解连接起来的交互式工具。

**方法:** 将议论文构建为论证图：论点作为节点，支持证据作为节点属性，边编码支持或攻击关系。使用LLM进行初始分类和评分，并提供可视化界面。

**结果:** 开发了IntelliProof系统，能够快速探索论证质量，提供定量连贯性指标，并保留人工监督能力。系统提供实时演示版本。

**结论:** IntelliProof成功填补了议论文结构语义与用户理解之间的鸿沟，为议论文分析提供了有效的交互式工具，结合了自动化分析和人工监督的优势。
...
**AI概要:** 最新开源大语言模型在代码生成中仍存在安全漏洞，研究发现早期报告的漏洞场景仍然有效，表明安全性与功能性之间存在权衡。论文提出了新的严重性度量标准Prompt Exposure (PE)和Model Exposure (ME)评分来评估和缓解LLM生成的漏洞风险。


<details>
  <summary>更多</summary>
  
**动机:** 随着基于大语言模型的编码助手在软件开发中的重要性增加，其生成的代码漏洞对网络安全的影响也日益关键。尽管已有多个LLM代码安全基准和改进方法，但这些方法对广泛使用的编码LLMs的实际影响程度尚不清楚。

**方法:** 研究显示最新开源模型在现实使用场景中仍然容易受到早期报告的漏洞攻击。为此，论文引入了Prompt Exposure (PE)这一新的严重性度量标准，综合考虑漏洞严重程度、生成概率和诱导漏洞代码生成的提示词表述。

**结果:** 研究发现安全性与功能性之间的权衡阻碍了漏洞的有效修补。基于PE度量，论文定义了Model Exposure (ME)评分，用于指示模型生成漏洞的严重性和普遍性。

**结论:** 论文提出了新的评估框架来帮助缓解最严重和普遍的漏洞，通过PE和ME评分系统为LLM代码安全提供了更全面的风险评估方法。
...
**AI概要:** 该论文提出了首个孟加拉语生物医学问答数据集BanglaMedQA和评估基准BanglaMMedBench，并测试了多种RAG策略在低资源语言医疗AI中的表现，其中Agentic RAG方法在GPT-oss-120b模型上取得了89.54%的最高准确率。
...
**方法:** 构建大规模孟加拉语生物医学多选题数据集，应用多种检索增强生成（RAG）策略（传统、零样本回退、代理式、迭代反馈和聚合RAG），结合基于教科书的检索和网络检索与生成推理，并通过OCR技术整合孟加拉语医学教科书语料库。

**结果:** Agentic RAG方法在openai/gpt-oss-120b模型上取得了89.54%的最高准确率，优于其他配置，并展现出优越的推理质量。

**结论:** 基于RAG的方法有潜力提高孟加拉语医疗问答的可靠性和可访问性，为多语言医疗人工智能的未来研究奠定了基础。
...
**AI概要:** DeReC是一个轻量级事实验证框架，使用密集检索和分类取代计算密集型LLM方法，在保持高准确性的同时大幅提升效率。


<details>
  <summary>更多</summary>
  
**动机:** 当前基于LLM的事实验证系统存在计算成本高和幻觉风险的问题，需要更高效实用的解决方案。

**方法:** 结合密集检索和专门分类的轻量级框架，利用通用文本嵌入替代自回归LLM方法。

**结果:** DeReC在RAWFC数据集上F1得分65.58%，超越L-Defense的61.20%；运行时间减少95%（RAWFC）和92%（LIAR-RAW）。

**结论:** 精心设计的基于检索的系统可以在专业任务中匹配甚至超越LLM性能，同时更适用于实际部署。
...
**AI概要:** LEASH是一种无需训练的解码算法，通过监控熵斜率和top-logit边际改善来自适应停止思维链推理生成，减少30-35%的token使用和27%的延迟，准确率仅下降10个百分点


<details>
  <summary>更多</summary>
  
**动机:** 传统思维链提示需要生成完整固定长度的推理过程，计算资源浪费严重，增加了token使用和延迟

**方法:** 提出LEASH算法，监控两个内在信号：token级熵的斜率和top-logit边际的改善程度，当两个信号达到平台期时停止生成

**结果:** 在GSM8K和AQuA-RAT基准测试中，平均减少30-35%的token生成和27%的延迟，准确率相比完整CoT下降10个百分点

**结论:** LEASH提供了一种简单高效的CoT解码替代方案，无需额外训练或监督，具有模型无关性
